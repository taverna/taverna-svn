...
public class WorkflowSummaryAsHTML {
	...
	static {
		...
		sb
				.append("  border-width: 1px; border-style: solid; border-color: #aaaaaa;\n");
		sb.append("}\n");
		sb.append("</style>");
		STYLE = sb.toString();
		sb = new StringBuffer();
	...
	public static String getSummary(ScuflModel model) {
		...
		/**
		 * for (int i = 0; i<processors.length; i++) { String resourceHost =
		 * processors[i].getResourceHost(); if (resourceHost !=
		 * Processor.ENACTOR) { if (resources.containsKey(resourceHost) ==
		 * false) { resources.put(resourceHost, new ArrayList()); }
		 * java.util.List processorForResource =
		 * (java.util.List)resources.get(resourceHost);
		...
		author = (author.equals("") ? "<font color=\"red\">no author</font>"
				: "<font color=\"green\">" + author + "</font>");

		String title = model.getDescription().getTitle();
		title = (title.equals("") ? "<font color=\"red\">no title</font>"
				: "<font color=\"green\">" + title + "</font>");

		String lsid = model.getDescription().getLSID();
		lsid = (lsid.equals("") ? "<font color=\"red\">no lsid</font>"
				: "<font color=\"green\">" + lsid + "</font>");

		sb
				.append("This report applies to the workflow titled '"
						+ title
						+ "' authored by '"
						+ author
						+ "' and with LSID '"
						+ lsid
						+ "'. The textual description, if any is shown below :<blockquote><em>"
						+ model.getDescription().getText()
						+ "</em></blockquote>");
		...
		for (Iterator i = resources.keySet().iterator(); i.hasNext();) {
			...
			sb
					.append("<tr><td valign=\"top\" bgcolor=\"#bcd2ee\"colspan=\"3\">Resources on   <code><b>"
							+ hostName
							+ "</b></code>, "
							+ usageList.size()
							+ " instance"
			...
			WSDLBasedProcessor[] wp = (WSDLBasedProcessor[]) temp
					.toArray(new WSDLBasedProcessor[0]);
			...
			for (int j = 0; j < wp.length; j++) {
				...
				Map operationToProcessorName = (Map) wsLocations
						.get(wsdlLocation);
			...
			SoaplabProcessor sp[] = (SoaplabProcessor[]) temp
					.toArray(new SoaplabProcessor[0]);
			...
			for (int j = 0; j < sp.length; j++) {
				...
				Map nameToProcessorNames = (Map) soaplabLocations
						.get(soaplabLocation);
				String appName = sp[j].getCategory()
						+ "::<font color=\"purple\">" + sp[j].getAppName()
						+ "</font>";
			...
			for (Iterator j = soaplabLocations.keySet().iterator(); j.hasNext();) {
				...
				sb.append("<tr>");
				sb.append("<td width=\"80\" valign=\"top\" rowspan=\"" + rows
						+ "\" bgcolor=\"#faf9d2\">Soaplab</td>");
				sb
						.append("<td colspan=\"2\" bgcolor=\"faf9d2\">Service rooted at <em>"
								+ location + "</em></td>");
				sb.append("</tr>");
				sb
						.append("<tr><td bgcolor=\"#eeeedd\">App category and name</td><td bgcolor=\"#eeeedd\">Processors</td></tr>");
				for (Iterator k = nameToProcessorName.keySet().iterator(); k
						.hasNext();) {
			...
			SeqhoundProcessor sq[] = (SeqhoundProcessor[]) temp
					.toArray(new SeqhoundProcessor[0]);
			...
			for (int j = 0; j < sq.length; j++) {
				...
				Map nameToProcessorNames = (Map) seqhoundLocations
						.get(seqhoundLocation);
				String methodName = "<font color=\"purple\">"
						+ sq[j].getMethodName() + "</font>";
			...
			for (Iterator j = seqhoundLocations.keySet().iterator(); j
					.hasNext();) {
				String location = (String) j.next();
				...
				sb.append("<tr>");
				sb.append("<td width=\"80\" valign=\"top\" rowspan=\"" + rows
						+ "\" bgcolor=\"#836fff\">SeqHound</td>");
				sb
						.append("<td colspan=\"2\" bgcolor=\"#836fff\">Service rooted at <em>"
								+ location + "</em></td>");
				sb.append("</tr>");
				sb
						.append("<tr><td bgcolor=\"#eeeedd\">Method name</td><td bgcolor=\"#eeeedd\">Processors</td></tr>");
				for (Iterator k = nameToProcessorName.keySet().iterator(); k
						.hasNext();) {
					String appName = (String) k.next();
					Set processorNames = (Set) nameToProcessorName.get(appName);
			...
			for (Iterator j = usageList.iterator(); j.hasNext();) {
				...
				if (p instanceof WSDLBasedProcessor == false
					...
					if (p instanceof BiomobyProcessor) {
						...
						sb.append("<td><font color=\"purple\">"
								+ bp.getServiceName() + "</font>&nbsp;in&nbsp;"
								+ bp.getEndpoint().getFile() + "</td>");
					} else if (p instanceof BiomartProcessor) {
						sb
								.append("<td bgcolor=\"d1eeed\" rowspan=\"2\">Biomart</td>");
