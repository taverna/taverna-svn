...
public class DataConstraint implements java.io.Serializable, Comparable {
	 ...
	 * [PROCESSOR]:[PORT] for source and sink. These must exist within the model
	 * specified.
	 * 
	 * @exception MalformedNameException
	 *                if the name supplied is not in the correct format
	 * @exception DataConstraintCreationException
	 *                if the ports are found but are incorrectly typed
	 * @exception UnknownPortException
	 *                if the processor is found but doesn't contain the named
	 *                port
	 * @exception UnknownProcessorException
	 *                if the model doesn't contain the particular processor name
	 */
	public DataConstraint(ScuflModel model, String source_name, String sink_name)
			...
			UnknownProcessorException, MalformedNameException {
		this(model, model.locatePortOrCreate(source_name, false), model
				.locatePortOrCreate(sink_name, true));
	}
	...
	/**
	 * Create a new data constraint from the source to the sink port. The ports
	 * must be instances of OutputPort and InputPort respectively
	 * 
	 * @exception DataConstraintCreationException
	 *                if the ports are found but are incorrectly typed
	 */
	public DataConstraint(ScuflModel model, Port source, Port sink)
		...
		if (model == null) {
			throw new DataConstraintCreationException(
					"Must specify a non null model for data constraint creation.");
		}
		this.model = model;
		...
		try {
			...
			throw new DataConstraintCreationException(
					"Sink port must be an instance of InputPort");
		}
		...
		try {
			...
			throw new DataConstraintCreationException(
					"Source port must be an instance of OutputPort");
		}

		// If the source port is an external input, set the type of it to the
		// same type as the sink
		if (this.source.getProcessor() == model.getWorkflowSourceProcessor()) {
			this.source.setSyntacticType(this.sink.getSyntacticType());
		}
		// If the sink port is an external input, set the type of it to the same
		// as the source
		...
		model.fireModelEvent(new ScuflModelEvent(this,
				"New data constraint created, '" + getName() + "'"));
	...
	public String getName() {
		...
		if (this.source.getProcessor() == model.getWorkflowSourceProcessor()) {
			...
			from = this.source.getProcessor().getName() + ":"
					+ this.source.getName();
		}
