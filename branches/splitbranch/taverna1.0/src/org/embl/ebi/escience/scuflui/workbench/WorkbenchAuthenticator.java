...
/**
 * Subclass of Authenticator which throws up the appropriate dialog in the
 * workbench when a request is made which requires a username / password of some
 * kind. Hopefully should resolve some of the issues with authenticating
 * proxies.
 * 
 * @author Tom Oinn
 */
public final class WorkbenchAuthenticator extends Authenticator {
	...
	public PasswordAuthentication getPasswordAuthentication() {
		...
		PasswordAuthentication pa = (PasswordAuthentication) credentials
				.get(host + port + prompt);
		if (pa != null) {
			// Return cached result
			return pa;
		}
		PasswordDialog pd = new PasswordDialog(this.hostFrame, true,
				"Password for " + host + ", " + prompt + " required.");
	...
	public PasswordDialog(JFrame frame, boolean modal, String myMessage) {
		...
		okay.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				result = new PasswordAuthentication(user.getText(), password
						.getPassword());
				setVisible(false);
			}
		});
