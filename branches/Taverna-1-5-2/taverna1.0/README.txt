...
6 	To run with the actual compiled components, rather than those fetched from the remote repository, first install the artifacts by running
	mvn install	
	Then edit the target/taverna-1.5.0-bin/taverna-1.5/runme.bat or target/taverna-1.5.2.0-bin/taverna-1.5.2.0/run.sh to add the argument -Draven.repository.0=file://path to local maven repository.
...
7 	Run Taverna from the new distribution directory  
	cd target/taverna-1.5.2.0-bin/taverna-1.5.2.0
