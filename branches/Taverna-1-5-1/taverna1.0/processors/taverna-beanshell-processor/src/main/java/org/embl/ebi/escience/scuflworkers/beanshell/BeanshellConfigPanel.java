...
public class BeanshellConfigPanel extends JPanel implements UIComponentSPI,
	...
	private abstract class PortTableModel extends AbstractTableModel {
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			...
			case 1:
				return new Integer(
					getListDepth(getPorts()[rowIndex].getSyntacticType()));
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
			if (columnIndex > 0) {
				...
				Iterator entryIterator =
					DataThing.mimeTypes.entrySet().iterator();
				...
				getPorts()[rowIndex].setSyntacticType(getSyntacticType(
					listCount, mimeType));
			}
		}
	...
	private class ListDepthRenderer extends DefaultTableCellRenderer implements
		ChangeListener {
	...
	private class JSpinnerEditor extends AbstractCellEditor implements
		TableCellEditor {
	...
	Action deletePortAction =
		new AbstractAction("Delete Port", TavernaIcons.deleteIcon) {
	...
	public BeanshellConfigPanel(BeanshellProcessor bp) {
		...
		deletePortAction.putValue(Action.ACCELERATOR_KEY,
		...
		ActionListener addInputAction = new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				try {
					...
					logger.warn("Could not create port " + portName, ex);
				} catch (DuplicatePortNameException ex) {
					logger.warn("Duplicate port " + portName, ex);
				}
			}
		};
		...
		outputTable.setDefaultEditor(String.class, new DefaultCellEditor(
			outputTypesCombo));
		outputTable.setDefaultRenderer(Port.class,
			new DefaultTableCellRenderer() {
				public Component getTableCellRendererComponent(JTable table,
					Object value, boolean isSelected, boolean hasFocus,
					int row, int column) {
		...
		addOutputField.getDocument().addDocumentListener(
		...
		addOutputButton =
			new JButton("Add Output", TavernaIcons.outputPortIcon);
