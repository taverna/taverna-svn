...
public abstract class GraphColours
{
	 ...
	 */
	public static final Color defaultForeground = getColour("black", null);
	/**
	 * The default background color (white).
	 */
	public static final Color defaultBackground = getColour("white", null);
	/**
	 * The default XOR color (light gray).
	 */
	public static final Color defaultXOR = getColour("light gray", null);
	/**
	 * The default font color (black).
	 */
	public static final Color defaultFontcolor = getColour("black", null);
	/**
	...
	{
		// gw: have i ever told you how much i love writing lexers by hand?
		...
		else if (Character.isDigit(first) || first == '.')
		{
			...
			while (N < 3)
			{
				...
				// i is still on delimiter if any...
				boolean comma = false;
				while (i < value.length())
				{
					char curr = value.charAt(i);
	...
	private static String canonColor(String name)
	{
		...
		for (int i = 0; i < array.length; i++)
		{
			...
			else if (Character.isLowerCase(array[i]) || Character.isDigit(array[i]))
			{
				array[len++] = array[i];
			}
			// else skip
	...
	public static Color getColour(String name, Color def)
	{
		...
		Color retColor = parseNumericColor(name);
		if (retColor == null)
		{
			String canonName = canonColor(name);
