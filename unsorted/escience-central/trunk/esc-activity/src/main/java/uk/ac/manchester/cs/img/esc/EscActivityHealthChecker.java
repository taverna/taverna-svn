 ...
 * 
 */
public class EscActivityHealthChecker implements
		HealthChecker<EscActivity> {

	public boolean canVisit(Object o) {
		// Return True if we can visit the object. We could do
		// deeper (but not time consuming) checks here, for instance
		// if the health checker only deals with EscActivity where
		// a certain configuration option is enabled.
		return o instanceof EscActivity;
	}
	...
	public VisitReport visit(EscActivity activity, List<Object> ancestry) {
		EscActivityConfigurationBean config = activity.getConfiguration();
