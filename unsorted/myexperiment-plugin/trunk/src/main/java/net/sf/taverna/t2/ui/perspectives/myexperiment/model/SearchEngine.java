...
public class SearchEngine
{
  ...
  /**
   * Creates an instance of a search engine that is aware of the current context of the search.
   */
  public SearchEngine(Vector<Long> currentSearchThreadIDVector, boolean bSearchByTag, SearchResultsPanel resultsPanel,
		  MainComponent pluginMainComponent, MyExperimentClient client, Logger logger)
  {
    super();
      ...
      public void run() {
        ...
        if (doc == null) {
          ...
          javax.swing.JOptionPane.showMessageDialog(
              null, "An error occurred while searching for the results of your query. Please try again.",
  ...
  /**
   * Generic worker method to process search results from both search
   * by tags and search by query.
   * 
   * @param foundSearchItemElements List of Elements that comprise the search results.
   */
  private void processSearchResults(Long thisSearchThreadID, List<Element> foundSearchItemElements)
  {
    ...
    for(Element foundItem : foundSearchItemElements)
    {
      ...
      if (!lThisSearchThreadID.equals(vCurrentSearchThreadID.get(0))) return;
    ...
    if (lThisSearchThreadID.equals(vCurrentSearchThreadID.get(0))) {
      SwingUtilities.invokeLater(new Runnable() {
        public void run() {
          jpResultsPanel.setStatus(lFoundSearchItemElements.size() + " items found " +(bSearchByTag ? "with tag '" : "by query '") + jpResultsPanel.getCurrentSearchTerm() + "'");
  ...
  public static class QuerySearchInstance implements Comparable<QuerySearchInstance>, Serializable
  {
    ...
    // constructor
    public QuerySearchInstance(String searchQuery, int resultCountLimit, boolean searchWorkflows, boolean searchFiles,
        boolean searchPacks, boolean searchUsers, boolean searchGroups)
    {
    ...
    public boolean equals(Object other)
    {
      if (other instanceof QuerySearchInstance)
      {
        ...
        return (this.strSearchQuery.equals(si.getSearchQuery()) && this.iResultCountLimit == si.getResultCountLimit() &&
            this.bSearchWorkflows == si.getSearchWorkflows() && this.bSearchFiles == si.getSearchFiles() &&
            this.bSearchPacks == si.getSearchPacks() && this.bSearchUsers == si.getSearchUsers() &&
            this.bSearchGroups == si.getSearchGroups());
      }
      else
        return (false);
    }
    ...
    public String detailsAsString()
    {
      ...
      int iCnt = 0;
      if (this.bSearchWorkflows) { str += "workflows, "; iCnt++; }
      if (this.bSearchFiles) { str += "files, "; iCnt++; }
      if (this.bSearchPacks) { str += "packs, "; iCnt++; }
      if (this.bSearchUsers) { str += "users, "; iCnt++; }
      if (this.bSearchGroups) { str += "groups, "; iCnt++; }
