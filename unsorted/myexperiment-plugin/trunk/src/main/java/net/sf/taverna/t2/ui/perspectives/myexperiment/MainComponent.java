...
public final class MainComponent extends JPanel implements UIComponentSPI, ChangeListener
{
  ...
  // components of the perspective
    ...
    if (myExperimentClient.getSettings().getProperty(MyExperimentClient.INI_DEFAULT_ANONYMOUS_TAB) == null)
      myExperimentClient.getSettings().put(MyExperimentClient.INI_DEFAULT_ANONYMOUS_TAB, "3");  // DEFAULT IS SEARCH TAB
    ...
    {
      // register the current instance of main component with the myExperiment perspective;
      // this will be used later on when shutdown operation needs to be performed - e.g.
  ...
  public SearchTabContentPanel getSearchTab()
  {
	...
	private void initialisePerspectiveUI()
	{
	  ...
	  // add listener to TabbedPane, so that the app "knows" when some tab was opened
    ...
    // check if 'auto-login' is required (NB! This requires the BASE_URL to be set correctly!)
  ...
  public void stateChanged(ChangeEvent e)
  {
    ...
    public PreviewResourceAction(int iResourceType, String strResourceURI) 
    {
      ...
      putValue(SHORT_DESCRIPTION,"Preview this " + Resource.getResourceTypeName(iResourceType).toLowerCase()
    ...
    public void actionPerformed(ActionEvent actionEvent) {
      getPreviewBrowser().preview("preview:" + this.iResourceType + ":" + this.strResourceURI);            
    }
  }
	...
	public class DownloadResourceAction extends AbstractAction 
  {
    ...
    public DownloadResourceAction(Resource resource, boolean bShowButtonLabel) 
    {
      ...
      // in either case the icon is the same; label might be displayed - based on the parameter
  ...
  public class ImportWorkflowAction extends AbstractAction
  {
    ...
    public void actionPerformed(ActionEvent actionEvent)
    {
      javax.swing.JOptionPane.showMessageDialog(null, "should import workflow: id = " + workflowId);
