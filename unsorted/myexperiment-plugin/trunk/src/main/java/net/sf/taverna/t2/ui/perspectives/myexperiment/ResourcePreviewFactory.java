...
{
  // CONSTANTS
  private static final int PREFERRED_LOWER_TABBED_PANE_HEIGHT = 250;  // used for all tabbed views inside preview for every resource type
  ...
  public ResourcePreviewContent createPreview(String action, EventListener eventHandler)
  {
    ...
    // parse the action string - we are now sure that it starts with a 'preview:'
    action = action.substring(action.indexOf(":") + 1);         // remove "preview:" 
    int iType = Integer.parseInt(action.substring(0, action.indexOf(":")));  // get type
    ...
    Document doc = null;
    try {
      // the resource type is known at this point, hence can use specialist method
      // that only fetches required metadata for (individual for each resource type) 
    ...
    switch (iType) {
      ...
      default:
        ...
        this.logger.error("Failed generating preview. Reason: unknown resource type - \"" + Resource.getResourceTypeName(iType) + "\"");
  ...
  private void generateWorkflowPreviewContent(Workflow w, JPanel panelToPopulate, EventListener eventHandler)
  {
    if (w != null) {
      try {
        ...
        content.append("<b>License: </b> <a href='" + w.getLicense().getLink() + "'>" + w.getLicense().getText() + "</a>"
            + "&nbsp;<img src='" + MyExperimentPerspective.getLocalResourceURL("external_link_small_icon") + "' />");
        content.append("</p>");
  ...
  private void generateFilePreviewContent(File f, JPanel panelToPopulate, EventListener eventHandler)
  {
    if (f != null) {
      try {
        ...
        content.append("<b>License: </b> <a href='" + f.getLicense().getLink() + "'>" + f.getLicense().getText() + "</a>"
            + "&nbsp;<img src='" + MyExperimentPerspective.getLocalResourceURL("external_link_small_icon") + "' />");
        content.append("</p>");
        ...
        jpFullFilePreview.setBackground(Color.WHITE);  // white background for the whole file preview panel
        ...
        c.fill = GridBagConstraints.VERTICAL;     // ..and fill all available space vertically
        ...
        //this.statusLabel.setText("File information found. Last fetched: " + new Date().toString());
      ...
      //statusLabel.setText("Could not find information for file ID: " + currentFileId);
      //clearContentTextPane();
      //disableButtons();
    }
  }
  ...
  private void generatePackPreviewContent(Pack p, JPanel panelToPopulate, EventListener eventHandler)
  {
    if (p != null) {
      try {
        ...
        jpFullPackPreview.setBackground(Color.WHITE);  // white background for the whole pack preview panel
        ...
        //this.statusLabel.setText("Pack information found. Last fetched: " + new Date().toString());
      ...
      //statusLabel.setText("Could not find information for pack ID: " + currentPackId);
      //clearContentTextPane();
      //disableButtons();
    }
  }
    ...
    if (u != null) {
      try {
        ...
        if (!u.getDescription().equals("")) {
          // HACK: the way JAVA renders html causes styling not to be inherited; hence need to
        ...
        jpFullUserPreview.setBackground(Color.WHITE);  // white background for the whole user preview panel
        ...
        //this.statusLabel.setText("User information found. Last fetched: " + new Date().toString());
    ...
    if (g != null) {
      try {
        ...
        while(iMembers.hasNext())
        {
          ...
          jpMembersTabContent.add(new JClickableLabel(uCurMember.getName(),
                                                      "preview:" + uCurMember.getItemType() + ":" + uCurMember.getURI(),
                                                      pluginMainComponent.getPreviewBrowser(),
                                                      new ImageIcon(MyExperimentPerspective.getLocalIconURL(uCurMember.getItemType()))
          ));
        }
        ...
        while(iSharedItems.hasNext())
        {
          ...
          jpSharedItemsTabContent.add(new JClickableLabel(rCurItem.getTitle(),
                                                          ...
                                                          pluginMainComponent.getPreviewBrowser(),
                                                          new ImageIcon(MyExperimentPerspective.getLocalIconURL(rCurItem.getItemType()))
          ));
        }
        ...
        jpFullGroupPreview.setBackground(Color.WHITE);  // white background for the whole group preview panel
        ...
        //this.statusLabel.setText("Group information found. Last fetched: " + new Date().toString());
      ...
      //statusLabel.setText("Could not find information for group ID: " + currentGroupId);
      //clearContentTextPane();
      //disableButtons();
    }
  }
  ...
  // *** Helper methods follow that generate particular reusable pieces of the previews ***
  ...
  private JScrollPane createWorkflowComponentPreviewTab(Workflow w)
  {
    ...
    if (!w.isTaverna1Workflow()) {
      ...
      JLabel lNotSupported = new JLabel("<html>This is " + w.getVisibleType() +
      ...
      // can display components for workflow of this type, but current user is not
      ...
      jpInputsWithTitle.add(new ShadedLabel("Inputs (" + vInputsData.size() + ")", ShadedLabel.BLUE, true), BorderLayout.NORTH);
      ...
      jpProcessorsWithTitle.add(new ShadedLabel("Processors (" + vProcessorsData.size() + ")", ShadedLabel.BLUE, true), BorderLayout.NORTH);
      ...
      jpOutputsWithTitle.add(new ShadedLabel("Outputs (" + vOutputsData.size() + ")", ShadedLabel.BLUE, true), BorderLayout.NORTH);
  ...
  private JScrollPane createPackItemPreviewTab(Pack p)
  {
    ...
    if (p.getItems().size() > 0)
    {
      ...
      for(PackItem piCurItem : p.getItems())
      {
        ...
        // item data is stored differently whether the item is internal or external
        if(piCurItem.isInternalItem()) {
          jpPackItemsTabContent.add(new JClickableLabel(piCurItem.getItem().getTitle(),
                                                        "preview:" + piCurItem.getItem().getItemType() + ":" + piCurItem.getItem().getURI(),
                                                        pluginMainComponent.getPreviewBrowser(),
                                                        new ImageIcon(MyExperimentPerspective.getLocalIconURL(piCurItem.getItem().getItemType()))
          ), c);
        }
        else {
          jpPackItemsTabContent.add(new JClickableLabel(piCurItem.getTitle(),
              piCurItem.getLink(), // link should open up directly in the web browser
              pluginMainComponent.getPreviewBrowser(),
              new ImageIcon(MyExperimentPerspective.getLocalIconURL(piCurItem.getItemType())),
              SwingConstants.LEFT,
              piCurItem.getTitle() + " (link: " + piCurItem.getLink() + ")"
          ), c);
        }
        ...
        jpWhoAddedTheItem.add(new JClickableLabel(piCurItem.getUserWhoAddedTheItem().getName(),
            "preview:" + Resource.USER + ":" + piCurItem.getUserWhoAddedTheItem().getURI(),
            pluginMainComponent.getPreviewBrowser()
        ), c1);
        ...
        c.weightx = 1.0;
        if (bNoCommentForPrevItem && (iCnt + 1 == p.getItems().size())) c.weighty = 1.0;
        ...
        if (! bNoCommentForPrevItem) {
          ...
          c.weightx = 1.0;
          if (iCnt + 1 == p.getItems().size()) c.weighty = 1.0; // only if this is the comment for the last item, shift all items to the top of the panel
  ...
  {
    TagCloudPanel jpTagTabContent = new TagCloudPanel("Resource tag cloud", TagCloudPanel.TAGCLOUD_TYPE_RESOURCE_PREVIEW, 
    ...
    // tag cloud panel itself already has a scroll pane in it; hence the outer scroll pane
    // is only used for consistency across the user interface (contents of all tabs in the
    // preview window are scroll panes); the preferred size of the tag cloud panel should
    // still be adjusted accordingly to the preferred size of the outer scroll pane
  ...
  private JScrollPane createCommentsPreviewTab(List<Comment> comments)
  {
    ...
    if (lComments.size() > 0)
    {
      ...
      JLabel lLoading = new JLabel("Loading comments...",
                                   new ImageIcon(MyExperimentPerspective.getLocalResourceURL("spinner")),
                                   SwingConstants.LEFT);
          ...
          SwingUtilities.invokeLater(new Runnable() {
            public void run() {
              ...
              for (Comment comment : lComments) {
                ...
                c.weighty = 0;
                c.gridwidth = 1;
                JClickableLabel lCommentAuthor = new JClickableLabel(comment.getUser().getName(),
                    "preview:" + comment.getUser().getItemType() + ":" + comment.getUser().getURI(),
                ...
                c.gridwidth = 2;
                c.fill = GridBagConstraints.HORIZONTAL;
                if (iCnt + 1 == lComments.size()) c.weighty = 1.0;
                
                JTextArea taCommentText = new JTextArea(Util.stripAllHTML(comment.getComment()));
  ...
  {
    // WRAPS HTML JTextPane PREVIEW AND A JTabbedPane WITH DETAILS INTO A SINGLE PREVIEW PANEL
    JPanel jpFullPreview = new JPanel();
    jpFullPreview.setBackground(Color.WHITE);  // white background for the whole preview panel
