...
public class MyStuffContributionsPanel extends JPanel implements ActionListener
{
  ...
  private static final int TOTAL_SECTION_VSPACING = SHADED_LABEL_HEIGHT + SECTION_VSPACING + SCROLL_PANE_PADDING + 5;  // the last literal is to cover all paddings / margins / etc
            ...
            public void run() {
              ...
              for (int i = 0; i < alVisiblePanelsWithHelperElements.size(); i++)
              {
                ...
                if ((jpContent.getPreferredSize().height + TOTAL_SECTION_VSPACING) < (iFullAvailableHeight / alVisiblePanels.size())) {
                  ...
                  iFullAvailableHeight -= (jpContent.getPreferredSize().height) + TOTAL_SECTION_VSPACING;
                }
                else
              ...
              if (alIndexesToScale.size() > 0)
              {
                ...
                {
                  d.height = (iFullAvailableHeight / alIndexesToScale.size()) - TOTAL_SECTION_VSPACING;
                  if (d.height > ((JPanel)alVisiblePanelsWithHelperElements.get(i)[2]).getPreferredSize().height + SCROLL_PANE_PADDING)
                    d.height = ((JPanel)alVisiblePanelsWithHelperElements.get(i)[2]).getPreferredSize().height + SCROLL_PANE_PADDING;
