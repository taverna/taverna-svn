...
public class AddRemoveFavouriteDialog extends JDialog implements ActionListener, ComponentListener 
{
  ...
  private void initialiseUI()
  {
    ...
    // add all components
    JLabel lInfo = new JLabel("<html><center>You are about to " + (this.bIsFavouriteBeingAdded ? "add" : "remove") + 
                              " \"" + resource.getTitle() + "\" " + resource.getItemTypeName() +
                              (this.bIsFavouriteBeingAdded ? " to" : " from") + " your favourites.<br/><br/>" +
  ...
  /**
   * Makes the dialog for adding / removing an item from favourites visible. Based
   * on the user actions, it might execute the adding / removing operation.
   * 
   * @return Returns an integer value which represents status of the operation:
   *          {@value #OPERATION_SUCCESSFUL} - favourite item was added / removed successfully;
   *          {@value #OPERATION_CANCELLED} - the user has cancelled operation;
   *          {@value #OPERATION_FAILED} - failed while adding / removing favourite item;
   */
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    if (e.getSource().equals(this.bAddRemoveFavourite))
    {
      ...
      JLabel lInfo = new JLabel(this.bIsFavouriteBeingAdded ? "Adding to favourites..." : "Removing from favourites...", 
          ...
          SwingUtilities.invokeLater(new Runnable() {
            public void run()
            {
              ...
              if (iOperationStatus == OPERATION_SUCCESSFUL) {
                ...
                contentPane.add(new JLabel("Item has been successfully " + (bIsFavouriteBeingAdded ? "added to" : "removed from") + " your favourites",
                                           new ImageIcon(MyExperimentPerspective.getLocalResourceURL("success_icon")),
                                           SwingConstants.LEFT), c);
              }
              else {
                // favourite wasn't added / removed - operation failed;
                // display error message
                contentPane.add(new JLabel("<html><center>Error occurred while " + (bIsFavouriteBeingAdded ? "adding" : "removing")
                                           + " the item " + (bIsFavouriteBeingAdded ? "to" : "from") + " your favourites:<br/>"
                                           + Util.retrieveReasonFromErrorXMLDocument(response.getResponseBody()) + "</center></html>",
                                           new ImageIcon(MyExperimentPerspective.getLocalResourceURL("failure_icon")),
                                           SwingConstants.LEFT), c);
              }
