...
public class MyExperimentClient
{
  ...
  private java.io.File fIniFileDir;          // a folder, where the INI file will be stored
  ...
  public void storeSettings()
  {
    ...
    // (as the storeSettings() can be called multiple times during the run-time of the program,
    //  it is crucial to restore the unencrypted values after write operation is completed)  
    this.iniSettings.put(MyExperimentClient.INI_LOGIN, strLogin);
    this.iniSettings.put(MyExperimentClient.INI_PASSWORD, strPass);
  }
  
  
  // Simulates a "login" action by verifying that login and password are correct and storing the "logged in" state
  // if successful. Stored authentication details will be submitted with all further server requests, hence
  // appearing as a real logging in to the user.
  public boolean doLogin(String strLogin, String strPassword) throws Exception
  {
    // Base64 encode login and password - then store these in an instance variable
    ...
    if(conn.getResponseCode() == HttpURLConnection.HTTP_OK)
    {
      ...
      // can fetch the 'current user' - get current user URI from current response
    ...
    else
    {
      ...
      throw new IOException("Received unexpected HTTP response code (" + conn.getResponseCode() + ") while testing login credentials at " + BASE_URL + "/whoami.xml");
    }
  }
  ...
  public void doLoginFromStoredCredentials()
  {
    ...
    else
    {
      ...
      try {
        ...
        // this is highly unlikely because the login credentials were validated successfully
        // just before this
        this.logger.error("Couldn't fetch user data from myExperiment (" + strCurrentUserURI + 
  ...
  // Simulates a "logout" action. Logging in and out in the plugin is only an abstraction created
  // for user convenience; it is a purely virtual concept, because the myExperiment API is
  // completely stateless - hence, logging out simply consists of "forgetting" the authentication
   ...
   * 
   * @param strURL The URL on myExperiment to issue GET request to.
   ...
   * @param strURL The URL on myExperiment with which the connection is established.
   * @param bIsGetRequest Flag for identifying type of the request. True when the current 
   *        connection executes GET request; false when it executes a POST request.
   * @return An object containing XML Document with server's response body and
   *         a response code. Response body XML document might be null if there
   *         was an error or the user wasn't authorised to perform a certain action.
   *         Response code will always be set.
  ...
  private ServerResponse doMyExperimentReceiveServerResponse(HttpURLConnection conn, String strURL, boolean bIsGETRequest) throws Exception
  {
    ...
    switch (iResponseCode)
    {
      case HttpURLConnection.HTTP_OK:
        // data retrieval was successful - parse the response XML and return it along with response code
        BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getInputStream()));
        Document doc = new SAXBuilder().build(new InputSource(reader));
        reader.close();
      ...
      default:
        // unexpected response code - raise an exception
        throw new IOException("Received unexpected HTTP response code (" + conn.getResponseCode() + ") while " +
            (bIsGETRequest ? "fetching data at " : "posting data to ") + strURL);
    }
  }
  
  
  // a method to fetch a user instance with full details (including avatar image)
   ...
   * 
   * @param iResourceType Type of the resource for which the XML data is to be fetched.
   *        This implies which elements are required to be selected.
   * @param strURI URI of the resource in myExperiment API.
   * @param iRequestType Determines the level of detail of data to be fetched from the API;
  ...
  public Document getResource(int iResourceType, String strURI, int iRequestType) throws Exception
  {
    ...
    {
      // it doesn't matter what kind of resource this is if all available data is requested anyway
      strURI += "&all_elements=yes";
    }
    else
    {
      // only required metadata is to be fetched; this depends on the type of the resource
      switch (iResourceType) {
        case Resource.WORKFLOW:
          strURI += "&elements=" + Workflow.getRequiredAPIElements(iRequestType);
          break;
        case Resource.FILE:
        ...
        case Resource.PACK_INTERNAL_ITEM:
          strURI += "&all_elements=yes"; // TODO determine which are required elements
          break;
   ...
   * @param strWorkflowURI URI of the workflow to be opened.
  ...
  public List<Workflow> getExampleWorkflows() {
    ...
    try {
      String strExampleWorkflowsGroupUrl = this.BASE_URL + "/group.xml?id=" + EXAMPLE_WORKFLOWS_GROUP_ID + "&elements=shared-items";
  ...
  public TagCloud getGeneralTagCloud(int size) {
    ...
    try {
      ...
      String strTagCloudURL = BASE_URL + "/tag-cloud.xml?num=" + (size > 0 ? ("" + size) : "all");
    ...
    logger.debug("Tag cloud retrieval successful; fetched " + tcCloud.getTags().size() + " tags from myExperiment");
    return (tcCloud);
  }
  ...
  public TagCloud getUserTagCloud(User user, int size)
  {
    ...
    try
    {
      // update user tags first (this happens concurrently with the other threads
      ...
      while(iTagsResourcesHashMaps.hasNext())
      {
        ...
        // fetch tag data from myExperiment (namely, number of times that this tag was applied)
      ...
      // a little preprocessing before tag selection - if "size" is set to 0, -1 or any negative number,
      ...
      logger.error("Failed midway through fetching user tags for user ID = " + user.getID() + "\n" + e);
    }
  ...
  /**
   * A helper to fetch workflows, files or packs of a specific user. This will only
   * make *one* request to the API, therefore it's faster than getting all the items
   * one by one.
   * 
   * @param user User instance for which the items are to be fetched.
   * @param iResourceType One of Resource.WORKFLOW, Resource.FILE, Resource.PACK
   * @param iRequestType Type of the request - i.e. amount of data to fetch.
   *                     One of Resource.REQUEST_SHORT_LISTING, Resource.REQUEST_FULL_LISTING,
   *                     Resource.REQUEST_FULL_PREVIEW, Resource.REQUEST_ALL_DATA.
   * @return An XML document containing data about all items in the amount that was specified.
   */
  public Document getUserContributions(User user, int iResourceType, int iRequestType)
  {
    ...
    try {
      ...
      strURL += MyExperimentClient.urlEncodeQuery(user.getResource()) + strElements;
  ...
  /**
   * Queries myExperiment API for all items that are tagged with particular type.
   * @param strTag The tag to search for. This will be URL encoded before submitting the query.
  ...
  public Document searchByTag(String strTag)
  {
    ...
    try {
      ...
      doc = this.doMyExperimentGET(BASE_URL + "/tagged.xml?tag=" + strUrlEncodedTag + Util.composeAPIQueryElements(null)).getResponseBody();
    }
    catch (Exception e) {
      logger.error("ERROR: Failed to fetch tagged items from myExperiment. Query tag was '" + strTag + "'\n" + e);
    }
  ...
  /**
   * Converts a tag list into tag cloud data by fetching tag application count for each
   * instance in the list.
   * @param tags Tag list to work on.
   */
  ...
  public void updateUserFavourites(User user)
  {
    ...
    try {
      ...
      logger.error("Failed to fetch favourites data from myExperiment for a user (URI: " + user.getURI() + "); exception:\n" + ex);
  ...
  /**
   * For each comment in the list fetches the user which made the comment, the date when it was made, etc.
   */
  ...
  public Document searchByQuery(QuerySearchInstance searchQuery)
  {
    ...
    try {
      // this will URL encode the query so that it can be directly inserted into the search URL
      ...
      if (strSearchFor.length() != 0)
      {
        ...
        if (strSearchFor.startsWith(",")) strSearchFor = strSearchFor.replaceFirst(",", "");
      ...
      strSearchURL = BASE_URL + "/search.xml?query=" + strUrlEncodedQuery + "&" + strParameters;
      ...
      logger.error("ERROR: Failed to run search on myExperiment. Query URL was'" + strSearchURL + "'\n" + e);
      e.printStackTrace();
    }
  ...
  {
    try {
      String strCommentData = "<comment><subject resource=\"" + resource.getResource() + "\"/><comment>" + strComment + "</comment></comment>";
      ServerResponse response = this.doMyExperimentPOST(BASE_URL + "/comment.xml", strCommentData);
      ...
      // will return the whole response object so that the application could decide
      // on the next steps
      return (response);
    }
    catch (Exception e) {
      logger.error("Failed while trying to post a comment for " + resource.getURI() + "\n" + e);
  ...
  {
    try {
      String strData = "<favourite><object resource=\"" + resource.getResource() + "\"/></favourite>";
      ServerResponse response = this.doMyExperimentPOST(BASE_URL + "/favourite.xml", strData);
      
      // will return full server response
      return (response);
    }
    catch (Exception e) {
      logger.error("Failed while trying to add an item (" + resource.getURI() + ") to favourites4"
          , e);
      return (new ServerResponse(ServerResponse.LOCAL_FAILURE, null));
    }
  }
  ...
  {
    try {
      // deleting a favourite is a two-step process - first need to retrieve the the
      // actual "favourite" object by current user's URL and favourited item's URL
      String strGetFavouriteObjectURL = BASE_URL + "/favourites.xml?user=" +
             MyExperimentClient.urlEncodeQuery(this.getCurrentUser().getResource()) +
             "&object=" + MyExperimentClient.urlEncodeQuery(resource.getResource());
      ...
      logger.error("Failed while trying to remove an item (" + resource.getURI() + ") from favourites\n" + e);
      e.printStackTrace();
      return (new ServerResponse(ServerResponse.LOCAL_FAILURE, null));
    }
  }
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    System.out.println(client.doMyExperimentGET(DEFAULT_BASE_URL + "/workflow.xml?id=56"));
    
    
    System.out.println("\n\nLogging in");
    System.out.println(client.doLogin("sergey", args[0]));
    System.out.println("\n\n");
    
    
    Element root = client.doMyExperimentGET(DEFAULT_BASE_URL + "/workflow.xml?id=56").getResponseBody().getRootElement();
