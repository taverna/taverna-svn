...
public class AddCommentDialog extends JDialog implements ActionListener, CaretListener, ComponentListener, KeyListener 
{
   ...
   * 
   * Window is simply closed if 'Cancel' button is pressed; on pressing 'Post Comment'
   * button the window will turn into 'waiting' state, post the comment and return the
   * resulting XML document (which would contain the newly added comment) back to the caller.
   *  
   * @return String value of the non-empty comment text to be sent to myExperiment or null if action was cancelled.
   */
  ...
  public void actionPerformed(ActionEvent e)
  {
    if (e.getSource().equals(this.bPost))
    {
      ...
      if (this.lStatusMessage != null) contentPane.remove(this.lStatusMessage);
      ...
      c.fill = GridBagConstraints.NONE;
      c.insets = new Insets(10, 5, 10, 5);
      lStatusMessage = new JLabel("Posting your comment...", 
          ...
          SwingUtilities.invokeLater(new Runnable() {
            public void run() {
              ...
              if (bPostingSuccessful) {
                ...
                lStatusMessage = new JLabel("Your comment was posted successfully",
                ...
                c.insets = new Insets(10, 5, 5, 5);
                lStatusMessage = new JLabel("Error occurred while posting comment: " + Util.retrieveReasonFromErrorXMLDocument(response.getResponseBody()),
    ...
    else if (e.getSource().equals(this.bCancel))
    {
      ...
      if (! this.bPostingSuccessful) this.strComment = null;
      this.dispose();
    }
  ...
  public void keyPressed(KeyEvent e)
  {
    ...
    if (e.getSource().equals(this.taComment) && e.getKeyCode() == KeyEvent.VK_TAB)
    {
