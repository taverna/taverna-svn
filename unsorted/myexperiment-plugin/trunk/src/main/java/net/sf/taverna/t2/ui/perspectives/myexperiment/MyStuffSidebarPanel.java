...
public class MyStuffSidebarPanel extends JPanel implements ActionListener
{
  ...
  public MyStuffSidebarPanel(MainComponent component, MyExperimentClient client, Logger logger)
  {
    ...
    this.add(jpMyProfileBox);
    layout.putConstraint(SpringLayout.NORTH, jpMyProfileBox, 5, SpringLayout.NORTH, this);
    layout.putConstraint(SpringLayout.WEST, jpMyProfileBox, 5, SpringLayout.WEST, this);
    layout.putConstraint(SpringLayout.EAST, jpMyProfileBox, -5, SpringLayout.EAST, this);
    
    jpMyFriendsBox = createMyFriendsBox();
    this.add(jpMyFriendsBox);
    layout.putConstraint(SpringLayout.NORTH, jpMyFriendsBox, 5, SpringLayout.SOUTH, jpMyProfileBox);
    layout.putConstraint(SpringLayout.WEST, jpMyFriendsBox, 5, SpringLayout.WEST, this);
    layout.putConstraint(SpringLayout.EAST, jpMyFriendsBox, 0, SpringLayout.EAST, jpMyProfileBox);
    
    jpMyGroupsBox = createMyGroupsBox();
    this.add(jpMyGroupsBox);
    layout.putConstraint(SpringLayout.NORTH, jpMyGroupsBox, 5, SpringLayout.SOUTH, jpMyFriendsBox);
    layout.putConstraint(SpringLayout.WEST, jpMyGroupsBox, 5, SpringLayout.WEST, this);
    layout.putConstraint(SpringLayout.EAST, jpMyGroupsBox, 0, SpringLayout.EAST, jpMyProfileBox);
    
    jpMyFavouritesBox = createMyFavouritesBox();
    repopulateFavouritesBox();
    this.add(jpMyFavouritesBox);
    layout.putConstraint(SpringLayout.NORTH, jpMyFavouritesBox, 5, SpringLayout.SOUTH, jpMyGroupsBox);
    layout.putConstraint(SpringLayout.WEST, jpMyFavouritesBox, 5, SpringLayout.WEST, this);
    layout.putConstraint(SpringLayout.EAST, jpMyFavouritesBox, 0, SpringLayout.EAST, jpMyProfileBox);
    
    jpMyTagsBox = createMyTagsBox();
    this.add(jpMyTagsBox);
    layout.putConstraint(SpringLayout.NORTH, jpMyTagsBox, 5, SpringLayout.SOUTH, jpMyFavouritesBox);
    layout.putConstraint(SpringLayout.WEST, jpMyTagsBox, 5, SpringLayout.WEST, this);
    layout.putConstraint(SpringLayout.EAST, jpMyTagsBox, 0, SpringLayout.EAST, jpMyProfileBox);
    
    // this last panel is not visible, however, it acts as a "glue" component
    // between the last box in the sidebar and the bottom of the sidebar itself
    JPanel jpGlue = new JPanel();
    jpGlue.setBorder(BorderFactory.createEmptyBorder());
    this.add(jpGlue);
    layout.putConstraint(SpringLayout.NORTH, jpGlue, 0, SpringLayout.SOUTH, jpMyTagsBox);
    layout.putConstraint(SpringLayout.WEST, jpGlue, 5, SpringLayout.WEST, this);
    layout.putConstraint(SpringLayout.EAST, jpGlue, 0, SpringLayout.EAST, jpMyProfileBox);
    layout.putConstraint(SpringLayout.SOUTH, this, 0, SpringLayout.SOUTH, jpGlue);
  ...
  // creates a JPanel displaying the currently logged in user, logout button, etc
  private JPanel createMyProfileBox()
  {
    JPanel jpProfile = new JPanel();
    jpProfile.setMaximumSize(new Dimension(1024, 0));  // HACK: this is to make sure that profile box won't be stretched
    jpProfile.setLayout(new BoxLayout(jpProfile, BoxLayout.Y_AXIS));
    
    User currentUser = this.myExperimentClient.getCurrentUser();
    JLabel jlUserAvatar = new JLabel(Util.getResizedImageIcon(currentUser.getAvatar(), 60, 60));
    jlUserAvatar.setAlignmentX(CENTER_ALIGNMENT);
    jpProfile.add(jlUserAvatar);
    
    JClickableLabel jclUserName = new JClickableLabel(currentUser.getName(), "preview:" + Resource.USER + ":" + currentUser.getURI(),
    ...
    jpProfile.setBorder(BorderFactory.createCompoundBorder(
        BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " My Profile "),
  ...
  private JPanel createMyFriendsBox()
  {
    ...
    if (iFriends.hasNext()) {
      while(iFriends.hasNext())
      {
        ...
        jpFriends.add(new JClickableLabel(hmCurFriend.get("name"), "preview:" + Resource.USER + ":" + hmCurFriend.get("uri"), pluginMainComponent.getPreviewBrowser(), this.iconUser));
      }
    }
    else {
    ...
    jpFriends.setBorder(BorderFactory.createCompoundBorder(
        BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " My Friends "),
        BorderFactory.createEmptyBorder(1, 8, 8, 5)
      ));
  ...
  private JPanel createMyGroupsBox()
  {
    ...
    if (iGroups.hasNext()) {
      while(iGroups.hasNext())
      {
        ...
        jpGroups.add(new JClickableLabel(hmCurGroup.get("name"), "preview:" + Resource.GROUP + ":" + hmCurGroup.get("uri"), pluginMainComponent.getPreviewBrowser(), iconGroup));
      }
    }
    else {
    ...
    jpGroups.setBorder(BorderFactory.createCompoundBorder(
        BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " My Groups "),
        BorderFactory.createEmptyBorder(1, 8, 8, 5)
      ));
  ...
  private JPanel createMyFavouritesBox()
  {
    ...
    jpFavourites.setBorder(BorderFactory.createCompoundBorder(
        BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " My Favourites "),
        BorderFactory.createEmptyBorder(1, 8, 8, 5)
      ));
    ...
    if (iFavourites.hasNext()) {
      while(iFavourites.hasNext())
      {
        ...
        this.jpMyFavouritesBox.add(new JClickableLabel(rFavourite.getTitle(),
                                             ...
                                             new ImageIcon(MyExperimentPerspective.getLocalIconURL(rFavourite.getItemType()))
        ));
      }
    }
    else {
  ...
  private JPanel createMyTagsBox()
  {
    ...
    jpTags.setBorder(BorderFactory.createCompoundBorder(
        BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " My Tags "),
        BorderFactory.createEmptyBorder(1, 8, 8, 5)
      ));
  ...
  public void actionPerformed(ActionEvent e)
  {
    if(e.getSource().equals(bLogout))
    {
      ...
      // remove "My Tags" from the tags browser tab and rerun last searches (tag & keyword)
      ...
      // also, update another tabs, so that they don't display any 'private' content?
      // TODO
    }
  }
