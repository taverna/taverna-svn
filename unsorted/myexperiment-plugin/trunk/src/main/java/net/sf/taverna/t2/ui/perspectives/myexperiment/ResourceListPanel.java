	  ...
	  if (this.listItems != null) {
			...
			if (this.listItems.size() > 0)
			{
  			...
  			for (int i = 0; i < this.listItems.size(); i++) {
  				try {
  				  // this will make the layout manager to push all extra space in Y-axis
  				  // to go to the last element in the panel; essentially, this will push
  				  // all list items to the top of the list view panel
  				  if (i == listItems.size() - 1) gbConstraints.weighty = 1.0;
  				  ...
  				  this.listPanel.add(res.createListViewPanel(bFullSizeItemsList, pluginMainComponent, this, logger), gbConstraints);
  					logger.debug("Added entry in resource list panel for the resource (Type: " + res.getItemTypeName() + ", URI: " + res.getURI() + ")");
  				}
  				catch (Exception e) {
  					logger.error("Failed to add item entry to ResourceListPanel (Item Type : " + res.getItemTypeName() + ", URI: " + res.getURI() + ")", e);
  				}
  			}
			}
			else {
