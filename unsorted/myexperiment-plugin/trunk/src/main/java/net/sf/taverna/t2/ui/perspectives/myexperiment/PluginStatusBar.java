...
public class PluginStatusBar extends JPanel implements ActionListener
{
  ...
  public PluginStatusBar(MainComponent component, MyExperimentClient client, Logger logger)
  {
    ...
    this.setBorder(BorderFactory.createEmptyBorder(1, 4, 1, 1));  // this will add a bit more spacing on the left - before the status message
  ...
  // (alternatively it will be displayed at the time when the tab becomes active)
  public void setStatus(String strTabClassName, String strStatus)
  {
    ...
    if (isTabActive(strBaseClassName)) displayStatus(strBaseClassName);
  }
  ...
  // Determine whether the tab in the parameter is currently active in the main tabbed pane.
  private boolean isTabActive(String strTabClassName)
  {
    // get the current active tab (this is a normal class name of the main tab content component)
