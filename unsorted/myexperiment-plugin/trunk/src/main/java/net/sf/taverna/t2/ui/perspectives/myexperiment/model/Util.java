...
public class Util
{
  ...
  /** 
   * The method will scale down the <b>sourceImageIcon</b> in a way that it will fit into
   * the rectangle with dimensions <b>(iRequiredWidth, iRequiredHeight)</b> with the original
   * aspect ratio being preserved.
   * 
   * @param iRequiredWidth Maximum desired width of the resized image.
   * @param iRequiredHeight Maximum desired height of the resized image.
   */ 
   ...
   * 
   * @param sourceImageURL URL to fetch the source image from.
   * @param iRequiredWidth Maximum desired width of the resized image.
   * @param iRequiredHeight Maximum desired height of the resized image.
   * 
   * @return Local file URL which can be directly used to fetch the resized image.
   */
  ...
  /**
   * Instantiates primitive User object from XML element. This is much
   * more lightweight than User.buildFromXML() where full details of the
   * user can be obtained from XML.
   */
  ...
  /**
   * Instantiates primitive Tag object from XML element. Very lightweight method.
   */
  ...
  /**
   * Instantiates primitive Comment object from XML element. Very lightweight method -
   * faster and requires less data than Comment.buildFromXML().
   * 
   ...
   * 
   * The method will extract 3 attributes for every item in the collection:
   * 1) name of the item;
   * 2) URI of the item (to access this item via the API);
  ...
  public static int getAccessTypeFromXMLElement(Element privileges)
  {
    ...
    while (iPrivileges.hasNext())
    {
      ...
      if (iCurrentPrivilege > iAccessType) iAccessType = iCurrentPrivilege;
    }
  ...
  /**
   * Parses an XML document containing resources favourited by a User.
   * These resources are instantiated with the basic data available in
   * the XML document and collected into a single List object.
   */
  ...
  /**
   * Returns a list of credits - can be applied to any XML document
  ...
  /**
   * Returns a list of attributions - can be applied to any XML document
   * which contains "attributions" element.
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Returns a list of tags - can be applied to any XML document
   * which contains "tags" element.
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Returns a list of comments - can be applied to any XML document
   * which contains "comments" element. This should be called when
   * the Resource object (for which the list of comments is being obtained)
   * is known.
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Tiny helper to strip out HTML tags. Basic HTML tags like
   * &nbsp; and <br/> are left in place, because these can be
   * rendered by JLabel. This helps to present HTML content
   * inside JAVA easier.
   */
   ...
   * relatively to the other component.
   * 
   * This method is applicable when 'dependentComponent' fully fits within the 'mainComponent'.
   ...
   * This will produce a string in the form of "&elements=<element1>[,<element2>,...]
   * 
   * @param queryInstance Instance of {@link QuerySearchInstance} that contains details
   *                      on the current search by query or null value if search by tag
   *                      is being made.
   */
  public static String composeAPIQueryElements(QuerySearchInstance queryInstance)
  {
    ...
    while (elementSetIterator.hasNext()) {
      ...
      if (elementSetIterator.hasNext()) strElements += ",";
    }
