...
public class MyStuffTabContentPanel extends JPanel implements ActionListener, KeyListener, FocusListener
{
  ...
  // synchronisation latch to let main thread know that all component-creation threads have been finished
  ...
  public void createAndInitialiseInnerComponents()
  {
    ...
    if(this.myExperimentClient.isLoggedIn())
    {
      ...
      JScrollPane spSidebar = new JScrollPane(jpSidebar);
      spSidebar.getVerticalScrollBar().setUnitIncrement(ResourcePreviewBrowser.PREFERRED_SCROLL);
      spSidebar.setMinimumSize(new Dimension(jpSidebar.getMyProfileBox().getPreferredSize().width + 30, 0)); // +30 --> 10 for padding and 10 for vertical scroll bar + 10 extra
      ...
      // set proportional sizes of the two panes as 30/70 percents of the total width of the SplitPane
      // this can only be done after the SplitPane is made visible - hence the need for the listener below
      ...
      // the SplitPane will be the only component in the Panel
  ...
  private JPanel createLoginBox()
  {
    ...
    jpLoginBox.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEtchedBorder(), 
  ...
  public void actionPerformed(ActionEvent e)
  {
      ...
      new Thread("Login to myExperiment") {
        public void run() {
          ...
          try {
            ...
            if(bLoginSuccessful)
            {
              // store the settings anyway (for instance, to clear stored login/password - when the 'remember me' tick is not checked anymore);
            ...
            SwingUtilities.invokeLater(new Runnable() {
              public void run() {
                if(myExperimentClient.isLoggedIn())
                {
                  ...
                  // ..also, refresh tag search results because these my include much more than
                  // during the previous search when the user was still not logged-in
                  pluginMainComponent.getTagBrowserTab().rerunLastTagSearch();
                  
                  // ..also, refresh the keyword search results (as more items can now be found)
    ...
    {
      // is "Log in automatically" check box was checked - make sure that "Remember me" is also checked
      if (cbLoginAutomatically.isSelected()) cbRememberMe.setSelected(true);
    }
  ...
  {
    // ENTER pressed - check which element is the source and determine what acion is to be taken
    if (e.getKeyCode() == KeyEvent.VK_ENTER)
    {
      ...
      else if (e.getSource().equals(this.pfPassword) || e.getSource().equals(this.cbRememberMe) ||
          e.getSource().equals(this.cbLoginAutomatically) || e.getSource().equals(this.bLogin)) {
