...
public class TagCloudPanel extends JPanel implements ChangeListener, ItemListener, ActionListener, HyperlinkListener
{
  ...
  public TagCloudPanel(String title, int iTagCloudType, ActionListener clickHandler, MainComponent component, MyExperimentClient client, Logger logger)
  {
    ...
    // set parameters and the main variables to ensure access to myExperiment, logger and the parent component
    this.strTitle = title;
  ...
  public void refresh()
  {
    this.lCloudTitle.setText(strTitle + " <span style='color: gray;'>(Loading...)</span>");
  ...
  public void repopulate() {
    ...
    SwingUtilities.invokeLater(new Runnable() {
      public void run() {
        lCloudTitle.setText(strTitle + " <span style='color: gray;'>(currently showing " + tcData.getTags().size() + ")</span>");
        revalidate();
      }
    });
  }
  ...
  public void hyperlinkUpdate(HyperlinkEvent e)
  {
    if (e.getSource().equals(this.tpTagCloudBody) && e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
      // one of the tags was clicked, but click processing is off-loaded to 'clickHandler'
