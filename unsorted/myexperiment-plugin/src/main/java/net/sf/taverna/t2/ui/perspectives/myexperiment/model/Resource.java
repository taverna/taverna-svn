...
public class Resource implements Comparable<Resource>
{
  ...
  /**
   * Determines whether the current type of resource can be downloaded in general.
   */
  public boolean isDownloadable()
  {
    return(this.itemType == Resource.WORKFLOW || this.itemType == Resource.FILE || this.itemType == Resource.PACK);
  }
  
  
  /**
   * Determines whether the current resource instance can be downloaded by the current user.
   */
  ...
  /**
   * Will create a small preview panel for the instance of the resource that it holds.
   * Shows reduced amount of information when "bCreateFullSizeView" is set to false.
   * 
   * @return JPanel containing title, description and links to actions on this resource.
   */
  public JPanel createListViewPanel(boolean bCreateFullSizeView, MainComponent pluginMainComponent, EventListener eventHandler, Logger logger)
  {
    try {
      ...
      content.append("<a href='preview:" + this.getItemType() + ":" + this.getURI() + "'>" + this.getTitle()
                     + ((this.getItemType() == Resource.WORKFLOW) ? (" (version " + ((Workflow)this).getVersion() + ")") : "")
                     + "</a>");
      content.append("</p>");
      ...
      if (bCreateFullSizeView)
      {
        ...
        if (this.hasUploader()) {
          ...
          content.append("Uploader: <a href='preview:" + Resource.USER + ":" + this.getUploader().getURI() + "'>" + this.getUploader().getName() + "</a>");
          content.append("</p>");
        }
        else if (this.hasCreator()) {
          content.append("<p class='uploader'>");
          content.append("Creator: <a href='preview:" + Resource.USER + ":" + this.getCreator().getURI() + "'>" + this.getCreator().getName() + "</a>");
          content.append("</p>");
        }
        else if (this.hasAdmin()) {
          content.append("<p class='uploader'>");
          content.append("Administrator: <a href='preview:" + Resource.USER + ":" + this.getAdmin().getURI() + "'>" + this.getAdmin().getName() + "</a>");
          content.append("</p>");
        }
      ...
      if (this.itemType == Resource.WORKFLOW || this.itemType == Resource.USER)
      {
        ...
        URI previewURI = null;
        if (this.itemType == Resource.WORKFLOW) previewURI = ((Workflow)this).getThumbnail();
        else if (this.itemType == Resource.USER && ((User)this).getAvatarResource() != null) {
        ...
        if (bCreateFullSizeView) {
          if (!bManualResizeNeeded) content.append("<img class='preview' src='" + previewURI + "'></img>");
          else {
            String resizedImageURL = Util.getResizedImageIconTempFileURL(previewURI.toURL(),
                ResourceListPanel.THUMBNAIL_WIDTH_FOR_FULL_LIST_VIEW,
                ResourceListPanel.THUMBNAIL_HEIGHT_FOR_FULL_LIST_VIEW);
            content.append("<img class='preview' src='" + resizedImageURL + "'></img>");
          }
        }
        else {
          String resizedImageURL = Util.getResizedImageIconTempFileURL(previewURI.toURL(),
                                                                       ResourceListPanel.THUMBNAIL_WIDTH_FOR_SHORT_LIST_VIEW,
                                                                       ResourceListPanel.THUMBNAIL_HEIGHT_FOR_SHORT_LIST_VIEW);
          content.append("<img class='preview' src='" + resizedImageURL + "'></img>");
        }
      ...
      if (bCreateFullSizeView)
      {
        content.append("<p style='text-align: left;'><b><a href='" + this.getResource() + "'>Open in myExperiment</a></b>" +
            "&nbsp;<img style='border: 0px;' src='" + MyExperimentPerspective.getLocalResourceURL("external_link_small_icon") + "' /></p>");
      }
      ...
      if (bCreateFullSizeView)
      {
        ...
        jpButtonsPanel.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(new Color(235,235,235)),    // subtle darker border around the button bar
      ...
      logger.error("Failed while creating " + this.getItemTypeName() + " list view:\n" + e);
      return (null);
    }
  }
   ...
   * 
   * @param resourceTypeCode This code should be one of the resource type constants defined in Resource class.
  ...
  /**
   * This method will act as dispatcher for local buildFromXML() methods for each of
   * individual resource types. This way there's a generic way to turn XML content into
   * a Resource instance.
   */
