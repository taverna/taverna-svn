...
import static net.sf.taverna.t2.workflowmodel.utils.Tools.sanitiseName;
		...
		try {
			if (replaceAll) {
				...
				replaceAllMatchingActivities(activityClass, cacb, configString,
						rename, fileManager.getCurrentDataflow());
			} else
	...
	private void replaceAllMatchingActivities(Class<?> activityClass,
		...
		for (Processor p : d.getProcessors()) {
			...
			if (a.getClass().equals(activityClass)
				...
				replaceAllMatchingActivities(activityClass, cacb, configString,
						rename, ((NestedDataflow) a).getNestedDataflow());

		}
	}

	private void replaceActivity(ComponentActivityConfigurationBean cacb,
			Processor p, boolean rename, Dataflow d)
		...
		if (rename) {
			...
			String newName = uniqueProcessorName(sanitiseName(possibleName), d);
