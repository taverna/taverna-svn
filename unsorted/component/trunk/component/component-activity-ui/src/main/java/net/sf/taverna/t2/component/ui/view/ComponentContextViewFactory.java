...
public class ComponentContextViewFactory implements ContextualViewFactory<Dataflow> {
	...
	public boolean canHandle(Object selection) {
		if (selection instanceof Dataflow) {
			Object dataflowSource = fileManager.getDataflowSource((Dataflow) selection);
	...
	public List<ContextualView> getViews(Dataflow selection) {
		Object dataflowSource = fileManager.getDataflowSource((Dataflow) selection);
		return Arrays.<ContextualView>asList(new ComponentContextualView((ComponentVersionIdentification) dataflowSource));
	}
