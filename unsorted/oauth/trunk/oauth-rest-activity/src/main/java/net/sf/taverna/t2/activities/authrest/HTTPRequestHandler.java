...
import net.sf.taverna.t2.activities.authrest.RESTActivity.DATA_FORMAT;
...
private static final String CONTENT_TYPE_HEADER_NAME = "Content-Type";
  ...
  public static HTTPRequestResponse initiateHTTPRequest(String requestURL,
	  ...
	  return initiateHTTPRequest(requestURL, configBean, inputMessageBody, urlParameters, null);
  }

  public static HTTPRequestResponse initiateHTTPRequest(String requestURL,
      RESTActivityConfigurationBean configBean, Object inputMessageBody,
		Map<String, String> urlParameters, HTTPRequestHandlerAdapter adapter)
  {	  
	  // If an adapter is supplied, attempt to modify the request URL. 
	  if (adapter != null) {
		  requestURL = adapter.modify(requestURL);
	  }
    ...
    switch (configBean.getHttpMethod()) {
      case GET:    return (doGET   (connectionManager, requestURL, configBean, urlParameters, adapter));
      case POST:   return (doPOST  (connectionManager, requestURL, configBean, inputMessageBody, urlParameters, adapter));
      case PUT:    return (doPUT   (connectionManager, requestURL, configBean, inputMessageBody, urlParameters, adapter));
      case DELETE: return (doDELETE(connectionManager, requestURL, configBean, urlParameters, adapter));
  ...
  private static HTTPRequestResponse doGET(ClientConnectionManager connectionManager, String requestURL, RESTActivityConfigurationBean configBean,
    ...
    return (performHTTPRequest(connectionManager, httpGet, configBean, urlParameters, adapter));
  }
		...
		Map<String, String> urlParameters, HTTPRequestHandlerAdapter adapter)
  {
    ...
    return(performHTTPRequest(connectionManager, httpPost, configBean, urlParameters, adapter));
  }
		...
		Map<String, String> urlParameters, HTTPRequestHandlerAdapter adapter)
  {
    ...
    return (performHTTPRequest(connectionManager, httpPut, configBean, urlParameters, adapter));
  }
  ...
  private static HTTPRequestResponse doDELETE(ClientConnectionManager connectionManager, String requestURL, RESTActivityConfigurationBean configBean,
    ...
    return (performHTTPRequest(connectionManager, httpDelete, configBean, urlParameters, adapter));
  }
	...
	private static HTTPRequestResponse performHTTPRequest(
			...
			Map<String, String> urlParameters, 
			HTTPRequestHandlerAdapter adapter) {
		...
		// If an adapter is supplied, attempt to finalize the HTTP request. 
		if (adapter != null) {
			adapter.finalize(httpRequest);
		}
