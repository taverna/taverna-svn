...
import net.sf.taverna.t2.activities.authrest.HTTPRequestHandler.HTTPRequestResponse;
import net.sf.taverna.t2.activities.authrest.URISignatureHandler.URISignatureParsingException;
...
public class RESTActivity extends
	...
	/**
	 * Returns an HTTPRequestHandlerAdapter object, or <code>null</code> (if the HTTP request is not modified). 
	 * 
	 * @param inputs 
	 * @param callback
	 * @return  an HTTPRequestHandlerAdapter object, or <code>null</code>.
	 */
	protected HTTPRequestHandlerAdapter createHTTPRequestHandlerAdapter(Map<String, T2Reference> inputs, AsynchronousActivityCallback callback) {
		...
		callback.requestRun(new Runnable() {
			...
			public void run() {
				...
				.initiateHTTPRequest(completeURL, configBean,
						inputMessageBody, urlParameters, createHTTPRequestHandlerAdapter(inputs, callback));
