...
/**
 * this code populates the DD table in the T2Provenance DB. The table is used to test the naive lineage query algorithm. The
 * table contains all pairwise data dependencies between input and output variables for a given processor,
 * at the level of individual element within a list -- and for each processor in the graph. <br/>
 * This works offline, using the log of the iteration events generated by the EventsProcessor
 * @author paolo
 *
 */
public class DataDependenciesBuilder {
	...
	public DataDependenciesBuilder(String location, String clearDB) { 
		...
		try {
			pw = new MySQLProvenanceWriter(location);
...
//				System.out.println(filename);
			...
			System.out.println("*** fillXferSteps starting for wfInstanceID = "+wfInstanceID);
	...
	private void fillXferSteps(String wfInstanceID) throws SQLException {
		...
		//////////////
//		String wfInstance = null;  // TODO only support one instance query at this time
//		ArrayList<String> instances = (ArrayList<String>) pq.getWFInstanceIDs();  // ordered by timestamp
//
//		if (instances.size()>0)  {  
...
//		FileReader fr = new FileReader(new File(EVENTS_LOG_DIR+"/"+filename));
		...
		try {
			d = b.build (new FileReader(new File(EVENTS_LOG_DIR+"/"+eventFilename)));
			...
			String processor = processName[processName.length-1];  // 3rd component of composite name
