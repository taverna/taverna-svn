...
public class RshellActivity extends AbstractAsynchronousActivity<JsonNode> {
            ...
            public void run() {
                ...
                synchronized (lock) {
                    ...
                    try {
                        ...
                        // this service - use it to open a connection
                        if (hasUsernameAndPasswordForService(rServerURI)) {
                            ...
                            // Try without credentials - just do a simple assign
                            // and watch for any
                        ...
                        if (rSrvException.getMessage().toLowerCase()
                                .contains("authorization failed")) {
                            // The simple assign test failed because the
                            // connection
                            ...
                            callback.fail("Invalid RShell connection: "
                                    + rSrvException.getMessage(), rSrvException);
                        }
                    ...
                    try {
                        String script = configurationBean.get("script")
                                .textValue();
                        ...
                        if (script.length() != 0) {
                            ...
                            if (r.inherits("try-error")) {
                                ...
                                throw new RserveException(connection,
                                        errorString);
                            }
                        }
                        ...
                        REXP results = connection
                                .parseAndEval("try(eval(parse(text=.tmp.)),silent=TRUE)");
                        if (results.inherits("try-error")) {
                            String errorString = results.asString();
                            if (errorString.contains(": ")) {
                                errorString = errorString.substring(errorString
                                        .indexOf(": ") + 2);
                            }
                        ...
                        if (results.isList()) {
                            ...
                            assert (resultList.keys().length == outputPorts
                                    .size());
                        ...
                        callback.fail(
                                "RShell failed: " + rSrvException.getMessage(),
                                rSrvException);
                    } catch (ReferenceServiceException e) {
                        callback.fail("Error accessing input/output data", e);
                    } catch (REXPMismatchException e) {
                        callback.fail("RShell failed: " + e.getMessage(), e);
    ...
    private Object rExpToJava(REXP rExp, DataTypes dataType)
        ...
        case BOOL_LIST:
            ...
            if (rExp.isString()) {
                for (String s : rExp.asStrings()) {
                    boolResult.add(s.equalsIgnoreCase("true") ? stringTrue
                            : stringFalse);
                }
                return boolResult;
            }
     ...
     */
    protected UsernamePassword getUsernameAndPasswordForService(URI rServerURI)
            throws CMException {

        // Try to get username and password for this service from Credential
        // Manager (which should pop up UI if needed)
        UsernamePassword username_password = credentialManager
                .getUsernameAndPasswordForService(rServerURI, false, null);
        if (username_password == null) {
            throw new CMException("No username/password provided for service "
                    + rServerURI);
        }
        return username_password;
    }

    protected boolean hasUsernameAndPasswordForService(URI rServerURI)
            throws CMException {
