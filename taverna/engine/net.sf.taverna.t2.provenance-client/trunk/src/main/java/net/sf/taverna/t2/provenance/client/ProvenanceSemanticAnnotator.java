 ...
 * @author paolo
 *
 *         this util propagates workflow-level semantic annotations to traces
 *         for workflow runs. <br/>
 *         It takes a workflow annotation file with lines of the form<br/>
 *         <workflow_element> <property> <resource> <br/>
 *         and produces an RDF graph where values and processor instances are
 *         annotated. The resulting graph is saved in RDF/XML format
 *         <p/>
 *         It also produces a workflow-level graph for use by the provenance
 *         query preprocessor Usage: -Dconf=<config file> -
 */
public class ProvenanceSemanticAnnotator extends ProvenanceBaseClient {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length > 0) { // first parm is workflow annotation file
			...
			System.out
					.println("Usage: ProvenanceSemanticAnnotator [-Dconf=<config file>] <workflowAnnotationsFile> [<runID>]");
			System.exit(0);
		}
	...
	private ProvenanceAccess getPAccess() {
		return pAccess;
	}

	public ProvenanceSemanticAnnotator() throws Exception {
		super();
//		setUp();
		init();
	}

	public ProvenanceSemanticAnnotator(String resourceFile) throws Exception {
		super(resourceFile);
//		setUp();
		init();
	}
	 ...
	 * @param runID
	 *            if null, latest available run is used
	 * @return an RDF/XML file with instance-level annotations
	 * @throws IOException
	 */
	public String generateTraceAnnotations(String workflowAnnotationsFile, String runID)
			throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			if (type.equals(PROCESSOR_TYPE)) {
				// annotateProcessorBinding(element, splitLine[1], splitLine[2],
				// runID);
	...
	private void annotateVarBinding(String element, String property, String object, String runID) {
		...
		// for each of them, create a new triple with <value>, <property>
		// <object>
		logger.info(vbs.size() + " varbindings to be annotated");
		for (PortBinding vb : vbs) {
			logger.debug("about to annotate vb " + vb.getValue() + " with property " + property
					+ " and object " + object);
	...
	// private void annotateProcessorBinding(String pname, String property,
	// String object, String runID) {
	//
	...
	//
	// // perform actual annotation
	// Resource pResource = getModel().createResource(makeURI(pb.getPNameRef(),
	// runID));
	// Property p = m.createProperty(property);
	// pResource.addProperty(p, m.createResource(object));
	// }
	...
	// for each triple in the file, apply the annotation propagation rule as
	// follows:

	// locate instance-level trace records that are relevant for the workflow
	// element, and apply the appropriate rule
