...
/**
 * Creates a component that renders an individual data item from a data tree.
 * The component can render the result according to the renderers existing for
 * the 'guessed' MIME type.
 * 
 * Inspired by {@link net.sf.taverna.t2.workbench.views.results.workflow.RenderedResultComponent}
 * 
 * @author Alex Nenadic
 * 
 */
@SuppressWarnings("serial")
public class RenderedDataComponent extends JPanel {

	private static Logger logger = Logger
			.getLogger(RenderedDataComponent.class);

	// Panel containing rendered result
	private JPanel renderedDataPanel;
	...
	// Currently selected tree node to render, if any.
	private DataTreeNode node = null;
	...
	private InvocationContext dummyContext;
	
	// Registry of all existing 'save individual value' actions,
	...
	// Panel containing all 'save individual value' buttons
	JPanel saveButtonsPanel = null;

	public RenderedDataComponent(ReferenceService referenceService) {

		this.referenceService = referenceService;
		dummyContext = new InvocationContextImpl(referenceService, null);
		...
		for (SaveIndividualResultSPI action : saveActions) {
			...
			action.setInvocationContext(dummyContext);
		...
		// Top panel contains result type combobox
		...
		renderedDataPanel = new JPanel(new BorderLayout());
		renderedDataPanel.setBorder(new EmptyBorder(5, 5, 5, 5));

		// Add all components
		add(topPanel, BorderLayout.NORTH);
		add(new JScrollPane(renderedDataPanel), BorderLayout.CENTER);
	}
	 ...
	 */
	public void setNode(DataTreeNode node) {
		this.node = node;
		if (this.node.isState(DataTreeNodeState.DATA_ITEM)){
			updateResult(); // display the data item the node contains
		}
		else{
			clearResult(); // this is a container node (for lists of (lists of ... ) data items)
		}
	}

	/**
	 * Update the component based on the data node selected from the tree.
	 * 
	 */
	public void updateResult() {
		...
		// Reference to the data to be displayed
		t2Reference = node.getDataReference();
		...
		if (identified instanceof ReferenceSet) {
			...
			if (mimeList.length > 0) {
				...
				// need
				// render the result as we may have switched from a different
				// result item in
	...
	public void renderResult() {
		...
		if (mimeList != null && selectedIndex >= 0) {
			...
			renderedDataPanel.removeAll();
			renderedDataPanel.add(component, BorderLayout.CENTER);
			repaint();
			revalidate();
		}
	}
	...
	class ColorCellRenderer implements ListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			if (value != null && index >= recognisedRenderersForMimeType.size()) { // one
																					// of
																					// the
																					// non-preferred
																					// renderers
