...
public class Parallelize extends
		AbstractDispatchLayer<ParallelizeConfig, Object> implements
		NotifiableLayer,
		PropertyContributingDispatchLayer<ParallelizeConfig, Object> {

	private static Logger logger = Logger.getLogger(Parallelize.class);

	private Map<ProcessIdentifier, StateModel> stateMap = new HashMap<ProcessIdentifier, StateModel>();
	...
	public void eventAdded(ProcessIdentifier owningProcess) {
		...
		if (stateModel == null) {
			 ...
			 */
			throw new WorkflowStructureException("Unknown owning process "
					+ owningProcess);
		}
	...
	public void receiveError(DispatchErrorEvent errorEvent,
		...
		StateModel model;
		ProcessIdentifier owningProcess = errorEvent.getOwningProcess();
		synchronized (stateMap) {
			model = stateMap.get(owningProcess);
		}
		callback.sendError(errorEvent);
		if (model == null) {
			logger.warn("Error received for unknown owning process: "
					+ owningProcess);
			return;
		}
	...
	public void receiveResult(DispatchResultEvent resultEvent,
			DispatchLayerCallback callback, Object state) {
		StateModel model;
		ProcessIdentifier owningProcess = resultEvent.getOwningProcess();
		synchronized (stateMap) {
			model = stateMap.get(owningProcess);
		}
	...
	public void receiveResultCompletion(
			...
			DispatchLayerCallback callback, Object state) {
		StateModel model;
		ProcessIdentifier owningProcess = completionEvent.getOwningProcess();
		synchronized (stateMap) {
			model = stateMap.get(owningProcess);
		}
	 ...
	 * :
	 * <ul>
	 * <li><code>dispatch.parallelize.queuesize [Integer]</code><br/>
	 * The current size of the incomming job queue, or -1 if the state isn't
	 * defined for the registered process identifier (which will be the case if
	 * the process hasn't started or has had its state purged after a final
	 * completion of some kind.</li>
	 * </ul>
	 */
	public void injectPropertiesFor(final DispatchStack dispatchStack,
			final String owningProcess, Object stateModel) {
		/**
	...
	class StateModel {
		...
		private DispatchLayerCallback callback;
		...
		protected StateModel(DispatchJobQueueEvent queueEvent, int maxJobs,
				DispatchLayerCallback callback) {
				...
				while (queueEvent.getQueue().peek() != null
					...
					if (e instanceof Completion && pendingEvents.peek() == null) {
						new Thread(new Runnable() {
							public void run() {
								callback
										.sendResultCompletion(new DispatchCompletionEvent(
												e.getOwningProcess(), e
														.getIndex(), e
														.getContext()));
							}
						}).start();
					...
					if (e instanceof Job) {
						...
						sentJobsCount++;
						callback.sendJob(new DispatchJobEvent(e
								.getOwningProcess(), e.getIndex(), e
								.getContext(), ((Job) e).getData(), queueEvent
								.getActivities()));
					}
				}
			}
	...
	public Object createNewStateModel(Processor parent) {
		// This particular class handles state itself so not using this part of
		// the new API yet
		return null;
	}
