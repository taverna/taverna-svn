...
public class RshellActivity extends
	...
	public void configure(RshellActivityConfigurationBean configurationBean,
			Edits edits) throws ActivityConfigurationException {
		this.configurationBean = configurationBean;
		configurePorts(configurationBean, edits);
				...
				try {
					...
					for (ActivityInputPort inputPort : getInputPorts()) {
						...
						Object input = referenceService.renderIdentifier(
							inputId, symanticTypeToClass(symanticType), callback.getContext());
						...
						for (OutputPort outputPort : outputPorts) {
							...
							default: {
								...
								Object object = rExpToJava(expression,
										symanticType);
								outputData.put(portName, referenceService
										.register(object, 0, true, callback
												.getContext()));
							}
							}
						}
	...
	private Class<?> symanticTypeToClass(SymanticTypes symanticType) {
		switch (symanticType) {
		case BOOL:
		case DOUBLE:
		case INTEGER:
	...
	private REXP javaToRExp(Object value, SymanticTypes symanticType)
		...
		switch (symanticType) {
		case REXP: {
			byte[] binaryRepresentation = (byte[]) value;
			REXP rexp = new REXP();
			REXP.parseREXP(rexp, binaryRepresentation, 0);
			return rexp;
		}
		case BOOL: {
	...
	private Object rExpToJava(REXP rExp, SymanticTypes symanticType)
		...
		switch (symanticType) {
		case REXP: {
			byte[] binaryRepresentation = new byte[rExp.getBinaryLength()];
			rExp.getBinaryRepresentation(binaryRepresentation, 0);
...
//				return new String[] { (rExp.asBool().isTRUE()) ? "1.0" : "0.0" };
				return Collections.singletonList((rExp.asBool().isTRUE()) ? "1.0" : "0.0");
			case REXP.XT_DOUBLE:
//				return new String[] { Double.toString(rExp.asDouble()) };
				return Collections.singletonList(Double.toString(rExp.asDouble()));
			case REXP.XT_INT:
//				return new String[] { Double.toString(rExp.asInt()) };
				return Collections.singletonList(Double.toString(rExp.asInt()));
			default:
			}
			break;
		case INTEGER_LIST:
...
//				return new String[] { Integer.toString((int) rExp.asDouble()) };
				return Collections.singletonList(Integer.toString((int) rExp.asDouble()));
			case REXP.XT_INT:
//				return new String[] { Integer.toString(rExp.asInt()) };
				return Collections.singletonList(Integer.toString(rExp.asInt()));
			default:
			}
			break;
...
//				return new String[] { Boolean.toString(rExp.asBool().isTRUE()) };
				return Collections.singletonList(Boolean.toString(rExp.asBool().isTRUE()));
			case REXP.XT_DOUBLE:
//				return new String[] { Double.toString((int) rExp.asDouble()) };
				return Collections.singletonList(Double.toString((int) rExp.asDouble()));
			case REXP.XT_INT:
//				return new String[] { Integer.toString(rExp.asInt()) };
				return Collections.singletonList(Integer.toString(rExp.asInt()));
			case REXP.XT_VECTOR: {
