...
public class WSDLActivityHealthChecker implements HealthChecker<WSDLActivity> {
	...
	public HealthReport checkHealth(WSDLActivity activity,
		...
		try {
			...
			reports.add(new HealthReport("WSDL Activity",
					"Error whilst parsing the WSDL:" + e.getMessage(),
					Status.SEVERE));
			...
			reports.add(new HealthReport("WSDL Activity",
					"Communication error whilst parsing the WSDL:"
							+ e.getMessage(), Status.SEVERE));
		} catch (SAXException e) {
			reports.add(new HealthReport("WSDL Activity",
					"XML error whilst parsing the WSDL:" + e.getMessage(),
					Status.SEVERE));
		}

		Status status = highestStatus(reports);
		HealthReport report = new HealthReport("WSDL Activity", "", status,
				reports);
		...
		try {
			...
			if (connection instanceof HttpURLConnection) {
				...
				if (code != 200) {
					...
					report = new HealthReport("WSDL Test", "The WSDL [" + wsdl
							+ "] responded OK", Status.OK);
				}
			} else {
				report = new HealthReport(
						"WSDL Test",
						"The WSDL is not HTTP based which may affect workflow portability",
						Status.WARNING);
			}
	...
	private HealthReport testStyleAndUse(WSDLParser parser, String operationName) {
		...
		try {
			...
			if (use.equals("literal") && style.equals("rpc")) {
				...
				report = new HealthReport("Style and Use", style + "/" + use
						+ " is OK", Status.OK);
			}
	...
	private HealthReport testEndpoint(WSDLParser parser, String operationName) {
		...
		for (String endpoint : endpoints) {
			...
			try {
				...
				reports.add(new HealthReport("Endpoint test", "The endpoint["
		...
		if (reports.size() == 1) {
			...
			return new HealthReport("Enpoint test",
					"No service endpoint could be determined from the WSDL",
					Status.SEVERE);
		} else {
