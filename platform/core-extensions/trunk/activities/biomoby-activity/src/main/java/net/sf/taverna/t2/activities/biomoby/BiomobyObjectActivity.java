...
public class BiomobyObjectActivity extends AbstractAsynchronousActivity<BiomobyObjectActivityConfigurationBean> {
	...
	public void configure(
			BiomobyObjectActivityConfigurationBean configurationBean, Edits edits)
...
//									_newChild.addContent(_child.cloneContent());
    ...
    private void generatePorts(Edits edits) {
    	//inputs
		addInput("namespace", 0, null, edits);

		addInput("id", 0, null, edits);

		addInput("article name", 0, null, edits);
    	...
    	try {
    		...
    		processRelationships(relations, edits);
    		...
    		if (parent.equalsIgnoreCase("String")
    			...
    			addInput("value", 0, null, edits);
    		} else {
    			if (!parent.equalsIgnoreCase("Object"))
    				extractParentContainerRelationships(parent, edits);
    		}
    	...
    	//outputs
		addOutput("mobyData", 0, "text/xml", edits);
    }

    private void extractParentContainerRelationships(String string, Edits edits) {
        try {
    ...
    private void processRelationships(MobyRelationship[] relations, Edits edits) {
