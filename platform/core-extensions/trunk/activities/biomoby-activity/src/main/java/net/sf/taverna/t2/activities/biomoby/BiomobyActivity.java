...
public class BiomobyActivity extends
	...
	@Override
	public void configure(BiomobyActivityConfigurationBean configurationBean, Edits edits)
		...
		if (doInit) {
			...
			configurationBean.setServiceType(
					mobyService.getServiceType() == null ? 
							"Service" : 
		...
		callback.requestRun(new Runnable() {
			public void run() {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Service " + mobyService.getUniqueName() /* + proc.getName() */);
				...
				if (inputMap.containsKey("input")) {
					...
					try {
						...
						for (ActivityInputPort myInput : getInputPorts()) {
							...
							Object input = referenceService.renderIdentifier(
									inputMap.get(portName), String.class,
									callback.getContext());
							if (myInput.getDepth() == 0) {
								inputXML = (String) input;
						...
						for (int inCount = 0; inCount < invocations.length; inCount++) {
							...
							if (!XMLUtilities.isEmpty(invocations[inCount]))
								invocations[inCount] = executeService(serviceEndpoint,methodName,invocations[inCount]);
							if (logger.isDebugEnabled())
	...
	private void generatePorts(Edits edits) {
		...
		for (int x = 0; x < serviceInputs.length; x++) {
			if (serviceInputs[x] instanceof MobyPrimaryDataSimple) {
				...
				addInput(portName, inputDepth, null, edits);
			} else {
				// collection of items
				inputDepth = 1;
		...
		addInput("input", inputDepth, null, edits);
		...
		for (int x = 0; x < serviceOutputs.length; x++) {
			if (serviceOutputs[x] instanceof MobyPrimaryDataSimple) {
				...
				addOutput(outputName, outputDepth, "text/xml", edits);
			} else {
				outputDepth = 1;
				// collection of items
		...
		addOutput("output", outputDepth, "text/xml", edits);

	}

	
	/**
	 * Updates config bean secondaries for those that aren't already defined
	 * so this only records new ones, but doesn't overwrite existing ones.
	 */
	...
	protected void addOutput(String portName, int portDepth, String type, Edits edits) {
	...
	private void configureSecondaries() {
		if (configurationBean.getSecondaries().size()>0 && containsSecondaries()) {
			MobyData[] datas = getMobyService().getSecondaryInputs();
			for (Entry<String, String> entry : configurationBean.getSecondaries().entrySet()) {
	...
	throws MobyException
{
	try {
		...
		throw new MobyException("Unable to create SOAP document builder for MOBY asynchronous call submission",pce);
	...
	try {
		...
		do {
			...
			if(pollingInterval!=maxPollingInterval) {
				pollingInterval = (long)((double)pollingInterval*backoff/*proc.getBackoff()*/);
				if(pollingInterval>maxPollingInterval) {
