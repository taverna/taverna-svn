...
public class BeanshellActivityHealthChecker implements
		HealthChecker<BeanshellActivity> {
	...
	public HealthReport checkHealth(BeanshellActivity activity,
		...
		Parser parser = new Parser(new StringReader(activity.getConfiguration()
				.getScript()));
		try {
			while (!parser.Line())
				;
		} catch (ParseException e) {
			return new HealthReport("Beanshell Activity", e.getMessage(),
					Status.SEVERE);
		}
