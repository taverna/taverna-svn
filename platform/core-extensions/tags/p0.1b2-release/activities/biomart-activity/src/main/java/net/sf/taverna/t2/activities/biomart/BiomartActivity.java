 ...
 */
public class BiomartActivity extends AbstractAsynchronousActivity<Element> {
	...
	@Override
	public void configure(Element configurationBean, Edits edits)
			throws ActivityConfigurationException {
		this.configurationBean = configurationBean;
		biomartQuery = MartServiceXMLHandler.elementToMartQuery(
				configurationBean, null);

		List<Edit<?>> editList = new ArrayList<Edit<?>>();
		buildInputPorts(editList, edits);
		buildOutputPorts(editList, edits);
	...
	public void executeAsynch(final Map<String, T2Reference> data,
		...
		callback.requestRun(new Runnable() {
			...
			public void run() {
				final ReferenceService referenceService = callback.getContext()
						.getReferenceService();
				...
				try {
					...
					for (Filter filter : filters) {
						...
						if (data.containsKey(name + "_filter")) {
							Object filterValue = referenceService
									.renderIdentifier(data
											.get(name + "_filter"),
					...
					if (biomartQuery.getQuery().getFormatter() == null) {
						if (STREAM_RESULTS) {
										...
										public void receiveResult(
											...
											for (int i = 0; i < resultLine.length; i++) {
												...
												try {
													...
													throw new ResultReceiverException(
															e);
													// callback.fail("Failure when calling the reference service",
													// e);
												}
											}
										...
										public void receiveError(
											...
											for (Attribute attribute : attributes) {
												...
												int outputDepth = outputMap
														...
														.getDepth();
												try {
													T2Reference error = referenceService
							...
							for (Attribute attribute : attributes) {
								...
								outputData.put(outputName, referenceService
										.register(outputLists.get(outputName),
												outputDepth, true, callback
														.getContext()));
							}
							...
							for (int i = 0; i < resultList.length; i++) {
								...
								int outputDepth = outputMap.get(outputName)
										.getDepth();
								outputData.put(outputName, referenceService
										.register(resultList[i], outputDepth,
						...
						outputData.put(outputName, referenceService.register(
								resultList[0], outputDepth, true, callback
										.getContext()));
					}
					...
					callback.fail(
							"Failure when receiving a result from biomart", e);
				}
			}
	...
	private void buildInputPorts(List<Edit<?>> editList, Edits edits) {
		...
		for (Filter filter : filters) {
			...
			if (inputMap.containsKey(name)) {
				...
				editList
						.add(edits.getAddActivityInputPortEdit(this, inputPort));
				editList.add(createAddMimeTypeAnnotationEdit(inputPort,
						"text/plain", edits));
			}
		}
	...
	private void buildOutputPorts(List<Edit<?>> editList, Edits edits) {
		...
		if (formatter == null) {
			...
			for (Attribute attribute : attributes) {
				...
				if (outputMap.containsKey(name)) {
					...
					editList.add(edits.getAddActivityOutputPortEdit(this,
							outputPort));
					editList.add(createAddMimeTypeAnnotationEdit(outputPort,
							"text/plain", edits));
				}
			}
			...
			if (outputMap.containsKey(name)) {
				...
				OutputPort outputPort = edits.createActivityOutputPort(name, 0,
						0);
				newOutputMap.put(name, outputPort);
				editList.add(edits.getAddActivityOutputPortEdit(this,
						outputPort));
