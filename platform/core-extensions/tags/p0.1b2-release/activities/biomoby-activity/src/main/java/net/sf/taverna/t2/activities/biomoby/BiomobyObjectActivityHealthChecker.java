...
public class BiomobyObjectActivityHealthChecker implements HealthChecker<BiomobyObjectActivity> {
	...
	public boolean canHandle(Object activity) {
		return (activity!=null && activity instanceof BiomobyObjectActivity);
	}

	public HealthReport checkHealth(BiomobyObjectActivity activity, List<HealthChecker<?>> checkers) {
		String endpoint = activity.getConfiguration().getMobyEndpoint();
		...
		return new HealthReport("Biomoby Object Activity [" + endpoint + "]",
				message, status);
	}
