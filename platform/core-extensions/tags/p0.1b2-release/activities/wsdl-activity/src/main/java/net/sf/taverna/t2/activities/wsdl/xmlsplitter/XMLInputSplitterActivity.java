 ...
 * 
 */
public class XMLInputSplitterActivity extends
	...
	@Override
	public void configure(XMLSplitterConfigurationBean config, Edits edits)
			throws ActivityConfigurationException {
		configBean = config;
		configurePorts(configBean, edits);
		...
		callback.requestRun(new Runnable() {
			...
			public void run() {
				try {
					...
					Map<String, Object> inputMap = buildInputMap(data,
							referenceService);
			...
			private Map<String, T2Reference> createOutputData(
				...
				for (String outputName : outputMap.keySet()) {
					...
					result.put(outputName, referenceService.register(xmlOut, 0,
			...
			private XMLInputSplitter createSplitter() {
				...
				for (ActivityInputPortDefinitionBean defBean : getConfiguration()
				...
				return new XMLInputSplitter(typeDescriptor, inputNames
						.toArray(new String[] {}), inputTypes
						.toArray(new String[] {}), outputNames
						.toArray(new String[] {}));
			}
			...
			private Map<String, Object> buildInputMap(
				...
				for (String inputName : data.keySet()) {
					...
					result.put(inputName, referenceService.renderIdentifier(id,
	...
	/**
	 * Returns a TypeDescriptor for the given port name. If the port cannot be
	 * found, or is not based upon a complex type, then null is returned.
	 * 
	 * @param portName
	 * @return
	 */
