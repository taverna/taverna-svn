...
import net.sf.taverna.t2.workflowmodel.ProcessorOutputPort; // import
// net.sf.taverna.t2.workflowmodel.impl.AbstractDataflowEdit;
// import net.sf.taverna.t2.workflowmodel.impl.DataflowImpl;
// import net.sf.taverna.t2.workflowmodel.impl.Tools;
import net.sf.taverna.t2.workflowmodel.processor.activity.Activity;
import net.sf.taverna.wsdl.parser.TypeDescriptor;
import static net.sf.taverna.t2.workflowmodel.util.Tools.*;
...
public class AddXMLSplitterEdit implements Edit<Dataflow> {
	...
	protected void doEditAction(Dataflow dataflow) throws EditException {
		...
		if (displayName.startsWith(processorName)) {
			...
			if (activity instanceof XMLOutputSplitterActivity
					|| activity instanceof XMLInputSplitterActivity) {
				// For splitters on splitters - avoid adding up
				// blah_bluh_blih_more_stuff
		...
		String name = uniqueProcessorName(candidateName, dataflow);
		...
		EventForwardingOutputPort source = getSourcePort(sourceProcessor,
				sourceActivity, sourcePortName, linkUpEditList);
		EventHandlingInputPort sink = getSinkPort(sinkProcessor, sinkActivity,
				sinkPortName, linkUpEditList, edits);
		...
		linkUpEditList.add(getCreateAndConnectDatalinkEdit(dataflow, source,
				sink, edits));
	...
	private EventHandlingInputPort getSinkPort(Processor processor,
			Activity<?> activity, String portName, List<Edit<?>> editList,
			Edits edits) {
		InputPort activityPort = getActivityInputPort(activity, portName);
		// check if processor port exists
		EventHandlingInputPort input = getProcessorInputPort(processor,
	...
	private EventForwardingOutputPort getSourcePort(Processor processor,
			Activity<?> activity, String portName, List<Edit<?>> editList) {
		OutputPort activityPort = getActivityOutputPort(activity, portName);
		// check if processor port exists
		EventForwardingOutputPort output = getProcessorOutputPort(processor,
				activity, activityPort);
		if (output == null) {
			// port doesn't exist so create a processor port and map it
	...
	protected void undoEditAction(Dataflow dataflow) {
