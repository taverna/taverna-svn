...
public class XMLOutputSplitterHealthChecker implements
		HealthChecker<XMLOutputSplitterActivity> {
	...
	public HealthReport checkHealth(XMLOutputSplitterActivity activity,
		...
		try {
			...
			return new HealthReport("XMLOutputSplitter Activity",
					"Error reading the configuration XML:" + e.getMessage(),
					Status.SEVERE);
		} catch (IOException e) {
			return new HealthReport("XMLOutputSplitter Activity",
					"Error reading the configuration XML:" + e.getMessage(),
		...
		TypeDescriptor typeDescriptor = XMLSplitterSerialisationHelper
				.extensionXMLToTypeDescriptor(element);
		if (typeDescriptor == null) {
			return new HealthReport("XMLOutputSplitter Activity",
					"The datatype is NULL", Status.SEVERE);
		} else {
			return new HealthReport("XMLOutputSplitter Activity",
					"The datatype is declared OK", Status.OK);
		}
	}
