	...
	public void executeAsynch(final Map<String, T2Reference> data,
		...
		callback.requestRun(new Runnable() {
			...
			public void run() {
				...
				// String message =
				// "Unable to obtain the classloader for Beanshell activity";
				...
				// We're removing the dependency support for now to get this
				// working for David's demo
				...
				try {
					...
					for (String inputName : data.keySet()) {
						// Locate the port definition bean to get the input
						// object type
						Class<?> translatedType = String.class;
						for (ActivityInputPortDefinitionBean portDef : configurationBean
								.getInputPortDefinitions()) {
							if (portDef.getName().equals(inputName)) {
								// If the port definition bean actually had the
								// translated type in I could set it here, but
								// it doesn't!
								translatedType = portDef
										.getTranslatedElementType();
								break;
							}
						}
						Object input = referenceService.renderIdentifier(data
								.get(inputName), translatedType, callback
								.getContext());
					...
					callback.fail("Error evaluating the beanshell script "
							+ this, e);
