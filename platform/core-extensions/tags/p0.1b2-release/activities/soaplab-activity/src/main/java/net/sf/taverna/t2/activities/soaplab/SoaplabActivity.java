...
public class SoaplabActivity extends
	...
	@Override
	public void configure(SoaplabActivityConfigurationBean configurationBean, Edits edits)
	...
	@SuppressWarnings("unchecked")
	private void generatePorts(Edits edits) throws ActivityConfigurationException {
		...
		try {
			...
			for (int i = 0; i < inputs.length; i++) {
				...
				if (input_type.equals("string")) {
					addInput(input_name, 0,
							new ArrayList<Class<? extends ExternalReferenceSPI>>(), edits);
					inputTypeMap.put(input_name, String.class);
				} else if (input_type.equals("string[]")) {
					addInput(input_name, 1,
							new ArrayList<Class<? extends ExternalReferenceSPI>>(), edits);
					inputTypeMap.put(input_name, String.class);
			...
			for (int i = 0; i < results.length; i++) {
				...
				if ((!output_name.equalsIgnoreCase("detailed_status"))) {
					...
					if (output_type.equals("string")) {
						addOutput(output_name, 0, "text/plain", edits);
					} else if (output_type.equals("string[]")) {
						addOutput(output_name, 1, "text/plain", edits);
					} else if (output_type.equals("byte[]")) {
						addOutput(output_name, 0, "application/octet-stream", edits);
					} else if (output_type.equals("byte[][]")) {
						addOutput(output_name, 1, "application/octet-stream", edits);
					} else {
