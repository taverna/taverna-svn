...
public class Job extends IterationInternalEvent<Job> {
	...
	@Override
	public Job popIndex() {
		return new Job(owner.getParent(), getPoppedIndex(), dataMap, context);
	}
	 ...
	 * @param data
	 */
	public Job(ProcessIdentifier owner, int[] index,
			Map<String, T2Reference> data, InvocationContext context) {
	...
	public Job popOwningProcess() throws ProcessIdentifierException {
		return new Job(owner.getParent(), index, dataMap, context);
	}

	@Override
	public Job pushOwningProcess(String localProcessName)
			throws ProcessIdentifierException {
		return new Job(owner.createChild(localProcessName), index, dataMap,
				context);
	}
