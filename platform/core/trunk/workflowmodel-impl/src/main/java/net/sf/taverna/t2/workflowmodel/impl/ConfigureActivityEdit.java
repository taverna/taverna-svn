...
public class ConfigureActivityEdit extends AbstractActivityEdit {
	...
	public ConfigureActivityEdit(Activity<?> activity, Object configurationBean, Edits edits) {
		super(activity);
		this.edits = edits;
	...
	protected void doEditAction(AbstractActivity<?> activity)
		...
		try {
			((Activity) activity).configure(configurationBean, edits);
	...
	protected void undoEditAction(AbstractActivity<?> activity) {
		try {
			if (serializedBean == null) {
				...
				((Activity<Object>) activity).configure(bean, edits);
			}
		} catch (ActivityConfigurationException e) {
