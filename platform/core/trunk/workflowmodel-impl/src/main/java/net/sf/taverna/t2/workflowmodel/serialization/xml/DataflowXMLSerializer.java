...
public class DataflowXMLSerializer extends AbstractXMLSerializer {
	...
	protected Element serializeDataflow(Dataflow df, PluginManager manager)
		...
		try {
			...
			for (Processor processor : df.getProcessors()) {
				processors.addContent(processorToXML(processor, manager));
			}
			...
			// do datalinks
			result.addContent(DatalinksXMLSerializer.getInstance()
					.datalinksToXML(df.getLinks()));
			// do annotations
		}
		// FIXME: improve error reporting
			...
			throw new SerializationException(
					"There was a problem generating the XML for the dataflow: "
							+ jdomException.toString(), jdomException);
	...
	private Element processorToXML(Processor processor, PluginManager manager)
