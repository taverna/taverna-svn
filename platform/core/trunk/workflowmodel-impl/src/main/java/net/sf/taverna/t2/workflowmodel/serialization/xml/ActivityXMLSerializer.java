...
public class ActivityXMLSerializer extends AbstractXMLSerializer {
	...
	public Element activityToXML(Activity<?> activity, PluginManager manager)
			throws JDOMException, IOException {
		Element activityElem = new Element(ACTIVITY, T2_WORKFLOW_NAMESPACE);

		PluginIdentifier pid = manager.definingPlugin(activity);
		if (pid != null) {
			activityElem.addContent(pluginElement(pid));
		}
