...
public class DispatchStackXMLDeserializer extends AbstractXMLDeserializer {
	...
	public void deserializeDispatchStack(Processor processor,
			Element dispatchStack, PluginManager manager)
			throws ClassNotFoundException, InstantiationException,
			IllegalAccessException, EditException, DeserializationException {
		int layers = 0;
		for (Element layer : (List<Element>) dispatchStack.getChildren(
				DISPATCH_LAYER, T2_WORKFLOW_NAMESPACE)) {
			DispatchLayer<?,?> dispatchLayer = DispatchLayerXMLDeserializer
					.getInstance().deserializeDispatchLayer(layer, manager);
			edits.getAddDispatchLayerEdit(processor.getDispatchStack(),
					dispatchLayer, layers++).doEdit();
		}
