...
public abstract class AbstractFilteringInputPort extends
	...
	public void pushToken(WorkflowDataToken dt,
			ProcessIdentifier owningProcess, int desiredDepth) {
		if (!dt.isActive()) {
			return;
		}
	...
	public void receiveToken(WorkflowDataToken token) {
		...
		ProcessIdentifier newOwner = transformOwningProcess(token
				.getOwningProcess());
		if (filterDepth == -1) {
			...
			if (tokenDepth == filterDepth) {
				if (filterDepth == getDepth()) {
					 ...
					 * ContextManager .getDataManager(newOwner);
					 * Iterator<ContextualizedIdentifier> children = dManager
					 * .traverse(token.getData(), getDepth()); while
					 * (children.hasNext()) { ContextualizedIdentifier ci =
					 * children.next(); int[] newIndex = new
					 * int[token.getIndex().length + ci.getIndex().length]; int
					 * i = 0; for (int indx : token.getIndex()) { newIndex[i++]
					 * = indx; } for (int indx : ci.getIndex()) { newIndex[i++]
					 * = indx; } pushData(getName(), newOwner, newIndex,
					 * ci.getDataRef()); } pushCompletion(getName(), newOwner,
					 * token.getIndex());
					 */
				...
				if (token.getIndex().length == 0) {
					...
					while (currentDepth < filterDepth) {
						...
						newList.add(ref);
						ref = rs.getListService().registerList(newList,
								token.getContext()).getId();
						currentDepth++;
					}
	...
	protected abstract void pushCompletion(String portName,
			ProcessIdentifier owningProcess, int[] index,
	 ...
	 * @param data
	 */
	protected abstract void pushData(String portName,
			ProcessIdentifier owningProcess, int[] index, T2Reference data,
			InvocationContext context);
	 ...
	 * @return
	 */
	protected ProcessIdentifier transformOwningProcess(
			ProcessIdentifier oldOwner) {
		return oldOwner;
	}
