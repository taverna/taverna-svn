...
public class MergeImpl implements Merge {
	...
	private Map<ProcessIdentifier, List<T2Reference>> partialOutputsByProcess = new HashMap<ProcessIdentifier, List<T2Reference>>();
	...
	protected void receiveEvent(WorkflowDataToken token, String portName) {
		...
		ProcessIdentifier owningProcess = token.getOwningProcess();
		synchronized (partialOutputsByProcess) {
			outputList = partialOutputsByProcess.get(owningProcess);
			if (outputList == null) {
				int numPorts = getInputPorts().size();
				outputList = new ArrayList<T2Reference>(Collections.nCopies(
						numPorts, (T2Reference) null));
		...
		output.sendEvent(new WorkflowDataToken(owningProcess, newIndex, token
				.getData(), context));
			...
			synchronized (outputList) {
				...
				if (!outputList.contains(null)) {
					...
					IdentifiedList<T2Reference> registeredList = listService
							.registerList(outputList, token.getContext());
