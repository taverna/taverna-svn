...
public final class ProcessorImpl extends AbstractAnnotatedThing<Processor>
		implements Processor, FailureAware {
	...
	private TerminalProcessHandler completionHandler = null;
	...
	protected ProcessorImpl() {
		...
		dispatchStack = new DispatchStackImpl() {
			 ...
			 */
			@Override
			protected boolean conditionsSatisfied(
			 ...
			 */
			@Override
			protected void finishedWith(ProcessIdentifier owningProcess,
					InvocationContext context) {
				if (!controlledConditions.isEmpty()) {
					ProcessIdentifier enclosingProcess = owningProcess
							.getParent();
				...
				if (completionHandler != null) {
					completionHandler.completeTerminal(owningProcess,
							ProcessorImpl.this, context);
				}
			}
			...
			@Override
			protected void errorEventReceived(DispatchErrorEvent e) {
				if (ProcessorImpl.this.failureHandler != null) {
					ProcessorImpl.this.failureHandler.processFailed(e
							.getOwningProcess(), e.getContext(),
							ProcessorImpl.this, e.getMessage(), e.getCause());
				}
			}
	...
	public void fire(ProcessIdentifier enclosingProcess,
			InvocationContext context) {
		Job newJob = new Job(enclosingProcess.createChild(this.name),
				new int[0], new HashMap<String, T2Reference>(), context);
	 ...
	 */
	@SuppressWarnings("unchecked")
	void registerWithMonitor(ProcessIdentifier process, MonitorReceiver monitor) {
		...
		String processID = process + ":" + getLocalName();
		...
		for (DispatchLayer<?, ?> layer : layers) {
			if (layer instanceof PropertyContributingDispatchLayer) {
				Object layerStateModel = null;
				int layerIndex = layers.indexOf(layer);
				if (layer.getStateScope().equals(
						DispatchLayerStateScoping.PROCESS)) {
					layerStateModel = dispatchStack.getStateForLayer(
							layerIndex, processID);
				}
				((PropertyContributingDispatchLayer<?, Object>) layer)
						.injectPropertiesFor(dispatchStack, processID,
								layerStateModel);
			}
		}
		...
		monitor.registerNode(this, process.createChild(getLocalName()),
				properties);
	}

	public void purgeState(ProcessIdentifier parentProcess) {
		// TODO - purge state in crystalizer, iteration strategy and any
		// activities
		System.out.println("State purge requested for '" + parentProcess
				+ "' on Processor '" + getLocalName());
	}
	...
	public void setCompletionHandler(TerminalProcessHandler completionHandler) {
