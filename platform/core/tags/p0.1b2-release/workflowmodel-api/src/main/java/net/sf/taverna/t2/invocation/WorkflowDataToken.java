...
public class WorkflowDataToken extends Event<WorkflowDataToken> {
	 ...
	 * @param dataRef
	 */
	public WorkflowDataToken(ProcessIdentifier owningProcess, int[] index,
	...
	public WorkflowDataToken popOwningProcess()
		...
		return new WorkflowDataToken(owner.getParent(), index, dataRef, context);
	}

	@Override
	public WorkflowDataToken pushOwningProcess(String localProcessName)
			throws ProcessIdentifierException {
		return new WorkflowDataToken(owner.createChild(localProcessName),
				index, dataRef, context);
	}
