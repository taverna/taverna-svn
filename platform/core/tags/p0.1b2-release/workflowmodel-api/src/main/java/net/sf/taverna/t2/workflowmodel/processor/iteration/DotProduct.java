...
public class DotProduct extends CompletionHandlingAbstractIterationStrategyNode {
	...
	Map<ProcessIdentifier, TreeCache[]> ownerToCache = new HashMap<ProcessIdentifier, TreeCache[]>();

	@Override
	public synchronized void innerReceiveJob(int inputIndex, Job newJob) {
		ProcessIdentifier owningProcess = newJob.getOwningProcess();
	...
	@Override
	protected synchronized void cleanUp(ProcessIdentifier owningProcess) {
