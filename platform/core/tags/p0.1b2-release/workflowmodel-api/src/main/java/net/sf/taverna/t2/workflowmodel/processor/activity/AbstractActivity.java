...
public abstract class AbstractActivity<ConfigType> extends
	 ...
	 */
	public abstract void configure(ConfigType conf, Edits edits)
	 ...
	 * (non-Javadoc)
	 * 
	 * @seenet.sf.taverna.t2.workflowmodel.processor.activity.Activity#
	 * getInputPortMapping()
	 */
	 ...
	 * 
	 * @see
	 * net.sf.taverna.t2.workflowmodel.processor.activity.Activity#getInputPorts
	 * ()
	 */
	 ...
	 * 
	 * @see
	 * net.sf.taverna.t2.workflowmodel.processor.activity.Activity#getOutputPorts
	 * ()
	 */
	 ...
	 */
	protected final void addInput(String portName, int portDepth,
	...
	List<Class<? extends ExternalReferenceSPI>> handledReferenceSchemes,
		...
		inputPorts.add(edits.createActivityInputPort(portName, portDepth,
	 ...
	 */
	protected final void addOutput(String portName, int portDepth,
			int granularDepth, Edits edits) {
		outputPorts.add(edits.createActivityOutputPort(portName, portDepth,
				granularDepth));
	}
	 ...
	 * @param portDepth
	 */
	protected final void addOutput(String portName, int portDepth, Edits edits) {
		addOutput(portName, portDepth, portDepth, edits);
	}
	...
	protected void configurePorts(ActivityPortsDefinitionBean configBean,
		...
		for (ActivityInputPortDefinitionBean inputDef : configBean
			...
			addInput(inputDef.getName(), inputDef.getDepth(), inputDef
					.getHandledReferenceSchemes(), edits);
		}
		outputPorts.clear();
		for (ActivityOutputPortDefinitionBean outputDef : configBean
				.getOutputPortDefinitions()) {
			OutputPort createActivityOutputPort = edits
					.createActivityOutputPort(outputDef.getName(), outputDef
							.getDepth(), outputDef.getGranularDepth());
			...
			for (String mimeType : outputDef.getMimeTypes()) {
				...
				try {
					edits.getAddAnnotationChainEdit(createActivityOutputPort,
