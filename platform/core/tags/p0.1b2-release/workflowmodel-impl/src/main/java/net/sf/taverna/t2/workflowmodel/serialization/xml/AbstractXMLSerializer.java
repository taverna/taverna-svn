...
/**
 * An abstract base class that contains serialisation methods common across all
 * dataflow elements.
 * 
 * @author Stuart Owen
 * 
 */
public abstract class AbstractXMLSerializer implements
		XMLSerializationConstants {

	protected Element pluginElement(PluginIdentifier pid) {
		Element element = new Element(PLUGIN, T2_WORKFLOW_NAMESPACE);
		// Group
		Element groupIdElement = new Element(PLUGIN_GROUP,
				T2_WORKFLOW_NAMESPACE);
		groupIdElement.setText(pid.getGroupId());
		element.addContent(groupIdElement);
		// Artifact ID
		Element artifactIdElement = new Element(PLUGIN_ID,
				T2_WORKFLOW_NAMESPACE);
		artifactIdElement.setText(pid.getPluginId());
		element.addContent(artifactIdElement);
		// Version
		Element versionElement = new Element(PLUGIN_VERSION,
				T2_WORKFLOW_NAMESPACE);
		versionElement.setText(pid.getVersion());
	...
	private void populateBeanElementForDataflow(Dataflow dataflow, Element bean) {
		...
		dataflowElement.setAttribute(DATAFLOW_REFERENCE, dataflow
				.getInternalIdentier());
