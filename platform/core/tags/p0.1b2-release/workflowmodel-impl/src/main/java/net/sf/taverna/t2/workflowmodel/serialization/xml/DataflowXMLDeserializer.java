...
public class DataflowXMLDeserializer extends AbstractXMLDeserializer {
	...
	public Dataflow deserializeDataflow(Element element,
		...
		Element inputPorts = element.getChild(DATAFLOW_INPUT_PORTS,
				T2_WORKFLOW_NAMESPACE);
		Element outputPorts = element.getChild(DATAFLOW_OUTPUT_PORTS,
				T2_WORKFLOW_NAMESPACE);
		...
		// processors
		Element processorsElement = element.getChild(PROCESSORS,
				T2_WORKFLOW_NAMESPACE);
		for (Element procElement : (List<Element>) processorsElement
				.getChildren(PROCESSOR, T2_WORKFLOW_NAMESPACE)) {
			Processor p = ProcessorXMLDeserializer.getInstance()
					.deserializeProcessor(procElement, innerDataflowElements,
							manager, edits);
		...
		Element conditions = element
				.getChild(CONDITIONS, T2_WORKFLOW_NAMESPACE);
		ConditionXMLDeserializer.getInstance().buildConditions(df, conditions,
				createdProcessors);

		// datalinks
		Element datalinks = element.getChild(DATALINKS, T2_WORKFLOW_NAMESPACE);
		DatalinksXMLDeserializer.getInstance().buildDatalinks(df,
				createdProcessors, datalinks);

		return df;
	}

	@SuppressWarnings("unchecked")
	private void addDataflowPorts(Dataflow df, Element inputPortsElement,
			Element outputPortsElement) throws EditException {
		for (Element port : (List<Element>) inputPortsElement.getChildren(
				DATAFLOW_PORT, T2_WORKFLOW_NAMESPACE)) {
			String name = port.getChildText(NAME, T2_WORKFLOW_NAMESPACE);
			int portDepth = Integer.valueOf(port.getChildText(DEPTH,
					T2_WORKFLOW_NAMESPACE));
			int granularDepth = Integer.valueOf(port.getChildText(
					GRANULAR_DEPTH, T2_WORKFLOW_NAMESPACE));
			edits.getCreateDataflowInputPortEdit(df, name, portDepth,
					granularDepth).doEdit();
		}

		for (Element port : (List<Element>) outputPortsElement.getChildren(
				DATAFLOW_PORT, T2_WORKFLOW_NAMESPACE)) {
			String name = port.getChildText(NAME, T2_WORKFLOW_NAMESPACE);
