...
public abstract class AbstractCrystalizer implements Crystalizer {
	...
	private Map<ProcessIdentifier, CompletionAwareTreeCache> cacheMap = new HashMap<ProcessIdentifier, CompletionAwareTreeCache>();

	public abstract Job getEmptyJob(ProcessIdentifier owningProcess,
	...
	public void receiveEvent(IterationInternalEvent e) {
		...
		ProcessIdentifier owningProcess = e.getOwningProcess();
	...
	protected class CompletionAwareTreeCache extends TreeCache {
		...
		public CompletionAwareTreeCache(ProcessIdentifier owningProcess,
		...
		public void resolveAt(ProcessIdentifier owningProcess,
				int[] completionIndex) {
			...
			if (n.contents == null) {
				...
				for (String outputName : listItems.keySet()) {
					...
					newDataMap.put(outputName, context.getReferenceService()
							.getListService().registerList(idlist, context)
							.getId());
