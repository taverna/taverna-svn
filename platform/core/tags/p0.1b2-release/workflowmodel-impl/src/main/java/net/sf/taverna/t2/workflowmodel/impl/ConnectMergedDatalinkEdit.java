 ...
 * using {@link Datalink}. Using a Merge facilitates multiple incoming
 * Datalinks connect to a single input port.
 * </p>
 * <p>
...
public class ConnectMergedDatalinkEdit extends AbstractMergeEdit {
	...
	protected void doEditAction(MergeImpl mergeImpl) throws EditException {
		Edits edits = new EditsImpl(null);
		String name = getUniqueMergeInputPortName(mergeImpl, sourcePort
				.getName()
				+ "To" + merge.getLocalName() + "_input", 0);
		mergeInputPort = new MergeInputPortImpl(mergeImpl, name, sinkPort
				.getDepth());
		...
		if (mergeImpl.getOutputPort().getOutgoingLinks().size() == 0) {
			...
			if (mergeImpl.getOutputPort().getOutgoingLinks().toArray(
					new Datalink[] {})[0].getSink() != sinkPort) {
