...
import net.sf.taverna.t2.invocation.FailureAware;
import net.sf.taverna.t2.invocation.FailureHandler;
import net.sf.taverna.t2.invocation.InvocationContext;
import net.sf.taverna.t2.invocation.ProcessIdentifier;
import net.sf.taverna.t2.invocation.WorkflowDataToken;
import net.sf.taverna.t2.invocation.impl.DataflowInputPortHandler;
import net.sf.taverna.t2.invocation.impl.DataflowOutputPortHandler;
import net.sf.taverna.t2.invocation.impl.TerminalProcessHandler;
...
public class DataflowImpl extends AbstractAnnotatedThing<Dataflow> implements
	...
	private DataflowOutputPortHandler outputPortHandler;
	private DataflowInputPortHandler inputPortHandler;
	private TerminalProcessHandler completionHandler;
	private Map<ProcessIdentifier, DataflowState> activeFlows = new HashMap<ProcessIdentifier, DataflowState>();
	...
	protected DataflowImpl() {
		...
		this.failureHandler = new FailureHandler() {
			...
			public void processFailed(ProcessIdentifier processIdentifier,
					...
					Throwable cause) {
				// We only receive these from workflow level entities so can
				// strip off the process to get our process identifier
				DataflowState state = getDataflowState(processIdentifier
						.getParent());
				state.fail(processIdentifier, invocationContext,
						workflowEntity, message, cause);
			}

		};

		this.outputPortHandler = new DataflowOutputPortHandler() {

			public void resultTokenProduced(WorkflowDataToken token,
					String portName) {
				ProcessIdentifier dataflowProcess = token.getOwningProcess()
						.createChild(getLocalName());
				DataflowState state = getDataflowState(dataflowProcess);
				state.resultTokenProduced(token, portName);
			}

		};

		this.inputPortHandler = new DataflowInputPortHandler() {
			...
			public void inputTokenReceived(WorkflowDataToken token) {
				...
				ProcessIdentifier dataflowProcess = token.getOwningProcess()
						.createChild(getLocalName());
				DataflowState state = getDataflowState(dataflowProcess);
				state.setStatus(WorkflowInstanceStatus.RUNNING);
			}

		};

		this.completionHandler = new TerminalProcessHandler() {

			public void completeTerminal(ProcessIdentifier processIdentifier,
					TokenProcessingEntity terminal, InvocationContext context) {
				DataflowState state = getDataflowState(processIdentifier
						.getParent());
				state.completeTerminal(terminal, context);
			}
		};
	...
	private DataflowState getDataflowState(ProcessIdentifier owningProcess) {
		synchronized (activeFlows) {
			if (!activeFlows.containsKey(owningProcess)) {
				activeFlows
						.put(owningProcess, new DataflowState(owningProcess));
			}
			return activeFlows.get(owningProcess);
		}
	}
	...
	protected synchronized void addProcessor(ProcessorImpl processor)
		...
		processor.setFailureHandler(failureHandler);
		processor.setCompletionHandler(completionHandler);
	...
	protected synchronized void addOutputPort(DataflowOutputPortImpl outputPort)
		...
		for (DataflowOutputPort existingOutputPort : outputs
			...
			if (existingOutputPort.getName().equals(outputPort.getName()))
				throw new NamingException(
						"There already is a dataflow output port named:"
								+ outputPort.getName());
		}
		if (outputPort.getDataflow() != this) {
			throw new EditException("Port specifies a different dataflow");
		}
		outputPort.setHandler(outputPortHandler);
	...
	public <T extends Object> List<? extends T> getEntities(Class<T> entityType) {
		...
		filterAndAdd(inputs, result, entityType);
		filterAndAdd(outputs, result, entityType);
		filterAndAdd(processors, result, entityType);
		filterAndAdd(merges, result, entityType);
		return Collections.unmodifiableList(result);
	}

	private <T extends Object> void filterAndAdd(Iterable<?> source,
			List<T> target, Class<T> type) {
		for (Object o : source) {
	 ...
	 */
	protected boolean tokenReceived(ProcessIdentifier owningProcess,
			InvocationContext context) {
		DataflowState state = getDataflowState(owningProcess);
		return state.startProcessing(context);
	}
	...
	public void fire(ProcessIdentifier owningProcess, InvocationContext context) {
		ProcessIdentifier newOwningProcess = owningProcess
				.createChild(getLocalName());
		DataflowState state = getDataflowState(newOwningProcess);
		state.startProcessing(context);
	}
	...
	public List<TokenProcessingEntity> getInternalTerminals() {
		List<TokenProcessingEntity> result = new ArrayList<TokenProcessingEntity>();
		for (TokenProcessingEntity tpe : getEntities(TokenProcessingEntity.class)) {
			if (tpe.getOutputPorts().isEmpty()) {
				result.add(tpe);
			}
		}
		return result;
	...
	public void addWorkflowInstanceListener(
		...
		getDataflowState(dataflowProcessIdentifier)
				.addWorkflowInstanceListener(listener);
	...
	public void removeWorkflowInstanceListener(
			...
			WorkflowInstanceListener listener) {
		getDataflowState(dataflowProcessIdentifier)
				.removeWorkflowInstanceListener(listener);
	}

	public WorkflowInstanceStatus getInstanceStatus(
			ProcessIdentifier dataflowProcessIdentifier) {
		return getDataflowState(dataflowProcessIdentifier).getStatus();
	}

	public void cancelWorkflowInstance(
			ProcessIdentifier dataflowProcessIdentifier,
			InvocationContext context) {
		DataflowState state = getDataflowState(dataflowProcessIdentifier);
		state.setStatus(WorkflowInstanceStatus.FAILED);
		state.fail(dataflowProcessIdentifier, context, this,
	...
	private static Timer stateCleanupTimer = new Timer(true);
	private static long stateCleanupDelay = 10 * 60 * 1000;

	/**
	 * Holds the state of an active instance of this dataflow
	 * 
	 * @author Tom Oinn
	 * 
	 */
	private class DataflowState {

		private ProcessIdentifier process;
		private List<TokenProcessingEntity> terminals;
		private List<DataflowOutputPort> outputs;
		private Map<String, WorkflowDataToken> finalResults = new HashMap<String, WorkflowDataToken>();
		private List<WorkflowInstanceListener> listeners = new ArrayList<WorkflowInstanceListener>();
		private boolean failed = false;
		private boolean completed = false;
		private WorkflowInstanceStatus status = WorkflowInstanceStatus.READY;
		private FailureDescription failureDescription = new FailureDescription();
		private boolean registeredWithMonitor = false;
		...
		private class FailureDescription {
			...
			public NamedWorkflowEntity workflowEntity = DataflowImpl.this;
			public ProcessIdentifier failedProcess = null;
		}

		private void requestStateRemoval() {
			stateCleanupTimer.schedule(new TimerTask() {
				public void run() {
					activeFlows.remove(DataflowState.this);
				}
			}, stateCleanupDelay);
		}

		public DataflowState(ProcessIdentifier processIdentifier) {
			this.process = processIdentifier;
			outputs = new ArrayList<DataflowOutputPort>(getOutputPorts());
			terminals = new ArrayList<TokenProcessingEntity>(
					getInternalTerminals());
		}

		public synchronized void setStatus(WorkflowInstanceStatus newStatus) {
			if (!status.equals(newStatus)) {
				synchronized (listeners) {
					for (WorkflowInstanceListener listener : listeners) {
						listener.workflowStatusChanged(status, newStatus);
					}
		...
		public void fail(ProcessIdentifier processIdentifier,
				InvocationContext invocationContext,
				NamedWorkflowEntity workflowEntity, String message,
				Throwable cause) {
			// Cache failure properties so we can send them to any listener
			// registered after the failure has occured
			failureDescription.cause = cause;
			failureDescription.message = message;
			failureDescription.context = invocationContext;
			failureDescription.failedProcess = processIdentifier;
			failureDescription.workflowEntity = workflowEntity;
			failed = true;
			invocationContext.setProcessInactive(process);
			for (FailureAware fa : getEntities(FailureAware.class)) {
				fa.purgeState(process);
			}
			synchronized (listeners) {
				for (WorkflowInstanceListener listener : listeners) {
					listener.workflowFailed(processIdentifier,
							invocationContext, workflowEntity, message, cause);

				}
				listeners.clear();
			}
			stopProcessing(invocationContext);
		}

		public void completeTerminal(TokenProcessingEntity terminal,
				InvocationContext context) {
		...
		public void addWorkflowInstanceListener(
			...
			synchronized (listeners) {
				for (String portName : finalResults.keySet()) {
					listener.resultTokenProduced(finalResults.get(portName),
							portName);
				}
				if (failed) {
					listener.workflowStatusChanged(
							WorkflowInstanceStatus.READY,
							WorkflowInstanceStatus.FAILED);
					listener.workflowFailed(failureDescription.failedProcess,
							failureDescription.context,
							failureDescription.workflowEntity,
							failureDescription.message,
							failureDescription.cause);
				} else if (completed) {
					listener.workflowStatusChanged(
							WorkflowInstanceStatus.READY,
							WorkflowInstanceStatus.COMPLETED);
					listener.workflowCompleted(process);
				}
		...
		public void resultTokenProduced(WorkflowDataToken token, String portName) {
			// At this point the token will already have been relayed onto any
			// downstream input ports, although in the case of the dataflow
			// output there are normally none

			// Firstly relay the event to any listeners
			synchronized (listeners) {
				for (WorkflowInstanceListener listener : listeners) {
					listener.resultTokenProduced(token, portName);
				}
			}

			// Now, if this is a final event, remove the output port from the
			// list of known outputs and check to see whether we've produced all
			// the output
			if (token.getIndex().length == 0) {
				// Final token
				finalResults.put(portName, token);
				synchronized (outputs) {
					DataflowOutputPort outputToRemove = null;
					for (DataflowOutputPort dop : outputs) {
						if (dop.getName().equalsIgnoreCase(portName)) {
							outputToRemove = dop;
							break;
						}
					}
			...
			checkCompletion(token.getContext());

		}

		public synchronized boolean startProcessing(InvocationContext context) {
			if (!registeredWithMonitor) {
				for (MonitorReceiver receiver : context
						.getEntities(MonitorReceiver.class)) {
					receiver.registerNode(DataflowImpl.this, process);
				}
				context.registerDataflow(DataflowImpl.this, process);
				// Message each processor within the dataflow and instruct
				// it to
				// register any properties with the monitor including any
				// processor level properties it can aggregate from its
				// dispatch
				// stack.
				for (ProcessorImpl p : getEntities(ProcessorImpl.class)) {
					for (MonitorReceiver receiver : context
							.getEntities(MonitorReceiver.class)) {
						p.registerWithMonitor(process, receiver);
					}
					if (p.getInputPorts().isEmpty()) {
						p.fire(process, context);
					}
				}
				registeredWithMonitor = true;
		...
		public synchronized void stopProcessing(InvocationContext context) {
			if (registeredWithMonitor) {
				registeredWithMonitor = false;
				for (MonitorReceiver receiver : context
						.getEntities(MonitorReceiver.class)) {
					for (Processor p : getEntities(Processor.class)) {
						receiver.deregisterNode(process.createChild(p
								.getLocalName()));
					}
					receiver.deregisterNode(process);
				}
				context.unregisterDataflow(process);
				requestStateRemoval();
			}
		}

		private void checkCompletion(InvocationContext context) {
			if (outputs.isEmpty() && terminals.isEmpty()) {
				// Set state to completed
				setStatus(WorkflowInstanceStatus.COMPLETED);
				synchronized (listeners) {
					for (WorkflowInstanceListener listener : listeners) {
						listener.workflowCompleted(process);
					}
