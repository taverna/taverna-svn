...
/**
 * A Health Checker associated with Processors. This iterates over the processor
 * activities invoking each HealthChecker available for each Activity to
 * generate an overal ProcessorHealthReport
 * 
 * @author Stuart Owen
 * @author Tom Oinn
 * 
	...
	public HealthReport checkHealth(Processor subject,
		...
		for (Activity<?> a : subject.getActivityList()) {
			for (HealthChecker<?> checker : checkers) {
				...
				if (checker.canHandle(a)) {
					reports.add(((HealthChecker<Activity>) checker)
							.checkHealth(a, checkers));
				}
				if (!reports.isEmpty()) {
					if (reports.size() == 1) {
						activityReports.add(reports.get(0));
					} else {
						activityReports.add(new HealthReport(
								"Activity tests...", "", Status.OK, reports));
					}
				}
			}
		}
		HealthReport processorHealthReport = new ProcessorHealthReport(subject
				.getLocalName()
				+ " Processor", activityReports);
