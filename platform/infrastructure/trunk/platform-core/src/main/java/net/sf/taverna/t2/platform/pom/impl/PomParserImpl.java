...
public class PomParserImpl implements PomParser {
	...
	public synchronized ArtifactDescription getDescriptionInternal(
		...
		Set<String> dependencyManagementExcludedScopes = new HashSet<String>();
		Map<String, Set<String>> dependencyManagementExclusions = new HashMap<String, Set<String>>();
		...
		for (int i = pomDocs.size() - 1; i >= 0; i--) {
			...
			for (Node depManagerNode : managerElementList) {
				...
				for (Node depNode : elementList) {
					...
					// Check whether an exclusions block is defined within the
					// dependency block
					List<Node> excludeNodes = DomUtils.findElements(depNode,
							new String[] { "exclusions", "exclusion" });
					if (!excludeNodes.isEmpty()) {
						// Check whether we already have a list of exclusions
						// for this dependency, and initialize if not
						if (!dependencyManagementExclusions.containsKey(groupId
								+ ":" + artifactId)) {
							dependencyManagementExclusions.put(groupId + ":"
									+ artifactId, new HashSet<String>());
						}
						for (Node excludeNode : excludeNodes) {
							Node exGroupNode = DomUtils.findElements(
									excludeNode, "groupId").iterator().next();
							String exGroupId = exGroupNode.getFirstChild()
									.getNodeValue().trim();
							Node exArtifactNode = DomUtils.findElements(
									excludeNode, "artifactId").iterator()
									.next();
							String exArtifactId = exArtifactNode
									.getFirstChild().getNodeValue().trim();
							dependencyManagementExclusions.get(
									groupId + ":" + artifactId).add(
									exGroupId + ":" + exArtifactId);
						}
					}

					// Check whether a scope is defined in the dependency
					// management block and manage it if so
					boolean excluded = false;
					List<Node> scopeNodeList = DomUtils.findImmediateElements(
							depNode, "scope");
					if (!scopeNodeList.isEmpty()) {
						String scope = scopeNodeList.get(0).getFirstChild()
								.getNodeValue().trim();
						for (String excludedScope : excludedScopes) {
							if (scope.equalsIgnoreCase(excludedScope)) {
								excluded = true;
								break;
							}
						}
					...
					// as we go along
					if (excluded) {
						dependencyManagementExcludedScopes.add(groupId + ":"
								+ artifactId);
					}
		...
		for (Node depNode : depNodeList) {
			...
			// Check scope, but also any scoping that has been inherited from
			// dependency management
			if (downloadableScope
					&& !dependencyManagementExcludedScopes.contains(groupId
							+ ":" + artifactId)) {
				// Incorporate any exclusions from the dependency management
				// section
				Set<String> inheritedExclusions = dependencyManagementExclusions
						.get(groupId + ":" + artifactId);
				if (inheritedExclusions != null) {
					exclusions.addAll(inheritedExclusions);
				}
				// Add a dependency, this may be removed subsequently due to
		...
		// Remove dependencies which have been excluded by an exclusion block
		for (ArtifactIdentifier dep : new ArrayList<ArtifactIdentifier>(deps)) {
			if (exclusions.contains(dep.getGroupId() + ":"
					+ dep.getArtifactId())) {
				deps.remove(dep);
			}
		}
		...
		ArtifactDescription description = new ArtifactDescriptionImpl(id, deps,
				optionalDeps, new ArrayList<String>(exclusions));
	...
	public static File getFile(ArtifactIdentifier id, List<URL> repositories,
		...
		try {
			if (remoteLocations.isEmpty()) {
				throw new ArtifactParseException("Unable to find artifact '"
						+ id + "' locally, no remote repositories defined");
			}
