 ...
 * 
 * <p>
 * This class works as a facade and all the actual work is delegated to a JAXB
 * provider that happens to be in the runtime (not necessarily the JAXB RI.)
 ...
 * 
 * <p>
 * The trick to make this work is two ObjectFactory classes that we generate in
 * the interface/implementation mode.
 * 
 * <p>
 ...
 * This private ObjectFactory refers to implementation classes, which are also
 * indistinguishable from value classes that JAXB generates.
 * 
 * <p>
 * All in all, the private ObjectFactory plus implementation classes give a JAXB
 * provider an illusion that they are dealing with value classes that happens to
...
public class JAXBContextFactory {
	...
	public static JAXBContext createContext(Class[] classes, Map properties)
		...
		for (int i = 0; i < r.length; i++) {
			...
			if (name.endsWith(DOT_OBJECT_FACTORY)
				...
				// we never generate into the root package, so no need to worry
				// about FQCN "ObjectFactory"

				// if we find one, tell the real JAXB provider to
				// load foo.bar.impl.ObjectFactory
				name = name.substring(0, name.length()
						- DOT_OBJECT_FACTORY.length())
		...
		if (!modified) {
			...
			// if we simply continue, we'll just end up with the infinite
			// recursion.
			...
			// anyway, bail out now.
			// if you hit this problem and wondering how to get around the
			// problem,
			// subscribe and send a note to users@jaxb.dev.java.net
	...
	public static JAXBContext createContext(String contextPath,
		...
		try {
			while (tokens.hasMoreTokens()) {
				...
				classes.add(classLoader
						.loadClass(pkg + IMPL_DOT_OBJECT_FACTORY));
			}
		} catch (ClassNotFoundException e) {
			throw new JAXBException(e);
		}
		...
		return JAXBContext.newInstance(classes
				.toArray(new Class[classes.size()]), properties);
	}
}
