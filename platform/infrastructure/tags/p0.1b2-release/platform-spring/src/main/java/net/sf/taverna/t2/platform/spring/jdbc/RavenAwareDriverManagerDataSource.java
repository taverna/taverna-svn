 ...
 ******************************************************************************/
package net.sf.taverna.t2.platform.spring.jdbc;
...
/**
 * Used to register JDBC drivers loaded from Raven artifacts. If the optional
 * 'repository' and 'driverArtifact' properties are injected (of type Repository
 * and String respectively) the specified driver will be loaded from the
 * artifact, including any download or other activity needed to resolve it
 * properly. The driver will be loaded in a temporary classloader and a proxy to
 * it loaded in the platform classloader, making it visible to systems which
 * can't see inside the artifact class loaders such as Hibernate. If these
 * properties are undefined the data source reverts to default behaviour for a
 * DriverManagerDataSource
 * 
 * @author Tom Oinn
 * 
 */
public class RavenAwareDriverManagerDataSource extends
		DriverManagerDataSource {

	private Log log = LogFactory
			.getLog(RavenAwareDriverManagerDataSource.class);
	...
	private Raven repository = null;
	private ArtifactIdentifier driverArtifact = null;
	 ...
	 * @param rep
	 */
	public void setRepository(Raven rep) {
		this.repository = rep;
	}
	...
	public void setDriverArtifact(String a) {
		driverArtifact = new ArtifactIdentifier(a);
	}
