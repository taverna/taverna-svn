...
public class SendEmail implements LocalWorker {
	...
	public Map execute(Map inputs) throws TaskExecutionException {
		if (inputs.containsKey("to") == false
				|| inputs.containsKey("from") == false) {
			throw new TaskExecutionException(
					"Inputs to this task must contain the 'to' and 'from' email addresses.");
		}
		...
		if (inputs.containsKey("subject")) {
			subject = (String) ((DataThing) inputs.get("subject"))
					.getDataObject();
		}
		...
		if (inputs.containsKey("smtpserver")) {
			mailProps.put("mail.smtp.host", (String) ((DataThing) inputs
					.get("smtpserver")).getDataObject());
		}
		try {
