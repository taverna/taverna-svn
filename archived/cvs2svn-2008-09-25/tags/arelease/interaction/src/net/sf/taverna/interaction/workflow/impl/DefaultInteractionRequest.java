...
/**
 * Implementation of the InteractionRequest interface that can be
 * used to submit requests to the HTTPInteractionServiceProxy
 * @author Tom Oinn
 */
public class DefaultInteractionRequest implements InteractionRequest {
    ...
    public static InteractionRequest createRequest(InteractionPattern pattern,
						   ...
						   Map inputData,
						   Date expiryTime) {
	return new DefaultInteractionRequest(pattern, email, inputData, expiryTime);
    }
    
    private DefaultInteractionRequest(InteractionPattern pattern,
				      String email,
				      Map inputData,
				      Date expiryTime) {
    ...
    /**
     * Use the DataThingXMLFactory and helpers to generate the
     * data map.
     */
    public byte[] getData() {
