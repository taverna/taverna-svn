...
/**
 * Parse an RDFS ontology file into a tree to be
 * displayed by some UI component or other. Builds
 * a tree based on DefaultMutableTreeNodes containing
 * the full text from the 'about' property of
 * each Class in the RDFS document. The tree is
 * denormalised, otherwise it wouldn't be a tree...
 * @author Tom Oinn
 */
public class RDFSParser {
     ...
     */
    static Namespace rdfsNS = Namespace.getNamespace("rdfs","http://www.w3.org/2000/01/rdf-schema#");
    
    /**
     * The RDF namespace
     */
    static Namespace rdfNS  = Namespace.getNamespace("rdf","http://www.w3.org/1999/02/22-rdf-syntax-ns#");
	...
	try {
		...
		for (Iterator j = classElement.getChildren("subClassOf",rdfsNS).iterator(); j.hasNext(); ) {
		...
		// Also put a placeholder in the other hash, we'll populate it later
	    ...
	    // Iterate over all the classes just created and create the arrays of
	    // children for each one, which is required for the tree generation.
	    for (Iterator i = classToParentList.keySet().iterator(); i.hasNext(); ) {
