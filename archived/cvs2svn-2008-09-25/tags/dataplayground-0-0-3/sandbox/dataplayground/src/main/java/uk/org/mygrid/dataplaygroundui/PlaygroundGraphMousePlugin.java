...
public class PlaygroundGraphMousePlugin extends AbstractGraphMousePlugin implements MouseListener, MouseMotionListener, KeyListener{
    ...
    /**
     * Code based on EditingGraphMousePlugin from Jung graph library version 1.7.6
     * original author:    Tom Nelson - RABA Technologies
     *
     */
	...
	public PlaygroundGraphMousePlugin(int modifiers) {
        ...
        playgroundInputPanel = (PlaygroundInputPanel)PlaygroundInputPanel.getInstance();
		...
		if(startVertex != null) {
            ...
            if(startVertex instanceof PlaygroundDataObject){
                ...
                if(pickSupport != null) {
                    ...
                    //method to get the closest vertex to the mouse point excluding the given vertex
                	Vertex vertex = ((PlaygroundPickSupport)pickSupport).getVertex(p.getX(), p.getY(),startVertex);
                    ...
                    if(vertex != null)
                    {
                    	if(vertex instanceof PlaygroundProcessorObject){
                    	    ...
                    	    ((PlaygroundVertexPaintFunction)((PluggableRenderer)vv.getRenderer()).getVertexPaintFunction()).setHighlighted(vertex);
                    	}
                    	...
                    	((PlaygroundVertexPaintFunction)((PluggableRenderer)vv.getRenderer()).getVertexPaintFunction()).setHighlighted(null);
                    	}
	...
	public void mouseReleased(MouseEvent e) {
        ...
        if(pickSupport != null) {
            ...
            if(vertex != null && startVertex != null) {
            	...
            	if(dragging == true){
            	if(startVertex instanceof PlaygroundDataThing && vertex instanceof PlaygroundDataObject){
            	Graph graph = vv.getGraphLayout().getGraph();
                graph.addEdge(new DirectedSparseEdge(startVertex, vertex));
                
                vv.repaint();
            	}
            	if(startVertex instanceof PlaygroundDataThing && vertex instanceof PlaygroundPortObject){
                	Graph graph = vv.getGraphLayout().getGraph();
                    graph.addEdge(new DirectedSparseEdge(startVertex, vertex));
                    ((PlaygroundPortObject)vertex).setMappedObject((PlaygroundDataThing)startVertex);
                    System.out.println("port object mapped");
                    vv.repaint();
                	}
                if(startVertex instanceof PlaygroundDataObject && vertex instanceof PlaygroundPortObject){
            		
                	Graph graph = vv.getGraphLayout().getGraph();
                    graph.addEdge(new DirectedSparseEdge(startVertex, vertex));
                    ((PlaygroundPortObject)vertex).setMappedObject((PlaygroundDataObject)startVertex);
                    vv.repaint();
            	...
            	if(startVertex instanceof PlaygroundDataObject && vertex instanceof PlaygroundProcessorObject){
        			...
        			int n = JOptionPane
        					.showOptionDialog(
        							...
        							"moby data type?",
        							"Annotate with data type?", JOptionPane.YES_NO_OPTION,
        							JOptionPane.QUESTION_MESSAGE, null, options, options[1]);
        			if (n == 0)
        			{
            		((PlaygroundProcessorObject)vertex).addInputType(((PlaygroundDataObject)startVertex).getProcessor());
    ...
    /**code lifted from EditingGraphMouse Plugin
