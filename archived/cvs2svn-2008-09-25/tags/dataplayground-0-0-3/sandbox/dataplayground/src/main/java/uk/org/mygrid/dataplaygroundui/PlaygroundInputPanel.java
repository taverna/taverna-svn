...
public class PlaygroundInputPanel extends JPanel implements UIComponentSPI {
				...
				model.getWorkflowSourceProcessor()
						.addPort(
								new OutputPort(model
										.getWorkflowSourceProcessor(), "Data"));
				model.forceUpdate();
			...
			*/
			...
			JButton setButton = new JButton(new AbstractAction("Set") {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					System.out.println(d.getDataObject() + " " + d.getSyntacticType() + " " + d.getMetadata().getFirstMIMEType());
	...
	public void setData(PlaygroundDataThing d){
		...
		//System.out.println("DataObject = " + d.getDataThing().getDataObject());
		//System.out.println("DataThing MIME type = " + d.getDataThing().getMetadata().getFirstMIMEType());
		...
		try {
			...
			model.getWorkflowSourceProcessor()
					.addPort(
							new OutputPort(model
									.getWorkflowSourceProcessor(), datathing.getName()));
			...
			builder.attachToModel(model);	
			header = new ShadedLabel(datathing.getName(), ShadedLabel.TAVERNA_BLUE);
