...
public class PlaygroundPanel extends JPanel implements UIComponentSPI, WorkflowModelViewSPI,
		DropTargetListener {
	...
	public PlaygroundPanel(PlaygroundObjectModel playgroundModel) {
		...
		playgroundRendererPanel = (PlaygroundRendererPanel)PlaygroundRendererPanel.getInstance();
		...
		recordAction r = new recordAction();
		toolbar.add(r);
		toolbar.addSeparator();
		toolbar.add(new stopAction(r));
		
		
		//create a new layout with the graph from the playgroundModel , create the new
		//Pluggable Renderer for this graph and then create the VisualisationViewer using 
		...
		graphMouse.add(new PlaygroundPopupGraphMousePlugin(vertexLocations,this));
...
*/
		...
		pr.setEdgePaintFunction(new PickableEdgePaintFunction(ps,Color.black,Color.cyan));
	...
	public void addData(Point2D point, BiomobyObjectProcessor d) {
		...
		vertexLocations.setLocation(v,vv.inverseTransform(point));
		ArrayList<PlaygroundDataObject> components = ((PlaygroundDataObject)v).getDataComponents();
		((FRLayout)layout).update();
		vv.repaint();
	...
	protected void addComponents(PlaygroundDataObject parent,ArrayList<PlaygroundDataObject> components){
		...
		playgroundModel.addDataComponent((PlaygroundDataObject)parent,pdo);
		System.out.println("Component  " + pdo);
		Point2D point = vertexLocations.getLocation(parent);
		vertexLocations.setLocation(pdo,new Point2D.Double(point.getX() + ((int)( Math.random() * 80) - 40)   ,point.getY() + ((int)( Math.random() * 80) - 40)));
		addComponents(pdo,pdo.getDataComponents());
	
		}
		
		//if the data type is Object we need to expose the namespace and id ports
		//if(parent.getDataType().equalsIgnoreCase("Object")){
			
		 ArrayList<PlaygroundPortObject> ports = new ArrayList(parent.getInputPortObjects().values());
		 iterator =  ports.iterator();
			...
			Point2D point = vertexLocations.getLocation(parent);
			vertexLocations.setLocation(ppo,new Point2D.Double(point.getX() + ((int)( Math.random() * 40) - 20)   ,point.getY() + ((int)( Math.random() * 40) - 20)));
			
					if(ppo.getMappedObject() != null){
						
						playgroundModel.addPortDataThing(ppo,(PlaygroundDataThing)ppo.getMappedObject());
						playgroundRendererPanel.add((PlaygroundDataThing)ppo.getMappedObject());
						Point2D p = vertexLocations.getLocation(ppo);
						vertexLocations.setLocation(ppo.getMappedObject(),new Point2D.Double(p.getX() + ((int)( Math.random() * 70) - 35)   ,p.getY() + ((int)( Math.random() * 70) - 35)));
...
protected void addPorts(PlaygroundObject parent){
        ...
        if(parent instanceof PlaygroundDataObject){
        	ports = new ArrayList(((PlaygroundDataObject)parent).getInputPortObjects().values());
        }
...
public class recordAction extends AbstractAction{
	...
	ImageIcon on;
		...
		public recordAction(){
			...
			URL iconOffURL = PlaygroundUIComponentFactory.class.getResource("media-record-small-off.gif");
			URL iconOnURL = PlaygroundUIComponentFactory.class.getResource("media-record-small.gif");
...
public class stopAction extends AbstractAction{
	...
	public stopAction(recordAction r){
		...
		URL iconURL = PlaygroundUIComponentFactory.class.getResource("media-playback-stop-small.gif");
	...
	r.stop();
		...
		//	tavernaModel =  playgroundModel.getRecordedWorkflow().clone();
			...
			XScuflView v = new XScuflView(playgroundModel.getRecordedWorkflow());
