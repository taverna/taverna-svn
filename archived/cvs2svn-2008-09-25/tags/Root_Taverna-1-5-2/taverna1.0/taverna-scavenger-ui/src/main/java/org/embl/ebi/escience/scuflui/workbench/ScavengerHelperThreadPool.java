...
/**
 * A thread pool that handles a collection of Scavenger threads (for URLBasedScavengers)
 * The pool ensures that there is a maximum number of concurrent threads, and maintains a waiting list
 * for those threads that cannot yet be started.
 * Each time getCompleted is called and the completed Scavengers are returned, waiting threads are moved
 * to the active list and started.
 * 
 * The pool is complete when all completed Scavengers have been return through getComplete, and isEmpty returns true
 * 
 * @author Stuart Owen
 *
 */
public class ScavengerHelperThreadPool {
	 ...
	 * This will be zero when all threads have finished and have had their results retrieved.
	 * @return
	 */
	...
	/**
	 * Adds a new scavenger helper, and creates a thread for it. The thread will either be started immediately, 
	 * or placed on the waiting queue.
	 * @param helper
	 */
	public void addScavengerHelper(ScavengerHelper helper) {
		ScavengerHelperThread thread = new ScavengerHelperThread(helper);
		addThread(thread);
	}
	...
	private class ScavengerHelperThread extends Thread
	{
		private ScavengerHelper scavengerHelper;
		private boolean complete = false;
		private Set<Scavenger> scavengers;
		public ScavengerHelperThread(ScavengerHelper scavengerHelper) {
