...
public class ProfileUpdateHandlerTest {
	...
	private static final String PROFILE_LIST = PROFILE_BASE_URL
			+ "taverna-1.5.2-profiles.xml";
	...
	private File createTempDirectory() throws IOException {
		File directory;
		try {
			directory = File.createTempFile("tavernahome", "");
		...
		directory.delete();
		assertTrue("Unable to create repository directory " + directory,
				directory.mkdir());
		return directory;
	}
	...
	public void testNoUpdateAvailable() throws Exception {
		URL local = copyToCurrent(new URL(PROFILE_BASE_URL
				+ "taverna-1.5.2.1-SNAPSHOT-profile.xml"));
		ProfileUpdateHandler handler = new ProfileUpdateHandler(new URL(
				PROFILE_LIST), local);
		assertFalse("There should be no update available", handler
	...
	public void testUpdateAvailable() throws Exception {
		URL local = copyToCurrent(new URL(PROFILE_BASE_URL
				+ "taverna-1.5.2.0-SNAPSHOT-profile.xml"));
		ProfileUpdateHandler handler = new ProfileUpdateHandler(new URL(
				PROFILE_LIST), local);
		assertTrue("There should be an update available", handler
				.isNewVersionAvailable());
	}
	...
	public void testDoUpdate() throws Exception {
		URL local = copyToCurrent(new URL(PROFILE_BASE_URL
				+ "taverna-1.5.2-SNAPSHOT-profile.xml"));
		ProfileUpdateHandler handler = new ProfileUpdateHandler(new URL(
				PROFILE_LIST), local);
		assertTrue("There should be an update available", handler
				.isNewVersionAvailable());
		...
		File file = new File(confdir, ProfileSelector.CURRENT_PROFILE
				+ "-1.5.2.0-SNAPSHOT.bak");
		assertTrue("No backup file found", file.exists());
	}

	private URL copyToCurrent(URL remoteProfile) throws Exception {
		File destinationFile = new File(confdir,
