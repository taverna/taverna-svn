...
public class ExtendedJTree extends JTree {
	...
	/**
	 * Expand or collapse all nodes, expand if the flag is true and collapse
	 * otherwise
	 */
	 ...
	 * path, if not then recurse into all children. boolean allLeaves = true;
	 * for (Enumeration en = node.children(); en.hasMoreElements() && allLeaves; ) {
	 * TreeNode child = (TreeNode)en.nextElement(); if (child.isLeaf() == false) {
	 * allLeaves = false; } } if (allLeaves) { tree.expandPath(parent); } else {
	 * for (Enumeration en = node.children(); en.hasMoreElements(); ) { TreeNode
	 * child = (TreeNode)en.nextElement(); if (child.isLeaf() == false) {
	 * expandAll(tree, parent.pathByAddingChild(child)); } } } }
	 */
