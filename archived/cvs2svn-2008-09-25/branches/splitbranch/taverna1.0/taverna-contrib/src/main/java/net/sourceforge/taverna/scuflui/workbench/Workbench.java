...
/**
 * A workbench application to allow editing and visualization of Scufl workflows
 * 
 * @author Mark
 */
public class Workbench extends JFrame {
	...
	/**
	 * This method clears the current ScuflModel, and detaches all listening
	 * objects.
	 * 
	 * @author Mark
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			logger.info("Failed to load ontology data! " + ex.getMessage());
			ex.printStackTrace();
		}
		...
		if (System.getProperty("taverna.workbench.useinternalframes") != null) {
			UIUtils.DEFAULT_FRAME_CREATOR = new UIUtils.FrameCreator() {
				public void createFrame(ScuflModel targetModel, ScuflUIComponent targetComponent, int posX, int posY,
						int sizeX, int sizeY) {
	 ...
	 */
	public Workbench() {
		super("Taverna Scufl Workbench v" + org.embl.ebi.escience.scufl.TavernaReleaseInfo.getVersion() + ", built "
				...
				if (files.length == 1) {
					...
					for (int i = 0; i < files.length; i++) {
						...
						new Thread() {
							public void run() {
								try {
									XScuflParser.populate(files[j].toURL().openStream(), Workbench.this.model, "file"
											+ j);
								} catch (Exception ex) {
