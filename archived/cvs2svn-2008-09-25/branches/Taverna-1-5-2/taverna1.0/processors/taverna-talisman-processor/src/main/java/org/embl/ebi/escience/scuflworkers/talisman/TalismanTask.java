...
public class TalismanTask implements ProcessorTaskWorker {
	...
	public Map execute(java.util.Map workflowInputMap, IProcessorTask parentTask) throws TaskExecutionException {
		try {
			...
			// name of the port that the input came in on, the value is the
			// value
			// of the input (obviously)
			// Map inputMap = new HashMap();
			// Parents are all PortTasks (I think?)
			/*
			 * GraphNode[] inputs = getParents(); for (int i = 0; i <
			 * inputs.length; i++) { if(inputs[i] instanceof PortTask) {
			 * PortTask pt = (PortTask)inputs[i]; Part p = pt.getData(); Element
			 * e = (Element)p.getValue();
			...
			for (Iterator i = talismanInputMap.keySet().iterator(); i.hasNext();) {
				...
				String portName = (String) i.next();
				// talismanName is the specifier for a field in the talisman
				// session
				String talismanName = (String) talismanInputMap.get(portName);
				// portValue is the string value of the port, and it had better
				// be
			...
			for (Iterator i = talismanOutputMap.keySet().iterator(); i.hasNext();) {
				...
				String portName = (String) i.next();
				// talismanName is the name of the field the value should be
				// read from
				String talismanName = (String) talismanOutputMap.get(portName);
				// talismanValue is the contents of the field defined in
				// talismanName
