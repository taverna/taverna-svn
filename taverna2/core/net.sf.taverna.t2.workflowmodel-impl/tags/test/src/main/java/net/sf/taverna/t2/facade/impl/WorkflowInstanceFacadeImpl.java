	...
	public WorkflowInstanceFacadeImpl(final Dataflow dataflow,
		...
		if (context.getProvenanceConnector() != null) {
			logger.info("Provenance being captured using: "
					+ context.getProvenanceConnector().getClass());
			XMLSerializerImpl serialiser = new XMLSerializerImpl();
	
			provEnabled = true;
			WorkflowProvenanceItem workflowItem = new WorkflowProvenanceItem();

			XMLDeserializerImpl deserialiser = new XMLDeserializerImpl();
			...
			for (Processor processor : dataflow.getProcessors()) {
				...
				for (int j = 0; j < layers.size(); j++) {
					if (layers.get(j) instanceof IntermediateProvenance) {
				...
				if (!provAlreadyAdded) {
					for (int j = 0; j < layers.size(); j++) {
						if (layers.get(j) instanceof Invoke) {
							...
							((IntermediateProvenance) provenance)
									.setConnector(context
											.getProvenanceConnector());

							Edits edits = EditsRegistry.getEdits();
							try {
								edits.getAddDispatchLayerEdit(dispatchStack,
										provenance, j - 1).doEdit();
								break;
							} catch (EditException e) {
								logger
										.warn("adding provenance layer to dispatch stack failed "
												+ e.toString());
							}
						}
					}
...
//				}
//				if (! foundProvenance ) {
//					throw new RuntimeException("Uh oh: " + processor);
//				}
