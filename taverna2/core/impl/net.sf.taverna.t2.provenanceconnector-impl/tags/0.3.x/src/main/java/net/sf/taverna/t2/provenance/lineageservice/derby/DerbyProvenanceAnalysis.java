 ...
 */
public class DerbyProvenanceAnalysis {

	private static final Object IP_ANNOTATION = "index-preserving";

	DerbyProvenanceQuery pq = null;
	AnnotationsLoader al = null;

	Map<String,List<String>> annotations = null;  // user-made annotations to processors

	public DerbyProvenanceAnalysis() throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {		
	...
	/**
	 * Must set this before doing anything or else it all fails
	 * @param provenanceQuery
	 */
	public void setProvenanceQuery(DerbyProvenanceQuery provenanceQuery) {
		pq = provenanceQuery;
	}
...
protected List<LineageAnnotation>  applyXferRule(List<LineageAnnotation> path, Map<String, List<String>> annotations) {
	...
	if (fromNesting == 0 && toNesting == 0)  { // s -> s see rule xfer/1
		...
		// we can only transfer the iteration to the IIC if this is a simple iteration, i.e., if the iteration has only
		// one index. otherwise, propagate the entire iteration vector as in rule 1

		Integer it = Integer.getInteger(prevNode.getIteration());
		if (it != null)
		{
			currNode.setIic(it+1); // CHECK
		} // else no transfer at all? CHECK
 ...
 */
public class LineageAnnotation {
