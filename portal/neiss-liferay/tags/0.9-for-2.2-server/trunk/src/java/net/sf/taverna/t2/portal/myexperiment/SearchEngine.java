...
/**
 * Adapted from net.sf.taverna.t2.ui.perspectives.myexperiment.model.SearchEngine.
 *
...
public class SearchEngine {
   ...
   * of the search.
   */
  public SearchEngine(boolean bSearchByTag, MyExperimentClient client) {
	super();
  ...
  public void searchAndPopulateResults(String tagQuery) {
	...
	new Thread("Search by tag") {
	  public void run() {
		...
		if (doc == null) {
		  System.out.println("Failed while fetching tagged items. See the error message above. Can't continue.");
		  ...
		  processSearchResults(root.getChildren());
		}
	  }
	}.start();
  }

  public Map<Integer, ArrayList<Resource>> searchAndPopulateResults(QuerySearchInstance querySearchDetails) {
...
//      vCurrentSearchThreadID.set(0, lThisSearchThreadID);
      ...
      if (doc == null) {
          System.out.println("Failed to fetch workflows from myExperiment. Returned document was null.");
          return null;
      } else {
          // search was successful, get found items' collection
          System.out.println("myExperiment returned document: " + doc.toString());
          Element root = doc.getRootElement();
          return processSearchResults(root.getChildren());
      }
  }
   ...
   */
  private Map<Integer, ArrayList<Resource>> processSearchResults(List<Element> foundSearchItemElements) {
	// prepare variables
...
//		return;
	  ...
	  Resource res = Resource.buildFromXML(foundItem, myExperimentClient);
