...
/**
 * Adapted from net.sf.taverna.t2.ui.perspectives.myexperiment.model.Resource.
 *
...
public class Resource implements Comparable<Resource>, Serializable {
   ...
   *         resource.
   */
  public JPanel createListViewPanel(boolean bCreateFullSizeView, EventListener eventHandler) {
	try {
	  ...
	  if (bCreateFullSizeView) {
		...
		if (this.hasUploader()) {
		  ...
		  content.append("<p class='uploader'>");
		  content.append("Administrator: " + this.getAdmin().getName());
		  content.append("</p>");
		}
	  ...
	  if (this.itemType == Resource.WORKFLOW || this.itemType == Resource.USER) {
		...
		content.append("<td valign='top'>");
		  ...
		  String resizedImageURL = Util.getResizedImageIconTempFileURL(previewURI.toURL(), THUMBNAIL_WIDTH_FOR_SHORT_LIST_VIEW, THUMBNAIL_HEIGHT_FOR_SHORT_LIST_VIEW);
		  content.append("<img class='preview' src='" + resizedImageURL
			  + "'></img>");
		}
	  ...
	  if (this.getDescription() != null && this.getDescription().length() > 0) {
		if (bCreateFullSizeView)
		  ...
		  if (strTruncatedDescription.length() > DESCRIPTION_TRUNCATE_LENGTH_FOR_SHORT_LIST_VIEW) {
			strTruncatedDescription = strTruncatedDescription.substring(0, DESCRIPTION_TRUNCATE_LENGTH_FOR_SHORT_LIST_VIEW);
	  ...
	  HTMLEditorKit kit = new StyledHTMLEditorKit(Util.getStyleSheet());
	  ...
	  System.out.println("Failed while creating " + this.getItemTypeName()
		  + " list view.");
          e.printStackTrace();
	  return (null);
	}
  ...
  /**
   * Create a preview for the instance of the resource that it
   * holds. Shows reduced amount of information when "bCreateFullSizeView" is
   * set to false.
   */
  public String createHTMLPreview(boolean bCreateFullSizeView, String workflowInputForm, RenderResponse response) {
    try {
	  ...
	  content.append("<p class='title'>");
	  content.append(((this.getItemType() == Resource.WORKFLOW) ? "Title: " : "") + this.getTitle()
		  + ((this.getItemType() == Resource.WORKFLOW) ? (" (version "
			  + ((Workflow) this).getVersion() + ")") : ""));
	  content.append("</p>");
...
//	  return (mainPanel);
   ...
   */
  public static Resource buildFromXML(Document resourceXMLDocument, MyExperimentClient client) {
	Element root = resourceXMLDocument.getRootElement();
	return (Resource.buildFromXML(root, client));
  }
   ...
   * XML content into a Resource instance.
   */
  public static Resource buildFromXML(Element docRootElement, MyExperimentClient client) {
	Resource res = null;
	switch (Resource.getResourceTypeFromVisibleName(docRootElement.getName())) {
	  case Resource.WORKFLOW:
		res = Workflow.buildFromXML(docRootElement);
		break;
	  case Resource.FILE:
		res = File.buildFromXML(docRootElement);
		break;
	  case Resource.PACK:
		res = Pack.buildFromXML(docRootElement, client);
		break;
	  case Resource.USER:
		res = User.buildFromXML(docRootElement);
		break;
	  case Resource.GROUP:
		res = Group.buildFromXML(docRootElement);
		break;
	  default:
