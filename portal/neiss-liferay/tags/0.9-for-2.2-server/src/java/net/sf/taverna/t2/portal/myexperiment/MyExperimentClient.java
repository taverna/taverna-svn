...
import net.sf.taverna.t2.portal.myexperiment.SearchEngine.QuerySearchInstance;
...
/**
 * A modified version of net.sf.taverna.t2.ui.perspectives.myexperiment.model.MyExperimentClient.
 * 
...
public class MyExperimentClient {
  ...
  public void doLoginFromStoredCredentials() {
    ...
    try {
      doc = this.doMyExperimentGET(this.BASE_URL + "/whoami.xml")
        ...
        e.printStackTrace();
      System.out.println("Error while attempting to verify login credentials from INI file:\n");
    }
    ...
    if (doc == null) {
      ...
      try {
        ...
        System.out.println("Logged in to myExperiment successfully with credentials that were loaded from INI file.");
      } catch (Exception e) {
        // this is highly unlikely because the login credentials were validated
        // successfully just before this
          e.printStackTrace();
        System.out.println("Couldn't fetch user data from myExperiment ("
            + strCurrentUserURI
  ...
  public User fetchCurrentUser(String uri) {
    ...
    try {
      ...
      user = User.buildFromXML(doc);
    } catch (Exception ex) {
      System.out.println("Failed to fetch user data from myExperiment (" + uri
          + ").");
      ex.printStackTrace();
    }
    ...
    try {
      if (user.getAvatarURI() == null) {
        ...
        ex.printStackTrace();
      System.out.println("Failed to fetch user's avatar from myExperiment ("
    ...
    try {
      ...
      if (doc != null) {
        ...
        for (Element e : allInternalItems) {
          ...
          workflows.add(Workflow.buildFromXML(docCurWorkflow));
        }
      }
    } catch (Exception e) {
        e.printStackTrace();
      System.out.println("Failed to retrieve example workflows" + e);
    }
  ...
  public TagCloud getGeneralTagCloud(int size) {
    ...
    try {
      ...
      if (doc != null) {
        ...
        e.printStackTrace();
      System.out.println("ERROR: Failed to get tag cloud.\n");
    }

    System.out.println("Tag cloud retrieval successful; fetched "
  ...
  public TagCloud getUserTagCloud(User user, int size) {
    ...
    try {
      ...
      Collections.sort(tcCloud.getTags(), byAlphabet);
        ...
        e.printStackTrace();
      System.out.println("Failed midway through fetching user tags for user ID = "
          + user.getID() + "\n" + e);
    }
  ...
  public Document getUserContributions(User user, int iResourceType,
    ...
    try {
      ...
      doc = this.doMyExperimentGET(strURL).getResponseBody();
        ...
        e.printStackTrace();
      System.out.println("ERROR: Failed to fetch user's contributions.");
    }
  ...
  public Document searchByTag(String strTag) {
    ...
    try {
      ...
      doc = this.doMyExperimentGET(
        ...
        e.printStackTrace();
      System.out.println("ERROR: Failed to fetch tagged items from myExperiment. Query tag was '"
              + strTag + "'\n" + e);
    }
  ...
  public void convertTagListIntoTagCloudData(List<Tag> tags) {
    try {
      ...
      for (Tag t : tags) {
        ...
        e.printStackTrace();
      System.out.println("Failed while getting tag application counts when turning tag list into tag cloud data");
    }
  }
  ...
  public void updateUserFavourites(User user) {
    ...
    try {
      ...
      System.out.println("Failed to fetch favourites data from myExperiment for a user (URI: "
              + user.getURI() + ").");
      ex.printStackTrace();
      JOptionPane
          .showMessageDialog(
  ...
  public void updateCommentListWithExtraData(List<Comment> comments) {
    try {
      ...
      System.out.println("Failed while updating comment list for preview");
      e.printStackTrace();
    }
  }
  ...
  public Document searchByQuery(QuerySearchInstance searchQuery) {
    ...
    try {
      ...
      System.out.println("Search URL when searching myExperiment: " + strSearchURL);
      // DEBUG
      ...
      doc = this.doMyExperimentGET(strSearchURL).getResponseBody();
        ...
        System.out.println("ERROR: Failed to run search on myExperiment. Query URL was'"
              + strSearchURL);
        e.printStackTrace();
    }
  ...
  public ServerResponse postComment(Resource resource, String strComment) {
    try {
      ...
      if (response.getResponseCode() == HttpURLConnection.HTTP_OK) {
        ...
        Comment cNew = Comment.buildFromXML(response.getResponseBody());
      ...
      System.out.println("Failed while trying to post a comment for "
          + resource.getURI());
      e.printStackTrace();
  ...
  public ServerResponse postWorkflow(String workflowContent, String title,
    ...
    try {
      ...
      System.out.println("Failed while trying to upload the workflow");
      e.printStackTrace();
      return (new ServerResponse(ServerResponse.LOCAL_FAILURE, null));
    }
  }
  ...
  public ServerResponse addFavourite(Resource resource) {
    try {
      ...
      return (response);
        ...
        e.printStackTrace();
      System.out.println("Failed while trying to add an item (" + resource.getURI()
          + ") to favourites4");
      return (new ServerResponse(ServerResponse.LOCAL_FAILURE, null));
    }
  }
  ...
  public ServerResponse deleteFavourite(Resource resource) {
    try {
      ...
      System.out.println("Failed while trying to remove an item ("
