...
/**
 * Adapted from net.sf.taverna.t2.ui.perspectives.myexperiment.model.PackItem.
 *
...
public class PackItem extends Resource {
  ...
  public static PackItem buildFromXML(Document doc) {
	...
	try {
	  ...
	  if (strID == null || strID.equals("")) {
		...
		System.out.println("Error while parsing pack item XML data - no ID provided for pack item with uri: \""
			+ p.getURI() + "\"");
	  } else {
		p.setID(strID);
	  }
	  ...
	  System.out.println("Found information for pack item with URI: " + p.getURI());
	} catch (Exception e) {
            e.printStackTrace();
	  System.out.println("Failed midway through creating pack item object from XML.");
	}
