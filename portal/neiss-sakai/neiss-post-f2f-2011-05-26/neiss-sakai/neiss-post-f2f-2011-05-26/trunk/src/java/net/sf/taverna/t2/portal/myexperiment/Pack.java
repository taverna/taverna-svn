...
/**
 * Adapted from net.sf.taverna.t2.ui.perspectives.myexperiment.model.pack.
...
public class Pack extends Resource
{
  ...
  public static Pack buildFromXML(Document doc, MyExperimentClient client)
  {
    ...
    return (Pack.buildFromXML(doc.getRootElement(), client));
  }


  // class method to build a pack instance from XML
  @SuppressWarnings("unchecked")
  public static Pack buildFromXML(Element docRootElement, MyExperimentClient client)
  {
    ...
    try {
      ...
      if (id == null || id.equals("")) {
        ...
        System.out.println("Error while parsing pack XML data - no ID provided for pack with title: \"" + docRootElement.getChildText("title") + "\"");
      }
      ...
      if (itemsElement != null) {
        ...
        for (Element e : itemsNodes) {
          ...
          PackItem piCurrentItem = PackItem.buildFromXML(docCurrentItem);
      ...
      System.out.println("Found information for pack with ID: " + p.getID() + ", Title: " + p.getTitle());
    }
    catch (Exception e) {
        e.printStackTrace();
      System.out.println("Failed midway through creating pack object from XML.");
   }
