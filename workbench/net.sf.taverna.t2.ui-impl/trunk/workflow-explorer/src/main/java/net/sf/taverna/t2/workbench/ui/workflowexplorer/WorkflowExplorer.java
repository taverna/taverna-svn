...
public class WorkflowExplorer extends JPanel implements UIComponentSPI {
	...
	public void initComponents() {
		...
		jspTree = new JScrollPane(wfTree, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		jspTree.setBorder(new EtchedBorder());
		
		// Title
		JLabel wfExplorerLabel = new JLabel("Workflow Explorer");
		wfExplorerLabel.setMinimumSize(new Dimension(0,0)); // so that it can shink completely
		...
		if (df != null){
			...
			wfTree.addMouseListener(new MouseAdapter(){
				...
				public void mouseClicked(MouseEvent evt){
					...
					int selRow = wfTree.getRowForLocation(evt.getX(), evt.getY());
					if (selRow != -1) {
						// Get the selection path for the row
						TreePath selectionPath = wfTree.getPathForLocation(evt.getX(), evt.getY());
						if (selectionPath != null){
							// Get the selected node
							DefaultMutableTreeNode selectedNode = (DefaultMutableTreeNode) selectionPath.getLastPathComponent();
													
							// For both left and right click - add the workflow object to selection model
							// This will cause the node to become selected (from the selection listener's code)
							DataflowSelectionModel selectionModel = DataflowSelectionManager
							.getInstance().getDataflowSelectionModel(workflow);
							
							// If the node that was clicked on was inputs, outputs, processors,
							// datalinks or nested workflow root node than just make it selected 
							// and clear the selection model (these are just containers for the 'real'
							// workflow components). 
							// Also if the selected node is the root of the nested workflow - do not
							// add to selection model as we do not want to show contextual view for it
							if(selectedNode.getUserObject() instanceof String ||
								((selectedNode.getUserObject() instanceof Dataflow) && (!selectedNode.isRoot())) ){
								...
								if (evt.getButton() == MouseEvent.BUTTON3) {
									...
									JPopupMenu menu = ContextMenuFactory.getContextMenu(workflow,
											selectedNode.getUserObject(), wfTree);
	...
	private void setSelectedNodes() {
		...
		if (!selection.isEmpty()){
			...
			for (Iterator<Object> iterator = selection.iterator(); iterator.hasNext(); ){
				TreePath path = WorkflowExplorerTreeModel.getPathForObject(iterator.next(), (DefaultMutableTreeNode) wfTree.getModel().getRoot());
				paths[--i] = path;
			}
