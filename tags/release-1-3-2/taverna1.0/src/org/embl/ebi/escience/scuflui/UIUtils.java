...
public class UIUtils {
    ...
    /**
     * Implement this interface and set the DEFAULT_FRAME_CREATOR field to change the
     * behaviour of the windowing system used by the Taverna Workbench
     */
     ...
     * platforms such as Mac OS X where the expected windowing behaviour is different
     * from the default desktop pane the default frame creator can be overridden
     * to produce whatever top level window is required - the Workbench class contains
     * code to do this in the case of both OS X and other window systems where the
     * desktop pane is not required.<p>
     * This method will handle the appropriate logic to bind to and unbind from a model
     * when given an implementation of the ScuflUIComponent interface. It's worth noting
     * that, in addition to implementing this interface, any object passed in as the 
     * target component must also be a subclass of JComponent!
     */
    ...
    /**
     * As for the method above but allows a non-ScuflUIComponent JComponent. Internally
     * this component is wrapped up in a trivial ScuflUIComponent which entirely ignores
     * the workflow model settings.
     */
    public static void createFrame(JComponent rawComponent, int posX, int posY, int width, int height) {
	ScuflUIComponent p = new WrapperFrame(rawComponent);
	createFrame(null, p, posX, posY, width, height);
    }
    
    /**
     * Trivial implementation of ScuflUIComponent to wrap a JComponent,
     * ignores all model handling methods.
     */
