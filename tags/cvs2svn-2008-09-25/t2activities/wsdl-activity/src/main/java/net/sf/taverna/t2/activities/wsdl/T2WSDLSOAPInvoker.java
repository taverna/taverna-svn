...
//		soapHeaders.add(new SOAPHeaderElement((Element) wsrfDoc
	...
	protected void configureSecurity(Call call) {
		...
		// alias to fetch it
		// from the Keystore. As alias value we use wsdlLocation so that the
		// credential
		// is tied to a particular service. Once Security Agent picks up the
		// alias, it will
		...
		// before our T2DoAllSender takes over)
		// the USERNAME_PROPERTY to be set to whatever non-empty value for
