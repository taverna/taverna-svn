...
public class PlaygroundPopupGraphMousePlugin extends
	...
	protected void handlePopup(MouseEvent e) {
		...
		if (ps != null) {
			...
			final Vertex vertex = ps.getVertex(inversePoint.getX(),
					inversePoint.getY());
			final Edge edge = ps.getEdge(inversePoint.getX(), inversePoint
					.getY());
			...
			if (vertex != null) {
							...
							Thread t = new Thread() {
								...
								public void run() {
									((PlaygroundProcessorObject) vertex)
											.setRunning(true);
									ArrayList<PlaygroundObject> result = PlaygroundObjectModel
											.run(
													(PlaygroundProcessorObject) vertex,
													pp.isRecording());
					...
					popupMenu.add(new AbstractAction("Get Processor Details") {
						...
						public void actionPerformed(ActionEvent e) {
							...
							Thread t = new Thread() {
								...
								public void run() {
									playgroundMobyPanel
											.Set(((PlaygroundProcessorObject) vertex)
													.getProcessor());
								}
							};
				...
				if (vertex instanceof PlaygroundDataObject) {
					...
					popupMenu
							.add(new AbstractAction("Get Data Object Details") {
								public void actionPerformed(ActionEvent e) {
									...
									Thread t = new Thread() {
										...
										public void run() {
											playgroundMobyPanel
													.Set(((PlaygroundDataObject) vertex)
															.getProcessor());
										}
									};
					...
					popupMenu.add(new AbstractAction("Expose id & namespace") {
						public void actionPerformed(ActionEvent e) {
							...
							ArrayList ports = new ArrayList(
									((PlaygroundDataObject) vertex)
											.getInputPortObjects().values());
							Iterator iterator = ports.iterator();

							while (iterator.hasNext()) {

								PlaygroundPortObject ppo = (PlaygroundPortObject) iterator
										.next();
								if (ppo.isInvisible()
										&& (ppo.getName()
												.equalsIgnoreCase("id") || ppo
												.getName().equalsIgnoreCase(
														"namespace"))) {
									pp.getPlaygroundModel().addPort(
											(PlaygroundDataObject) vertex, ppo);
									System.out.println("Component  " + ppo);
									Point2D point = vl
											.getLocation((PlaygroundDataObject) vertex);
									vl
											.setLocation(
													ppo,
													new Point2D.Double(
				...
				popupMenu.add(new AbstractAction("Delete") {
					public void actionPerformed(ActionEvent e) {
						...
						for (Iterator i = pickedVerticies.iterator(); i
								.hasNext();) {
				...
				popupMenu.add(new AbstractAction("Create Data Item") {
					public void actionPerformed(ActionEvent e) {
						...
						vl
								.setLocation(d, vv
										.inverseViewTransform(inversePoint));
						Layout layout = vv.getGraphLayout();
						for (Iterator i = graph.getVertices().iterator(); i
								.hasNext();) {
							layout.lockVertex((Vertex) i.next());
