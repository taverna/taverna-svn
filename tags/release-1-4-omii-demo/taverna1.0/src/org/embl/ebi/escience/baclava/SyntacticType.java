...
/**
 * A Syntactic type representing what a DataThing is composed from. The
 * recognised collection types are Set, List, Tree and Partial Order. The atomic
 * types are represented by MIME types.
 * 
...
public class SyntacticType {
	...
	public static final Collection PARTIAL_ORDER = new Collection(
			"Partial Order", "p");
	...
	/**
	 * Convert a string version of a syntactic type into a SyntacticType
	 * instance. This should round-trip with toString().
	 * 
	 * @param val
	 ...
	 * the element type will be null. Otherwise, the element type will be
	 * guessed using the first element of the collection.
	 * 
	 * @param example
	 *            an example Object
	 * @throws IllegalArgumentException
	 *             if the example can't be processed
	 */
	public static SyntacticType introspect(Object example) {
		if (example instanceof java.util.Collection) {
			...
			return new SyntacticType(MimeTypeGuesser.DEFAULT
	 ...
	 * 
	 * @param elementType
	 *            the type of each element in the collection
	 * @param collectionType
	 *            the type of the collection (List, Set etc.)
	...
	/**
	 * Create a new synctactic type from a list of mimetypes. This will be
	 * atomic.
	 * 
	 * @param mimeTypes
	 *            a List of Strings giving the mimetypes to use
	 * @throws NullPointerException
	 ...
	 * collection. If this is an atomic type it will return the type of the atom
	 * directly.
	 * 
	...
	public static SyntacticType collectionOf(SyntacticType type,
	...
	public int hashCode() {
		...
		result = 29 * result
				+ (mimeTypeList != null ? mimeTypeList.hashCode() : 0);
		return result;
	}
