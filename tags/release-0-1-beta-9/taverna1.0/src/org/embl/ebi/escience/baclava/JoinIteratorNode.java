 ...
 * which wraps a JoinIterator.
 * @author Tom Oinn
 */
public class JoinIteratorNode extends DefaultMutableTreeNode implements ResumableIterator {
	...
	iterator = new JoinIterator() {
		...
		ResumableIterator[] iterators() {
		    ResumableIterator[] result = new ResumableIterator[JoinIteratorNode.this.getChildCount()];
		    for (int i = 0; i < JoinIteratorNode.this.getChildCount(); i++) {
			result[i] = (ResumableIterator)JoinIteratorNode.this.getChildAt(i);
		    }
		    return result;
		}
