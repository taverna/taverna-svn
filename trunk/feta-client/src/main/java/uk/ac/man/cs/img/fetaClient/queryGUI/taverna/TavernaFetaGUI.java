...
public class TavernaFetaGUI extends JTabbedPane implements ScuflUIComponent,
	...
	public void drop(DropTargetDropEvent e) {
		...
		/*
		 * try { XMLOutputter xo = new XMLOutputter(); org.jdom.Element
		 * droppedProcessorElement = null; setCursorBusy(); DataFlavor f =
		 * SpecFragmentTransferable.factorySpecFragmentFlavor; DataFlavor p =
		 * SpecFragmentTransferable.processorSpecFragmentFlavor;
		 * 
		 * boolean attemptToRecognize = false; Transferable t =
		 * e.getTransferable();
		 * 
		 * if (e.isDataFlavorSupported(f)) { FactorySpecFragment fsf =
		 * (FactorySpecFragment)t.getTransferData(f); System.out.println("Found
		 * factory spec fragment");
		 * 
		 * System.out.println(xo.outputString(fsf.getElement())); String
		 * validName = model.getValidProcessorName(fsf.getFactoryNodeName());
		 * attemptToRecognize = true;
		 * 
		 * droppedProcessorElement = fsf.getElement();
		 *  } else if (e.isDataFlavorSupported(p)) {
		 * 
		 * attemptToRecognize = true; ProcessorSpecFragment psf =
		 * (ProcessorSpecFragment)t.getTransferData(p); // Remove the various
		 * fault tolerance etc attributes Element processorElement =
		 * psf.getElement(); List attributes = processorElement.getAttributes();
		 * for (Iterator i = attributes.iterator(); i.hasNext();) { Attribute
		 * att = (Attribute)i.next(); processorElement.removeAttribute(att); }
		 * 
		 * System.out.println("Found processor spec fragment");
		 * 
		 * System.out.println(xo.outputString(processorElement));
		 * 
		 * droppedProcessorElement = processorElement; }//else-if
		 * 
		 * if (attemptToRecognize==true){ List fetaDescURIs =
		 * helper.reverseLookup(droppedProcessorElement);
		 * 
		 * if (fetaDescURIs != null){
		 * 
		 * int returnVal = JOptionPane.showConfirmDialog(this, "There already
		 * exists a Feta description for the service you want to annotate.\n " +
		 * "Do you want the annotator to be launched with the existing
		 * annotation ", "Annotation Exists ", JOptionPane.INFORMATION_MESSAGE);
		 * 
		 * if (returnVal == JOptionPane.YES_OPTION) {
		 * 
		 * //we get the first one for now
		 * GUIUtil.launchAnnotator((String)fetaDescURIs.get(0));
		 *  } else if (returnVal == JOptionPane.NO_OPTION) { //setCursor (new
		 * Cursor (Cursor.WAIT_CURSOR));
		 * GUIUtil.launchAnnotator((org.w3c.dom.Document)invokeImporter(droppedProcessorElement));
		 *  } else { //User pressed the cancel button e.rejectDrop(); }
		 * 
		 * }else {
		 * GUIUtil.launchAnnotator((org.w3c.dom.Document)invokeImporter(droppedProcessorElement));
		 * 
		 ...
		 * 
		 * 
		 * }//try catch (Exception ex) { e.rejectDrop();
		 *  } unsetCursorBusy();
		 * 
		 */
	}
