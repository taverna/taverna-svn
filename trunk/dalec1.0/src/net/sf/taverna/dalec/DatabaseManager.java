 ...
 * automatically within a separate thread.  One file per sequence is created within the database.
 * <p/>
 * A client can: <il>Call <code>getPendingResults()</code> to retrieve a list of all results yet to be written,</il>
 * <il>Then call </code>writeToFile()</code> to write all pending results to the Database.</il> Each method should be
 * invoked in turn.
 * <p/>
 * Alternatively, the method <code>DatabaseManager.run()</code> can be called - it is intended that this is used by
 * creating a new thread and supplying DatabaseManager as it's Runnable.  This automatically enters sequences into the
 * database as they are generated. This is the recommended method.
 * <p/>
 * However, if a client is to use the "manual" method of writing results to the database, it should be considered that
 * every workflow instance submits its results to DatabaseManager as they are generated, and that these results are
 * retained in memory until written.  Alternatively, an instance of DatabaseManager can be run in a dedicated thread. In
 * this case, results are handled automatically, and are dynamically written to the database as they are generated. If
 * there are no pending results, DatabaseManager will wait until new results are submitted. Otherwise, any instance of
 * this class will run continuously whilst results need to be written to a file.
 *
...
public class DatabaseManager extends Thread implements Runnable
{
     ...
     * database in the order in which they are generated, and stored in a buffer until they can be written.
     *
    ...
    /**
     * Returns a List representing all data which haa <i>yet to</i> be written to the database - they are held in an
     * in-memory cache until they are written to disk.  All elements within this List should be instances of
     * GFFEntrySet.
     *
