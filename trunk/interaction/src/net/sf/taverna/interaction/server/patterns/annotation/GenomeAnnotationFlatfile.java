...
public class GenomeAnnotationFlatfile extends AbstractServerInteractionPattern {
	...
	private static Logger log = Logger.getLogger(GenomeAnnotationFlatfile.class);
	
	/* (non-Javadoc)
	 * @see net.sf.taverna.interaction.server.ServerInteractionPattern#getMessageBody(java.net.URL, net.sf.taverna.interaction.server.InteractionState)
	 */
	public String getMessageBody(URL baseURL, InteractionState state) {
		...
		try {
			...
			URL acceptURL = new URL(baseURL, "client/upload?id="+id+"&response=accept&manualchange=no");
		    URL rejectURL = new URL(baseURL, "client/upload?id="+id+"&response=reject");
		    
		    URL artemisAppletURL = new URL(baseURL, "client/download?id="+id+"&launch=Artemis");
            ...
            String title = (String) ((DataThing)dataThings.get("title")).getDataObject();
		    String comment = (String) ((DataThing)dataThings.get("comment")).getDataObject();
		    
		    message.append("This is an interaction request for genome annotation flatfiles. ");
		    message.append("The workflow designer has requested that you to review the annotation "+title+".");
		    ...
		    message.append("When you have finished reviewing the data you can use the Artemis client to accept the data, submit modifications or reject the data.\n\n");
	...
	public void handleInputDownload(HttpServletRequest request,
		...
		// TODO For coming multifile version: serve files one after another during request.getParameter(fileIndex)
	    
	    try { 
	        //	The default behaviour is to launch the Artemis applet which handles data download
	        if (request.getParameter("launch") != null) {                              
	            response.setContentType("application/x-java-jnlp-file");
                Document jnlp = new ArtemisInteractionJNLP(state.getID(), server.getBaseURLString());                
	        ...
	        // If the launch parameter is not given the result will be downloaded as plain text
	        else {
	            response.setContentType("text/plain");
                PrintWriter writer = response.getWriter();
                Document doc = state.getInputDocument();
                Map dataThings = DataThingXMLFactory.parseDataDocument(doc);
                String flatfileContent = (String) ((DataThing)dataThings.get("annotation")).getDataObject();
    ...
    public void handleResultUpload (HttpServletRequest request,
        ...
        /* The HTTP Post is made up by exactly 5 parameters all sent as UTF-8 encoded Strings:
         * id: the ID of the interaction request
         * response: "accept" or "reject"
         * manualchange: "yes" or "no" depending on if manual changes are submitted 
         * modified: the modified result, if it exists
         * notes: the expert reviewers notes about modifications made
         * 
         * All of these should be present and only contain the values described.
         * However, the interaction will only be failed if 
        ...
        try {           
            ...
            boolean manualChangeMade = (manualChanges != null && manualChanges.equals("yes"));
            if (manualChangeMade && modified == null) {
                badRequest("Manual changes indicated but modified data parameter does not exist.",
                        response, state);
                return;
            }
            ...
            else dataThingMap.put("decision", new DataThing(responseValue));             
            ...
            if (responseValue.toLowerCase().equals("accept")) {           
                ...
                if (   modified == null || 
                       (manualChanges != null && manualChanges.toLowerCase().equals("no")) ) {
                    ...
                    Map inputThings = DataThingXMLFactory.parseDataDocument(inputDoc);
                    dataThingMap.put("reviewed_result", inputThings.get("annotation"));                
                }
                else if (modified != null) dataThingMap.put("reviewed_result", new DataThing(modified));                
            }
            else dataThingMap.put("reviewed_result", new DataThing("request rejected"));
    ...
    private static void badRequest (String message, HttpServletResponse response, InteractionState state) 
    throws IOException {
        log.error(message);
