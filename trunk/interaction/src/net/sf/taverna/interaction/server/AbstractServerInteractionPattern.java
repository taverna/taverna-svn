 ...
 * download and results upload plugins. The default behaviour for the
 * request data download is to serve back the xml data file directly without
 * any transformation, for the results upload it just dumps the filepart
 * named 'results' into the results file and marks the interaction as completed.
 * <p>This behaviour is almost certainly too simple for many cases - it
 * inherently requires the Taverna DataThing library on the client which is
 * probably overkill. The intention is therefore that almost all pattern
 * implementations will override these methods to perform some manner of data
 * marshalling and unmarshalling, moving the burden of handling the complex
 * encoded XML onto the server and allowing extremely lightweight clients, the
 * only real requirement from the client is that it should be able to send
 * HTTP POST requests (this being the only sane way of getting data back to
 * the server), or, in the degenerate case, HTTP GET to send a simple 'yes / no' 
 * type response.
 * @author Tom Oinn
 */
public abstract class AbstractServerInteractionPattern 
     ...
     * interaction job. By default this implementation just returns the
     * input document as an XML stream.
     */
    ...
    /**
     * Handle a result upload. By default this method just dumps the 
     * contents of the POSTed data, assuming a data item named 'data'
     * and writing it to the result file before messaging the state
     * object that it's been completed. If anything goes wrong the
     * state object is messaged as failing.
     */
	...
	try {
	    ...
	    if (!isMultipart) {
		log.debug("Response isn't a multipart file upload, failing the interaction");
		state.fail();
		return;
	    }
	    ...
	    log.error("Exception thrown whilst trying to handle upload, failing interaction", ex);
	    state.fail();
	    try {
		response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, 
				   ex.getMessage());
