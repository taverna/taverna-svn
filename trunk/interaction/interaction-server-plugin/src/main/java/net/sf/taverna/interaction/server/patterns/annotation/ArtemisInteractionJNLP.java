...
/**
 * Generates a new Java Network Launching Protocol (JNLP) XML Document for launching
 * the modified Artemis viewer.
 * 
 *  @author andersl
 */
...
public class ArtemisInteractionJNLP extends JNLPDocument {
     ...
     */
     static final String jarLocation = 
        "http://www.bioinfo.no/services/annotation/artemis_interaction/";    
  
     /**
      * Create JNLP document with the appropriate parameters for this 
      * interaction request 
      * @param interactionID the ID of the interaction request
      * @param interactionURL the URL of the interaction server
      */
    public ArtemisInteractionJNLP(String interactionID, String interactionURL) {
        ...
        String interactionUrlTrimmed = (interactionURL.charAt(interactionURL.length()-1)=='/')? interactionURL : interactionURL + "/";    
        //this.setSelfLocation(interactionUrlTrimmed + "client/download?id="+interactionID+"&launch=Artemis");
        // Although formally required, self reference is not set since this casues a bug in javaws when launching for the second time
        ...
        this.addDescription("Modified version of Artemis for Interaction Server");
        this.addDescription(
