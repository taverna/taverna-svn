...
/**
 * Task to send an interaction request to the interaction service, wait
 * for a response then fetch any result data and proceed.
 * @author Tom Oinn
 */
public class InteractionServiceTask implements ProcessorTaskWorker {
    
    private InteractionServiceProcessor processor;
    private static Logger logger = Logger.getLogger(InteractionServiceTask.class);
     ...
     * inputs and properties of the processor to construct an InteractionRequest,
     * submit this to the InteractionService defined by the processor then
     * wait for completion or failure events from the InteractionReceipt
     */
	...
	try {
		...
		throw new TaskExecutionException("Must specify an email address for the "+
		...
		if (inputMap.containsKey(inputPortName) == false) {
		    throw new TaskExecutionException("Required input '"+inputPortName+"' "+
						     "is not bound, failing.");
		}
	    }
	    ...
	    Date expiryTime = new Date(currentTime + processor.getRequestLifetime());
	    
	    // Use the InteractionService to submit a new InteractionRequest based
	    // on the properties gathered so far
	    InteractionRequest request = 
		DefaultInteractionRequest.createRequest(processor.getInteractionPattern(),
							email,
							otherInputs,
							expiryTime);
		...
		throw new TaskExecutionException("Submission to interaction service failed", se);
	    }
	    
	    // Create a new callback for the InteractionReceipt then go to sleep,
	    ...
	    receipt.addInteractionStateListener(new InteractionStateListener() {
			...
			if (receipt.getInteractionStatus() 
			    instanceof TerminalInteractionStatus) {
