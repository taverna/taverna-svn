		...
		try {
			...
			for (Iterator i = playgroundPortObjects.iterator(); i.hasNext();) {
				...
				if (recording) {
					...
					if (!(p.getName().contains(new StringBuffer("Collection")) && !p
							.getName().contains(new StringBuffer("As Simples")))) {
			...
			for (Iterator i = resultMap.keySet().iterator(); i.hasNext();) {
				...
				for (Iterator j = portNames.iterator(); j.hasNext();) {
					...
					if (portNameArticle.contains(new StringBuffer("namespace"))) {
						...
						.get("namespace");
						portObject.setMappedObject(new PlaygroundDataThing(
						...
						if (portName.indexOf("'") != -1) {
							...
							.get("id")
							.setMappedObject(
									new PlaygroundDataThing(
											(DataThing) workflowResultMap
											.get(portName),
											portName.substring(portName
													.lastIndexOf("_"))));
							...
							.get("id");
							portObject
							.setMappedObject(new PlaygroundDataThing(
									(DataThing) workflowResultMap
					...
					} else if (portNameArticle.contains(new StringBuffer("ns"))) {
