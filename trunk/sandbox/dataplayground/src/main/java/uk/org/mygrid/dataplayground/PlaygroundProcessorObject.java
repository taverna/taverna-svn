	...
	public PlaygroundProcessorObject(BiomobyProcessor p) {
		...
		setName(p.getServiceName());
			...
			for (int i = 0; i < inPorts.length; i++) {
				...
				if (port.getName().equals("namespace") || port.getName().equals("id")
						|| port.getName().equals("article name")|| port.getName().equals("input")) {
					continue;
				}
			...
			for (int i = 0; i < outPorts.length; i++) {
				...
				if (port.getName().equals("namespace") || port.getName().equals("id")
						|| port.getName().equals("article name")|| port.getName().equals("output")) {
					continue;
				}
...
//returns true if we were sucessful or false otherwise n.b. a "sucessful" workflow run may not 
//mean that we have a valid output 
	...
	public boolean combine(Vertex v) {
		...
		// map the output from the Object (mobyData) to the input of the processor
		// make the workflow output the output(s) from this processor excluding "output"
		...
		if(v instanceof PlaygroundDataObject){
			...
			System.out.println("Combine " + this + " " + v);
			Map <String,DataThing>inputMap = new HashMap<String,DataThing>();
			
			//easiest case if we only have 1 input then just map the dataObject to the input and
			//run a workflow using the dataObjects dataThing(if it has one)
			if(inputPortObjects.size() == 1){
				
				model.addProcessor(p);
				BiomobyObjectProcessor object = d.getProcessor();
				model.addProcessor(d.getProcessor());
				try {
				
					model.addDataConstraint(new DataConstraint(model,object.locatePort("mobyData"),inputPortObjects.iterator().next().getPort()));
				...
				if(d.getDataComponents().size() > 0){					
					...
					inputMap = addComponentsToModel(model,d);
				
				}
				else{
				//we have just a simple data object	
				//add workflow input connecting to the dataObject and create map with this dataObjects dataThing (if connected)	
				
					try {
						
						PlaygroundDataThing playgroundDataThing = d.getData();
						
						if(playgroundDataThing == null)
							return false;
						...
						//loop through the Objects inputs till we find one that isn't input , namespace , etc
						
						//InputPort[] ports = object.getInputPorts();
						
				//		for (int i = 0; i < ports.length; i++) {
				//			InputPort p = ports[i];
				//			
				//			if ( p.getName().equals("value")) {
				//				 model.addDataConstraint(new DataConstraint(model,newInput,p));
				//			}
				//		}
		...
		return true;
			...
			//adds the DataObjects components to the model and returns the input map required 
			//for this part of the generated workflow 
			
			public Map<String,DataThing> addComponentsToModel(ScuflModel model , PlaygroundDataObject parent ){
				
				Map <String,DataThing>result = new HashMap<String,DataThing>();
				
				BiomobyObjectProcessor processor = parent.getProcessor();
				ArrayList<PlaygroundDataObject> dataComponents = parent.getDataComponents();
				...
				for(Iterator i = dataComponents.iterator() ; i.hasNext();){
					...
					try {
						...
						model.addDataConstraint(new DataConstraint(model,childProcessor.locatePort("mobyData"),processor.locatePort(child.getName())));
					    if(child.getDataComponents().size() > 0){
						addComponentsToModel(model,child);
					    }
					    else{
					    	
					    	//create input and add to map the dataThing & input port mapping
