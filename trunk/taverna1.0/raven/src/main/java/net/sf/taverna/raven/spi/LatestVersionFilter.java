 ...
 * are sorted as in VersionComparator.
 * 
...
public class LatestVersionFilter extends AbstractArtifactFilter  {
	...
	public Set<Artifact> filter(Set<Artifact> artifacts) {
		SortedSet<Artifact> sortedArtifacts = 
			new TreeSet<Artifact>(VersionComparator.getInstance());
		sortedArtifacts.addAll(artifacts);
		Set<Artifact> results = new HashSet<Artifact>();
		Artifact previous = null;
		for (Artifact artifact : sortedArtifacts) {
			if (previous != null && (
				 previous.getGroupId() != artifact.getGroupId() ||
				 previous.getArtifactId() != artifact.getArtifactId())) {
