...
/**
 * A processor that uses the BSF scripting engine
 * to allow arbitrary scripts to be invoked on workflow
...
public class BSFProcessor extends Processor implements java.io.Serializable {
     ...
     * port name arrays.
     */
    public BSFProcessor(ScuflModel model, String name, String script, String[] inputs, String[] outputs)
	...
	try {
	    ...
	    throw new ProcessorCreationException("The supplied specification for the BSF script processor '"+
						 name+"' contained a duplicate port '"+
						 dpne.getMessage()+"'");
	}
	catch (PortCreationException pce) {
	    throw new ProcessorCreationException("An error occured whilst generating ports for the BSF processor "+pce.getMessage());
	}
    }
