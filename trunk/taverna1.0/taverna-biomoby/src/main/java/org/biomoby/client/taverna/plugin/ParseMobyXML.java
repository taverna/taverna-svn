...
public class ParseMobyXML {
	 ...
	 * 
	 * @param names
	 *            an array of article names in the order that we will extract
	 *            our information
	 * @param type
	 *            one of {NAMESPACE | ID | VALUE} denoting what exactly it is
	 ...
	 */
	public static ArrayList<String> getContentForDataType(ArrayList<String> names, int type, String xml) {
		...
		try {
			contents = MobyDataUtils.fromXMLDocument(new ByteArrayInputStream(xml.getBytes("UTF8")));
		} catch (Exception e) {
	...
	private static void recurse(ArrayList<String> names,
			MobyDataInstance data, ArrayList<String> output, int type) {
		...
		if (data instanceof MobyDataObjectVector) {
			// recurse on the children -- the recursion will extract by articlename
			...
			for (Iterator i = vector.iterator(); i.hasNext();) {
				recurse((ArrayList)(names.clone()), (MobyDataInstance)i.next(), output, type);	
			}
			
		} else if (data instanceof MobyDataComposite) {
			// recurse on the child given by name.get(0)
			MobyDataInstance d = ((MobyDataComposite)data).remove(names.remove(0));
	...
	private static void baseCase(MobyDataInstance data, ArrayList<String> output, int type) {
		if (data == null)
			return;
		switch (type) {
		case NAMESPACE: {
			if (data instanceof MobyDataObjectVector) {
				MobyDataObjectVector vector = (MobyDataObjectVector)data;
				for (Iterator i = vector.iterator(); i.hasNext();) {
					MobyNamespace[] namespaces = ((MobyDataObject)i.next()).getNamespaces();
					for (int j = 0; j < namespaces.length; j++) {
						output.add(namespaces[j].getName());
					}
				}
			} else {
				MobyNamespace[] namespaces = ((MobyDataObject)data).getNamespaces();
				for (int j = 0; j < namespaces.length; j++) {
					output.add(namespaces[j].getName());
				}
			}
