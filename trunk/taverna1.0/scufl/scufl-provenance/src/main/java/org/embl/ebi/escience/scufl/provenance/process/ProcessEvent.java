 ...
 * Subclasses extend this to show information for each interesting transition.
 * 
 * @author Tom Oinn
 */
public abstract class ProcessEvent {
	...
	/**
	 * Subclass this method to add information to the Element returned, the
	 * eventTopLevelElement will build the enclosing element for you.
	 */
	...
	public Element eventTopLevelElement() {
		...
		String leafClassName = (eventClass.getName().split("\\."))[eventClass
					...
					if (ProcessEvent.class.isAssignableFrom(declaringClass)) {
						...
						if (methods[i].getParameterTypes().length == 0) {
							...
							if (methods[i].getReturnType().equals(String.class)) {
								...
								// Invoke the method.
								String attributeValue = (String) methods[i]
										.invoke(this, new Object[0]);
								theElement.setAttribute(attributeName,
										attributeValue);
							} else {
