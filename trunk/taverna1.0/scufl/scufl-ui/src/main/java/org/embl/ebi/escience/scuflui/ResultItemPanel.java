...
public class ResultItemPanel extends JPanel {
	...
	class ChangeAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			String newData =
				((javax.swing.text.JTextComponent) component).getText();
		...
		if (!isEmptyCollection) {
			...
			structureTree.addTreeSelectionListener(new TreeSelectionListener() {
				public void valueChanged(TreeSelectionEvent e) {
					DataThingTreeNode node =
						(DataThingTreeNode) structureTree.getLastSelectedPathComponent();
					if (node != null /* && node.isLeaf() */) {
						...
						if (renderer != null) {
							try {
								...
								if (component != null) {
									...
									JPanel rightPanel =
										new JPanel(new java.awt.FlowLayout());
									JPanel leftPanel =
										new JPanel(new java.awt.BorderLayout());
										...
										public void actionPerformed(
											...
											if (node != null /*
												...
												if (component instanceof javax.swing.text.JTextComponent) {
													...
													if (!oldData.equals(newData)) {
														String oldLSID =
															newDataThing.getLSID(newDataThing.getDataObject());
														DataThing thing =
															newDataThing.drillAndSet(
																dataThing,
																newData);
														if (thing != null
															...
															((DefaultMutableTreeNode) tn).removeAllChildren();
															DefaultMutableTreeNode root =
																(DefaultMutableTreeNode) structureTree.getModel().getRoot();
															root.removeAllChildren();
															DataThingTreeNode rootDataThing =
																DataThingTreeFactory.getTree(newDataThing);
		...
		structureTree.addMouseListener(new MouseAdapter() {
			...
			void doEvent(MouseEvent e) {
				final DataThingTreeNode node =
					(DataThingTreeNode) (structureTree.getPathForLocation(
				...
				JMenuItem saveAction =
					new JMenuItem("Save to file", TavernaIcons.saveIcon);
				saveAction.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent ae) {
						try {
							...
							String curDir =
								prefs.get("currentDir",
									System.getProperty("user.home"));
							fc.setCurrentDirectory(new File(curDir));
							int returnVal =
								fc.showSaveDialog(ResultItemPanel.this);
				...
				JMenuItem copyAction =
					new JMenuItem("Copy to Clipboard", TavernaIcons.copyIcon);
						...
						try {
							...
							if (theDataObject instanceof byte[]) {
								BytesSelection bytesSelection =
									new BytesSelection((byte[]) theDataObject);
				...
				Iterator renderers =
					ResultItemPanel.this.renderers.getRenderers(nodeThing).iterator();
				if (!isEmptyCollection && renderers.hasNext()) {
					...
					while (renderers.hasNext()) {
						...
						viewers.add(new JMenuItem(new AbstractAction(
							renderer.getName(), renderer.getIcon(
							...
							public void actionPerformed(ActionEvent e) {
								try {
									...
									if (ui != null) {
										...
										JButton cmdEdit = new JButton("Change");
										cmdEdit.addActionListener(new ChangeAction(
											component, newDataThing, tn,
											structureTree));
										panel.removeAll();
										leftPanel.add(jp);
										rightPanel.add(cmdEdit);
