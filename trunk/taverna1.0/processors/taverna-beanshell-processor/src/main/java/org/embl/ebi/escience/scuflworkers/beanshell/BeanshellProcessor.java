 ...
 * @author Tom Oinn
 */
public class BeanshellProcessor extends DependencyProcessor implements
	Serializable {
	 ...
	 * port name arrays.
	 */
	public BeanshellProcessor(ScuflModel model, String name, String script,
		String[] inputs, String[] outputs) throws ProcessorCreationException,
		...
		try {
			...
			throw new ProcessorCreationException(
				"The supplied specification for the beanshell processor '"
					+ name + "' contained a duplicate port '"
					+ dpne.getMessage() + "'");
		} catch (PortCreationException pce) {
			throw new ProcessorCreationException(
				"An error occured whilst generating ports for the beanshell processor "
					+ pce.getMessage());
		}
	}
