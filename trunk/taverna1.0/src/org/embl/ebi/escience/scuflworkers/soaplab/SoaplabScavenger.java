...
public class SoaplabScavenger extends Scavenger {
	 ...
	 * http://foo.bar/soap/AnalysisFactory the parameter shuld be
	 * http://foo.bar/soap/
	 */
	public SoaplabScavenger(String base) throws ScavengerCreationException {
		super("Soaplab @ " + (base.endsWith("/") ? base : base + "/"));
		// Get the categories for this installation
		try {
			if (!base.endsWith("/")) {
				base = base + "/";
				// throw new RuntimeException("The url for soaplab should look
				// like http://foo/bar/, see the faq for more information");
			}
			...
			try {
				...
				for (int i = 0; i < categories.length; i++) {
					...
					for (int j = 0; j < services.length; j++) {
						SoaplabProcessorFactory f = new SoaplabProcessorFactory(
								base, services[j]);
