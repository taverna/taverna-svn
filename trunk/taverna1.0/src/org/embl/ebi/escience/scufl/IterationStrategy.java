...
public class IterationStrategy {
	...
	Namespace.getNamespace("i","http://org.embl.ebi.escience/xscufliteration/0.1beta10"); 
	...
	MutableTreeNode rootNode = nodeForElement((Element)strategyElement.getChildren().get(0));
	...
	rootElement.addContent(elementForNode((MutableTreeNode)strategyModel.getRoot()));
	return rootElement;
    }
	...
	if (node instanceof LeafNode) {
		...
		combination.addContent(elementForNode((MutableTreeNode)en.nextElement()));
	    }
	    return combination;
	}
    }
     ...
     * objects.
     * @throws IntrospectionException if a leaf iterator is named in the 
     * strategy but doesn't occur in the map
     */
    public ResumableIterator buildIterator(Map iteratorNodes) 
	throws IntrospectionException {
	return iteratorForNode((MutableTreeNode)strategyModel.getRoot(), iteratorNodes);
    }
    private ResumableIterator iteratorForNode(MutableTreeNode node, Map iteratorNodes) 
	throws IntrospectionException {
	if (node instanceof LeafNode) {
	    ...
	    for (Enumeration en = node.children(); en.hasMoreElements();) {
		combination.add((MutableTreeNode)iteratorForNode((MutableTreeNode)en.nextElement(), iteratorNodes));
	    }
