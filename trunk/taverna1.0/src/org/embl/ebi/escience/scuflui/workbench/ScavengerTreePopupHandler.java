...
public class ScavengerTreePopupHandler extends MouseAdapter {
	...
	void doEvent(MouseEvent e) {
		...
		if (path != null) {
			...
			final DefaultTreeModel tmodel = (DefaultTreeModel) scavenger
					.getModel();
			...
			if (scuflObject != null) {
				...
				JMenuItem getDescriptions = new JMenuItem("Fetch descriptions",
						ScuflIcons.zoomIcon);
					...
					public void actionPerformed(ActionEvent ae) {
						...
						new Thread() {
							public void run() {
								...
								for (Iterator i = pflist.iterator(); i
										.hasNext();) {
									ProcessorFactory pf = (ProcessorFactory) i
											.next();
									...
									if (pf.getDescription() == null) {
										...
										try {
											...
											pf
													.setDescription("<font color=\"red\">Cannot fetch description!</font>");
											ex.printStackTrace();
										} finally {
				...
				if (scuflObject instanceof ProcessorFactory) {
					...
					JMenuItem test = new JMenuItem("Invoke",
							ScuflIcons.windowRun);
					test.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent ae) {
							try {
								final ScuflModel m;
								if (pf instanceof WorkflowProcessorFactory) {
									WorkflowProcessor wp = (WorkflowProcessor) pf
											.createProcessor("workflow",
													new ScuflModel());
									m = wp.getInternalModel();
								} else {
									m = new ScuflModel();
									Processor p = pf.createProcessor(
											"processor", m);
									// m.addProcessor(p);
									...
									for (int i = 0; i < ip.length; i++) {
										...
										OutputPort port = new OutputPort(m
												.getWorkflowSourceProcessor(),
												portName);
										m.getWorkflowSourceProcessor().addPort(
												port);
										m.addDataConstraint(new DataConstraint(
												m, port, ip[i]));
									}
									OutputPort[] op = p.getOutputPorts();
									for (int i = 0; i < op.length; i++) {
										String portName = op[i].getName();
										InputPort port = new InputPort(m
												.getWorkflowSinkProcessor(),
												portName);
										m.getWorkflowSinkProcessor().addPort(
												port);
										m.addDataConstraint(new DataConstraint(
												m, op[i], port));
									}
								...
								if (m.getWorkflowSourcePorts().length != 0) {
									DataThingConstructionPanel thing = new DataThingConstructionPanel() {
										public void launchEnactorDisplay(
												Map inputObject) {
											try {
												UIUtils
														.createFrame(
									...
									UIUtils.createFrame(m, thing, 100, 100,
											600, 400);
								} else {
									try {
										// No inputs so launch the enactor
										// directly
										UIUtils.createFrame(m,
												new EnactorInvocation(
														FreefluoEnactorProxy
																.getInstance(),
														m, new HashMap()), 100,
					...
					menu.add(new ShadedLabel(pf.getName(),
							ShadedLabel.TAVERNA_GREEN));
					menu.addSeparator();
					menu.add(test);
				} else {
					menu.add(new ShadedLabel(scuflObject.toString(),
							ShadedLabel.TAVERNA_BLUE));
					menu.addSeparator();
				}
				if (scuflObject instanceof ProcessorFactory
						&& scavenger.model != null) {
					final ProcessorFactory pf = (ProcessorFactory) scuflObject;
					// Show the popup for adding new processors to the model
					JMenuItem add = new JMenuItem("Add to model",
							ScuflIcons.importIcon);
					menu.addSeparator();
					menu.add(add);
					JMenuItem addWithName = new JMenuItem(
							"Add to model with name...", ScuflIcons.importIcon);
					menu.add(addWithName);
					// Prepare the 'add as alternate menu'
					Processor[] processors = scavenger.model.getProcessors();
					if (processors.length > 0) {
						JMenu processorList = new JMenu(
								"Add as alternate to...");
						for (int i = 0; i < processors.length; i++) {
							JMenuItem processorItem = new JMenuItem(
									processors[i].getName(), ProcessorHelper
										...
										public void actionPerformed(
											...
											try {
												...
												if (theProcessor.getModel() != null) {
													boolean isOffline = theProcessor
															.getModel()
															.isOffline();
													if (isOffline) {
														alternateProcessor
					...
					if (scuflObject instanceof WorkflowProcessorFactory) {
						JMenuItem imp = new JMenuItem("Import workflow...",
								ScuflIcons.webIcon);
						final String definitionURL = ((WorkflowProcessorFactory) scuflObject)
								.getDefinitionURL();
					...
					addWithName.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent ae) {
							String name = (String) JOptionPane.showInputDialog(
									null, "Name for the new processor?",
									"Name required",
					...
					if (choice.equals("Available Processors")) {
						...
						for (Iterator i = ProcessorHelper
								.getScavengerToTagNames().keySet().iterator(); i
								.hasNext();) {
							String scavengerClassName = (String) i.next();
							ImageIcon scavengerIcon = ProcessorHelper
									.getIconForTagName((String) (ProcessorHelper
											.getScavengerToTagNames()
											.get(scavengerClassName)));
							// Instantiate a ScavengerHelper...
							try {
								Class scavengerHelperClass = Class
										.forName(scavengerClassName);
								ScavengerHelper sh = (ScavengerHelper) scavengerHelperClass
										.newInstance();
								String scavengerDescription = sh
										.getScavengerDescription();
								JMenuItem scavengerMenuItem = new JMenuItem(
										scavengerDescription, scavengerIcon);
								scavengerMenuItem
										.addActionListener(sh
						...
						JMenuItem addWeb = new JMenuItem(
								"Collect scavengers from web...",
								ScuflIcons.webIcon);
						menu.addSeparator();
						menu.add(addWeb);
						addWeb.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent ae) {
								...
								if (rootURL != null) {
									try {
										ScavengerTreePopupHandler.this.scavenger
												.addScavenger(new WebScavenger(
														rootURL,
														ScavengerTreePopupHandler.this.scavenger.treeModel));
						...
						JMenuItem collect = new JMenuItem(
						...
						hideAllNodes.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent ae) {
								...
								scavenger.expandPath(new TreePath(
										scavenger.treeModel.getRoot()));
							}
						});
						...
						// option to remove it
						if (choice.equals("Internal Services") == false
								&& choice.equals("Local Java widgets") == false) {
							// JPopupMenu menu = new JPopupMenu();
							JMenuItem remove = new JMenuItem(
									"Remove from tree", ScuflIcons.deleteIcon);
							...
							remove.addActionListener(new ActionListener() {
								public void actionPerformed(ActionEvent ae) {
									...
									// list
									ScavengerTreePopupHandler.this.scavenger.scavengerList
											.remove(scavengerName);
								}
							});
							menu.add(remove);
							// menu.show(scavenger, e.getX(), e.getY());
