...
public class SaveAsExcel implements ResultMapSaveSPI {
	...
	return new ActionListener() {
		...
		private void setStyle(int currentCol, int column, int row) {
		    ...
		    int e = 0;
		    if (row < 2 ||
			!hasValue(column, row - 1)) {
			n = 1;
		    }
		    if (column == currentCol ||
			!hasValue(column - 1, row)) {
			w = 1;
		    }
		...
		private void doStyle(int column, int row) {
		    HSSFCell targetCell = getCell(column, row);
		    // Test for left border condition
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (Iterator i = resultMap.keySet().iterator(); i.hasNext();) {
			    ...
			    String[] mimeTypes = resultValue.getMetadata().getMIMETypes();
				...
				while(bi.hasNext()) {
				    ...
				    //setStyle(currentCol + columnOffset, baseRow + rowOffset);
				}
				// Set the styles
