...
public class BaclavaDataServiceFactory {
	static Logger log = Logger.getLogger(BaclavaDataServiceFactory.class
			.getName());
	...
	public static BaclavaDataService getStore() {
		...
		if (storageClassName != null) {
			try {
				...
				log.error("Unable to find class: " + storageClassName
						+ " defined by taverna.datastore.class");
				e.printStackTrace();
			} catch (InstantiationException e) {
				log.error("InstatiationException when creating "
						+ storageClassName, e);
			} catch (IllegalAccessException e) {
				log.error("IllegalAccessException when creating "
						+ storageClassName, e);
			}
		}
		return result;
